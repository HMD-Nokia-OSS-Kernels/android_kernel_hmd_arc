...
static int cmd_threadstats(int argc, const cmd_args *argv) {
    for (uint i = 0; i < SMP_MAX_CPUS; i++) {
        ...
        dprintf(INFO,"thread stats (cpu %d):\n", i);
        dprintf(INFO,"\ttotal idle time: %lld\n", thread_stats[i].idle_time);
        dprintf(INFO,"\ttotal busy time: %lld\n", current_time_hires() - thread_stats[i].idle_time);
        dprintf(INFO,"\treschedules: %lu\n", thread_stats[i].reschedules);
#if WITH_SMP
        dprintf(INFO,"\treschedule_ipis: %lu\n", thread_stats[i].reschedule_ipis);
#endif
        dprintf(INFO,"\tcontext_switches: %lu\n", thread_stats[i].context_switches);
        dprintf(INFO,"\tpreempts: %lu\n", thread_stats[i].preempts);
        dprintf(INFO,"\tyields: %lu\n", thread_stats[i].yields);
        dprintf(INFO,"\tinterrupts: %lu\n", thread_stats[i].interrupts);
        dprintf(INFO,"\ttimer interrupts: %lu\n", thread_stats[i].timer_ints);
        dprintf(INFO,"\ttimers: %lu\n", thread_stats[i].timers);
    }
...
static void kevdump_cb(const uintptr_t *i) {
    switch (i[1] & 0xffff) {
        case KERNEL_EVLOG_CONTEXT_SWITCH:
            dprintf(INFO,"%lu.%lu: context switch from %p to %p\n", i[0], i[1] >> 16, (void *)i[2], (void *)i[3]);
            break;
        case KERNEL_EVLOG_PREEMPT:
            dprintf(INFO,"%lu.%lu: preempt on thread %p\n", i[0], i[1] >> 16, (void *)i[2]);
            break;
        case KERNEL_EVLOG_TIMER_TICK:
            dprintf(INFO,"%lu.%lu: timer tick\n", i[0], i[1] >> 16);
            break;
        case KERNEL_EVLOG_TIMER_CALL:
            dprintf(INFO,"%lu.%lu: timer call %p, arg %p\n", i[0], i[1] >> 16, (void *)i[2], (void *)i[3]);
            break;
        case KERNEL_EVLOG_IRQ_ENTER:
            dprintf(INFO,"%lu.%lu: irq entry %lu\n", i[0], i[1] >> 16, i[2]);
            break;
        case KERNEL_EVLOG_IRQ_EXIT:
            dprintf(INFO,"%lu.%lu: irq exit  %lu\n", i[0], i[1] >> 16, i[2]);
            break;
        default:
            dprintf(INFO,"%lu: unknown id 0x%lx 0x%lx 0x%lx\n", i[0], i[1], i[2], i[3]);
    }
}
