...
usage:
        dprintf(INFO,"usage:\n");
        dprintf(INFO,"%s phys2virt <address>\n", argv[0].str);
        dprintf(INFO,"%s virt2phys <address>\n", argv[0].str);
        dprintf(INFO,"%s map <phys> <virt> <count> <flags>\n", argv[0].str);
        dprintf(INFO,"%s unmap <virt> <count>\n", argv[0].str);
        return ERR_GENERIC;
    }
    ...
    if (!strcmp(argv[1].str, "phys2virt")) {
        ...
        dprintf(INFO,"paddr_to_kvaddr returns %p\n", ptr);
    } else if (!strcmp(argv[1].str, "virt2phys")) {
        if (argc < 3) goto notenoughargs;

        vmm_aspace_t *aspace = vaddr_to_aspace((void *)argv[2].u);
        if (!aspace) {
            dprintf(INFO,"ERROR: outside of any address space\n");
            return -1;
        }
        ...
        dprintf(INFO,"arch_mmu_query returns %d\n", err);
        if (err >= 0) {
            dprintf(INFO,"\tpa 0x%lx, flags 0x%x\n", pa, flags);
        }
        ...
        dprintf(INFO,"arch_mmu_map returns %d\n", err);
        ...
        dprintf(INFO,"arch_mmu_unmap returns %d\n", err);
    } else {
