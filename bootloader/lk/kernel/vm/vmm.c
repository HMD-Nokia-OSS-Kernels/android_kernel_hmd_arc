...
static void dump_region(const vmm_region_t *r) {
    dprintf(INFO,"\tregion %p: name '%s' range 0x%lx - 0x%lx size 0x%zx flags 0x%x mmu_flags 0x%x\n",
           r, r->name, r->base, r->base + r->size - 1, r->size, r->flags, r->arch_mmu_flags);
}

static void dump_aspace(const vmm_aspace_t *a) {
    dprintf(INFO,"aspace %p: name '%s' range 0x%lx - 0x%lx size 0x%zx flags 0x%x\n",
           a, a->name, a->base, a->base + a->size - 1, a->size, a->flags);

    dprintf(INFO,"regions:\n");
    vmm_region_t *r;
...
usage:
        dprintf(INFO,"usage:\n");
        dprintf(INFO,"%s aspaces\n", argv[0].str);
        dprintf(INFO,"%s alloc <size> <align_pow2>\n", argv[0].str);
        dprintf(INFO,"%s alloc_physical <paddr> <size> <align_pow2>\n", argv[0].str);
        dprintf(INFO,"%s alloc_contig <size> <align_pow2>\n", argv[0].str);
        dprintf(INFO,"%s free_region <address>\n", argv[0].str);
        dprintf(INFO,"%s create_aspace\n", argv[0].str);
        dprintf(INFO,"%s create_test_aspace\n", argv[0].str);
        dprintf(INFO,"%s free_aspace <address>\n", argv[0].str);
        dprintf(INFO,"%s set_test_aspace <address>\n", argv[0].str);
        return ERR_GENERIC;
    }
    ...
    if (!strcmp(argv[1].str, "aspaces")) {
        ...
        dprintf(INFO,"vmm_alloc returns %d, ptr %p\n", err, ptr);
        ...
        dprintf(INFO,"vmm_alloc_physical returns %d, ptr %p\n", err, ptr);
        ...
        dprintf(INFO,"vmm_alloc_contig returns %d, ptr %p\n", err, ptr);
        ...
        dprintf(INFO,"vmm_free_region returns %d\n", err);
    } else if (!strcmp(argv[1].str, "create_aspace")) {
        vmm_aspace_t *aspace;
        status_t err = vmm_create_aspace(&aspace, "test", 0);
        dprintf(INFO,"vmm_create_aspace returns %d, aspace %p\n", err, aspace);
        ...
        dprintf(INFO,"vmm_free_aspace returns %d\n", err);
