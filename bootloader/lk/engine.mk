...
OUTIMAGE := $(BUILDDIR)/lk.bin
OUTBIN := $(BUILDDIR)/lk-nodtb.bin
OUTDTB :=  $(BUILDDIR)/lk.dtb
...
EXTERNAL_DEFINES += CONFIG_AUTOBOOT
endif
...
# top level rule
all:: $(OUTIMAGE) $(OUTELF).lst $(OUTELF).debug.lst $(OUTELF).sym $(OUTELF).sym.sorted $(OUTELF).size $(OUTELF).dump $(BUILDDIR)/srcfiles.txt $(BUILDDIR)/include_paths.txt
...
ifneq ($(CONFIG_OF_CONTROL),)
$(OUTIMAGE): $(OUTBIN) $(OUTDTB)
	$(info generating image: $@)
	$(NOECHO)for i in $(filter-out FORCE,$^); do \
		dd if=$$i ibs=4096 obs=4096 conv=sync >> $@; \
	done
	@cp $@ $(BUILDDIR)/lk-fdl2.bin

# Build $(OUTDTB) image: the logic to compile and link stuff is in here
include make/dtb.mk
...
# Build $(OUTBIN) image: the logic to compile and link stuff is in here
...
clean: $(EXTRA_CLEANDEPS)
	rm -f $(ALLOBJS) $(DEPS) $(GENERATED) $(OUTIMAGE) $(OUTBIN) $(OUTELF) $(OUTELF).lst $(OUTELF).debug.lst $(OUTELF).sym $(OUTELF).sym.sorted $(OUTELF).size $(OUTELF).hex $(OUTELF).dump
