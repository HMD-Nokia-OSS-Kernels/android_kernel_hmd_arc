...
usage:
        dprintf(INFO,"usage: %s create <size> <count>\n", argv[0].str);
#if KLOG_RETENTION_TEST
        dprintf(INFO,"usage: %s createret \n", argv[0].str);
        dprintf(INFO,"usage: %s recoverret \n", argv[0].str);
#endif
        dprintf(INFO,"usage: %s getbufcount\n", argv[0].str);
        dprintf(INFO,"usage: %s getbufnum\n", argv[0].str);
        dprintf(INFO,"usage: %s getbufptr\n", argv[0].str);
        dprintf(INFO,"usage: %s setbufnum <num>\n", argv[0].str);
        dprintf(INFO,"usage: %s puts <string>\n", argv[0].str);
        dprintf(INFO,"usage: %s read <length> <buffer num>\n", argv[0].str);
        dprintf(INFO,"usage: %s printftest\n", argv[0].str);
        dprintf(INFO,"usage: %s dump [buffer num]\n", argv[0].str);
        dprintf(INFO,"usage: %s vec [buffer num]\n", argv[0].str);
        return -1;
    }
    ...
    if (!strcmp(argv[1].str, "create")) {
        ...
        if (!ptr) {
            dprintf(INFO,"error allocating memory for klog\n");
            return -1;
        }
        err = klog_create(ptr, size, count);
        dprintf(INFO,"klog_create returns %d\n", err);
        if (err < 0)
            free(ptr);
#if KLOG_RETENTION_TEST
        ...
        dprintf(INFO,"klog_recover returns %d\n", err);
#endif
    } else if (!strcmp(argv[1].str, "getbufcount")) {
        dprintf(INFO,"%d buffers\n", klog_buffer_count());
    } else if (!strcmp(argv[1].str, "getbufnum")) {
        dprintf(INFO,"%d current buffer\n", klog_current_buffer());
        ...
        dprintf(INFO,"klog_set_current_buffer returns %d\n", err);
        ...
        char *buf = malloc(len);
        if (!buf) {
            dprintf(INFO,"error allocating memory for klog read\n");
            return -1;
        }
        size_t count = klog_read(buf, len, buf_id);
        if (count > 0) {
            dprintf(INFO,"read %zu byte(s): \"", count);
            for (size_t i = 0; i < count; i++)
                putchar(buf[i]);
            putchar('\"');
            putchar('\n');
        } else {
            dprintf(INFO,"read returned error: %d\n", count);
        }
        free(buf);
        ...
        int err = klog_get_buffer(buffer, vec);
        dprintf(INFO,"klog_get_buffer returns %d\n", err);
        dprintf(INFO,"vec %d: base %p, len %zu\n", 0, vec[0].iov_base, vec[0].iov_len);
        dprintf(INFO,"vec %d: base %p, len %zu\n", 1, vec[1].iov_base, vec[1].iov_len);
    } else {
        errorf("ERROR unknown command\n");
        goto usage;
    }
