...
static inline void HEAP_DUMP(void) {
    ...
    dprintf(INFO,"\tmallinfo (dlmalloc):\n");
    dprintf(INFO,"\t\tarena space 0x%zx\n", minfo.arena);
    dprintf(INFO,"\t\tfree chunks 0x%zx\n", minfo.ordblks);
    dprintf(INFO,"\t\tspace in mapped regions 0x%zx\n", minfo.hblkhd);
    dprintf(INFO,"\t\tmax total allocated 0x%zx\n", minfo.usmblks);
    dprintf(INFO,"\t\ttotal allocated 0x%zx\n", minfo.uordblks);
    dprintf(INFO,"\t\tfree 0x%zx\n", minfo.fordblks);
    dprintf(INFO,"\t\treleasable space 0x%zx\n", minfo.keepcost);

    dprintf(INFO,"\theap block list:\n");
    void dump_callback(void *start, void *end, size_t used_bytes, void *arg) {
        dprintf(INFO,"\t\tstart %p end %p used_bytes %zu\n", start, end, used_bytes);
    }
...
void *malloc(size_t size) {
    LTRACEF("size %zd\n", size);
    if(size <= 0 || size > (MEMSIZE + MEMBASE - (unsigned long)&_end)) {
        errorf("error: malloc size %zd is invalid!!!!!,return NULL\n", size);
        return NULL;
    }
    ...
    if (heap_trace)
        dprintf(INFO,"caller %p malloc %zu -> %p\n", __GET_CALLER(), size, ptr);
    return ptr;
}
...
void *memalign(size_t boundary, size_t size) {
    ...
    if(size <= 0 || size > (MEMSIZE + MEMBASE - (unsigned long)&_end)) {
        printf("error: memalign size %zd is invalid!!!!!,return NULL\n", size);
        return NULL;
    }
    ...
    if (heap_trace)
        dprintf(INFO,"caller %p memalign %zu, %zu -> %p\n", __GET_CALLER(), boundary, size, ptr);
    return ptr;
}
...
void *calloc(size_t count, size_t size) {
    ...
    if(size <= 0 || size > (MEMSIZE + MEMBASE - (unsigned long)&_end)) {
        printf("error: calloc size %zd is invalid!!!!!,return NULL\n", size);
        return NULL;
    }
    ...
    if (heap_trace)
        dprintf(INFO,"caller %p calloc %zu, %zu -> %p\n", __GET_CALLER(), count, size, ptr);
    return ptr;
}
...
void *realloc(void *ptr, size_t size) {
    ...
    if(size <= 0 || size > (MEMSIZE + MEMBASE - (unsigned long)&_end)) {
        printf("error: realloc size %zd is invalid!!!!!,return NULL\n", size);
        return NULL;
    }
    ...
    if (heap_trace)
        dprintf(INFO,"caller %p realloc %p, %zu -> %p\n", __GET_CALLER(), ptr, size, ptr2);
    return ptr2;
}
...
usage:
        dprintf(INFO,"usage:\n");
        dprintf(INFO,"\t%s info\n", argv[0].str);
        dprintf(INFO,"\t%s trace\n", argv[0].str);
        dprintf(INFO,"\t%s trim\n", argv[0].str);
        dprintf(INFO,"\t%s alloc <size> [alignment]\n", argv[0].str);
        dprintf(INFO,"\t%s realloc <ptr> <size>\n", argv[0].str);
        dprintf(INFO,"\t%s free <address>\n", argv[0].str);
        return -1;
    }
    ...
    if (strcmp(argv[1].str, "info") == 0) {
        ...
        dprintf(INFO,"heap trace is now %s\n", heap_trace ? "on" : "off");
