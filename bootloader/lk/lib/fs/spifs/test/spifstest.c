...
bool test_setup(const char *dev_name, uint32_t toc_pages) {
    ...
    if (res != NO_ERROR) {
        dprintf(INFO,"spifs_format failed dev = %s, toc_pages = %u, retcode = %d\n",
    ...
    if (res != NO_ERROR) {
        dprintf(INFO,"fs_mount failed path = %s, fs name = %s, dev name = %s,"
...
static int spifs_test(int argc, const cmd_args *argv) {
    if (argc != 3) {
        dprintf(INFO,"Expected 3 arguments, got %d.\n", argc);
        return -1;
    }
    ...
    if (!dev) {
        dprintf(INFO,"error: could not open block device %s\n", argv[2].str);
        return -1;
    }
    bio_close(dev);
    ...
    for (size_t i = 0; i < countof(tests); i++) {
        ...
        if (!test_setup(argv[2].str, tests[i].toc_pages)) {
            dprintf(INFO,"Test Setup failed before %s. Exiting.\n", tests[i].name);
            break;
        }

        if (tests[i].func(argv[2].str)) {
            dprintf(INFO," [Passed] %s\n", tests[i].name);
            ++passed;
        } else {
            dprintf(INFO," [Failed] %s\n", tests[i].name);
        }

        if (!test_teardown()) {
            dprintf(INFO,"Test teardown failed after %s. Exiting.\n", tests[i].name);
            break;
        }
    }
...
static int spifs_bench(int argc, const cmd_args *argv) {
    ...
    for (size_t file_size = min_bench_bytes;
        ...
        dprintf(INFO," == Benchmark for %zu bytes == \n", file_size);

        // Create file benchmark
        start = current_time_hires();
        st = fs_create_file(test_file_path, &handle, file_size);
        end = current_time_hires();
        dprintf(INFO,"\tfs_create_file = %llu usecs\n", end - start);

        if (st != NO_ERROR) {
            dprintf(INFO,"SPIFS Benchmark Failed to create a %zu byte file at %s. "
        ...
        dprintf(INFO,"\tfs_write_file = %llu usecs\n", end - start);

        if (n_bytes < 0) {
            dprintf(INFO,"SPIFS Benchmark Failed to write to file at %s. "
        ...
        dprintf(INFO,"\tfs_read_file = %llu usecs\n", end - start);

        if (n_bytes < 0) {
            dprintf(INFO,"SPIFS Benchmark Failed to read from file at %s. "
                   "Reason = %ld.\n", test_file_path, n_bytes);
            retcode = -1;
            fs_close_file(handle);
            goto finish;
        ...
        if (st != NO_ERROR) {
            dprintf(INFO,"SPIFS Benchmark Failed to close file at %s. "
                   "Reason = %d.\n", test_file_path, st);
            retcode = -1;
            goto finish;
        }
        ...
        dprintf(INFO,"\tfs_remove_file = %llu usecs\n", end - start);

        if (st != NO_ERROR) {
            dprintf(INFO,"SPIFS Benchmark Failed to remove file at %s. "
                   "Reason = %d.\n", test_file_path, st);
            retcode = -1;
            goto finish;
        }
