...
static int cmd_fs_ioctl(int argc, const cmd_args *argv) {
    ...
    switch (request) {
        ...
        case FS_IOCTL_IS_LINEAR: {
            ...
            err = fs_open_file(argv[3].str, &handle);
            if (err != NO_ERROR) {
                dprintf(INFO,"error %d opening file\n", err);
                return err;
            }
            ...
            if (is_mapped) {
                dprintf(INFO,"file at %s is memory mapped\n", argv[3].str);
            } else {
                dprintf(INFO,"file at %s is not memory mapped\n", argv[3].str);
            }
        ...
        default: {
            dprintf(INFO,"error, unsupported ioctl: %d\n", request);
        }
    }
...
usage:
        dprintf(INFO,"%s mount <path> <type> [device]\n", argv[0].str);
        dprintf(INFO,"%s unmount <path>\n", argv[0].str);
        dprintf(INFO,"%s write <path> <string> [<offset>]\n", argv[0].str);
        dprintf(INFO,"%s format <type> [device]\n", argv[0].str);
        dprintf(INFO,"%s stat <path>\n", argv[0].str);
        dprintf(INFO,"%s ioctl <request> [args...]\n", argv[0].str);
        return -1;
    }
    ...
    if (!strcmp(argv[1].str, "mount")) {
        ...
        if (err < 0) {
            errorf("error %d mounting device\n", err);
            return err;
        }
        ...
        if (err < 0) {
            errorf("error %d unmounting device\n", err);
            return err;
        }
        ...
        if (err != NO_ERROR) {
            errorf("error %d formatting device\n", err);
            return err;
        }
        ...
        if (err != NO_ERROR) {
            errorf("error %d statting filesystem\n", err);
            return err;
        }

        dprintf(INFO,"\ttotal bytes: %llu\n", stat.total_space);
        dprintf(INFO,"\tfree bytes: %llu\n", stat.free_space);
        dprintf(INFO,"\n");
        dprintf(INFO,"\ttotal inodes: %d\n", stat.total_inodes);
        dprintf(INFO,"\tfree inodes: %d\n", stat.free_inodes);
