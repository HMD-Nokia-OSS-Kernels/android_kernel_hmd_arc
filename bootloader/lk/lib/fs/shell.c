...
static int cmd_ls(int argc, const cmd_args *argv) {
    ...
    while ((err = fs_read_dir(dhandle, &ent)) >= 0) {
        ...
        if (err < 0) {
            errorf("error %d statting file\n", err);
            continue;
        }

        dprintf(INFO,"%c %16llu %s\n", stat.is_dir ? 'd' : ' ', stat.size, ent.name);
    }
...
static int cmd_mkdir(int argc, const cmd_args *argv) {
    ...
    if (status < 0) {
        errorf("error %d making directory '%s'\n", status, path);
    }
...
static int cmd_rm(int argc, const cmd_args *argv) {
    ...
    if (err < 0) {
        errorf("error %d removing file '%s'\n", err, path);
        return err;
    }
...
static int cmd_stat(int argc, const cmd_args *argv) {
    ...
    dprintf(INFO,"stat successful:\n");
    dprintf(INFO,"\tis_dir: %d\n", stat.is_dir ? 1 : 0);
    dprintf(INFO,"\tsize: %lld\n", stat.size);
