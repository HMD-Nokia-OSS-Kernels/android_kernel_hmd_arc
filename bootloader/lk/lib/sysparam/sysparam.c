...
void sysparam_dump(bool show_all) {
    dprintf(INFO,"system parameters:\n");

    size_t total_memlen = 0;

    struct sysparam *param;
    list_for_every_entry(&params.list, param, struct sysparam, node) {
        dprintf(INFO,"________%c %-16s : ",
    ...
    dprintf(INFO,"total in-memory usage: %zu bytes\n", total_memlen);
}
...
usage:
        dprintf(INFO,"usage: %s dump\n", argv[0].str);
        dprintf(INFO,"usage: %s list\n", argv[0].str);
        dprintf(INFO,"usage: %s reload\n", argv[0].str);
#if SYSPARAM_ALLOW_WRITE
        dprintf(INFO,"usage: %s add <param> <string value>\n", argv[0].str);
        dprintf(INFO,"usage: %s addhex <param> <hex value>\n", argv[0].str);
        dprintf(INFO,"usage: %s addlong <param>\n", argv[0].str);
        dprintf(INFO,"usage: %s remove <param>\n", argv[0].str);
        dprintf(INFO,"usage: %s lock <param>\n", argv[0].str);
        dprintf(INFO,"usage: %s write\n", argv[0].str);
#endif
        dprintf(INFO,"usage: %s length <param>\n", argv[0].str);
        dprintf(INFO,"usage: %s read <param>\n", argv[0].str);
        return -1;
    }
    ...
    if (!strcmp(argv[1].str, "dump")) {
        ...
        for (int i = 0; i < len; i++)
            dprintf(INFO,"%02x", buf[i]);
        dprintf(INFO,"\n");
        free(buf);
    } else {
        dprintf(INFO,"ERROR unknown command\n");
        goto usage;
    }
