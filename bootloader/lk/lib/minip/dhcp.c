...
static void dhcp_cb(void *data, size_t sz, uint32_t srcip, uint16_t srcport, void *arg) {
...
#if TRACE_DHCP
    dprintf(INFO,"dhcp op=%d len=%d from p=%d ip=", msg->opcode, sz, srcport);
    printip("", srcip);
#endif

    if (configured) {
        dprintf(INFO,"already configured\n");
        return;
    }
#if TRACE_DHCP
    ...
    dprintf(INFO,"chaddr %02x:%02x:%02x:%02x:%02x:%02x\n",
...
void minip_init_dhcp(tx_func_t tx_func, void *tx_arg) {
    ...
    dprintf(INFO,"dhcp opened udp: %d\n", ret);
