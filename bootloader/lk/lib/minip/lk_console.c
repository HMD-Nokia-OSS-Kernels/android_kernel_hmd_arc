...
void arp_usage(void) {
    dprintf(INFO,"arp list                        print arp table\n");
    dprintf(INFO,"arp query <ipv4 address>        query arp address\n");
}
...
static int cmd_minip(int argc, const cmd_args *argv) {
    if (argc == 1) {
minip_usage:
        dprintf(INFO,"minip commands\n");
        dprintf(INFO,"mi [a]rp                        dump arp table\n");
        dprintf(INFO,"mi [s]tatus                     print ip status\n");
        dprintf(INFO,"mi [t]est [dest] [port] [cnt]   send <cnt> test packets to the dest:port\n");
    } else {
        switch (argv[1].str[0]) {
            ...
            case 's': {
                ...
                dprintf(INFO,"hostname: %s\n", minip_get_hostname());
                dprintf(INFO,"ip: %u.%u.%u.%u\n", IPV4_SPLIT(ipaddr));
            }
            break;
            case 't': {
                uint32_t count = 1;
                ...
                if (udp_open(host, port, port, &handle) != NO_ERROR) {
                    dprintf(INFO,"udp_open to %u.%u.%u.%u:%u failed\n", IPV4_SPLIT(host), port);
                    return -1;
                }
...
#define BUFSIZE 1470
                ...
                dprintf(INFO,"sending %u packet(s) to %u.%u.%u.%u:%u\n", count, IPV4_SPLIT(host), port);
                ...
                t = current_time() - t;
                errorf("%d pkts failed\n", failures);
                uint64_t total_count = (uint64_t)count * BUFSIZE;
                dprintf(INFO,"wrote %llu bytes in %u msecs (%llu bytes/sec)\n",
