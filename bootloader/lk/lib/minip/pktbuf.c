...
void pktbuf_dump(pktbuf_t *p) {
    dprintf(INFO,"pktbuf data %p, buffer %p, dlen %u, data offset %lu, phys_base %p\n",
...
static void pktbuf_init(uint level) {
...
#if LK_DEBUGLEVEL > 0
    dprintf(INFO,"pktbuf: creating %u pktbuf entries of size %zu (total %zu)\n",
    ...
    if (vmm_alloc_contiguous(vmm_get_kernel_aspace(), "pktbuf",
        ...
        dprintf(INFO,"Failed to initialize pktbuf hdr slab\n");
        return;
    }
#else
