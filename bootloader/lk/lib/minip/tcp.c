...
__NO_INLINE static void dump_tcp_header(const tcp_header_t *header) {
    dprintf(INFO,"TCP: src_port %u, dest_port %u, seq %u, ack %u, win %u, flags %c%c%c%c%c%c\n",
...
static void dump_socket(tcp_socket_t *s) {
    dprintf(INFO,"socket %p: state %d (%s), local 0x%x:%hu, remote 0x%x:%hu, ref %d\n",
    ...
    if (s->state == STATE_ESTABLISHED || s->state == STATE_CLOSE_WAIT) {
        dprintf(INFO,"\trx: wsize %u wlo %u whi %u (%u)\n",
               s->rx_win_size, s->rx_win_low, s->rx_win_high,
               s->rx_win_high - s->rx_win_low);
        dprintf(INFO,"\ttx: wlo %u whi %u (%u) highest_seq %u (%u) bufsize %u bufoff %u\n",
...
usage:
        dprintf(INFO,"usage: %s sockets\n", argv[0].str);
        dprintf(INFO,"usage: %s listenclose <port>\n", argv[0].str);
        dprintf(INFO,"usage: %s listen <port>\n", argv[0].str);
        dprintf(INFO,"usage: %s debug\n", argv[0].str);
        return ERR_INVALID_ARGS;
    }
    ...
    if (!strcmp(argv[1].str, "sockets")) {
        ...
        dprintf(INFO,"tcp_open_listen returns %d, handle %p\n", err, handle);

        tcp_socket_t *accepted;
        err = tcp_accept(handle, &accepted);
        dprintf(INFO,"tcp_accept returns returns %d, handle %p\n", err, accepted);

        err = tcp_close(accepted);
        dprintf(INFO,"tcp_close returns %d\n", err);
        ...
        for (;;) {
            ...
            dprintf(INFO,"tcp_read returns %ld\n", err);
            if (err < 0)
                break;
            if (err > 0) {
                hexdump8(buf, err);
            ...
            dprintf(INFO,"tcp_write returns %ld\n", err);
            if (err < 0)
                break;
        }
