...
static void dump_ipv4_addr(uint32_t addr) {
    ...
    dprintf(INFO,"%hhu.%hhu.%hhu.%hhu", a[0], a[1], a[2], a[3]);
}
...
static void dump_ipv4_packet(const struct ipv4_hdr *ip) {
    ...
    dprintf(INFO," hlen 0x%x, prot 0x%x, cksum 0x%x, len 0x%x, ident 0x%x, frag offset 0x%x\n",
