...
void ptable_dump(void) {
    ...
    list_for_every_entry(&ptable.list, mentry, struct ptable_mem_entry, node) {
        ...
        dprintf(INFO,"%d: %16s off 0x%016llx len 0x%016llx flags 0x%08x\n",
...
usage:
        dprintf(INFO,"usage: %s scan <bio_device> <offset>\n", argv[0].str);
        dprintf(INFO,"usage: %s default <bio_device> <offset>\n", argv[0].str);
        dprintf(INFO,"usage: %s list\n", argv[0].str);
        dprintf(INFO,"usage: %s add <name> <length> <flags>\n", argv[0].str);
        dprintf(INFO,"usage: %s remove <name>\n", argv[0].str);
        dprintf(INFO,"usage: %s alloc <len>\n", argv[0].str);
        dprintf(INFO,"usage: %s allocend <len>\n", argv[0].str);
        dprintf(INFO,"usage: %s write\n", argv[0].str);
        return -1;
    }
    ...
    if (!strcmp(argv[1].str, "scan")) {
        ...
        if (ptable_dev) {
            ...
            if (err < 0) {
                dprintf(INFO,"ptable nuke failed (err %d)\n", err);
            } else {
                dprintf(INFO,"ptable nuke OK\n");
            }
            bio_close(ptable_dev);
        } else {
            dprintf(INFO,"Failed to find ptable device\n");
        }
    } else if (!strcmp(argv[1].str, "add")) {
        if (argc < 5) goto notenoughargs;
        err = ptable_add(argv[2].str, argv[3].u, argv[4].u);
        if (err < NO_ERROR)
            dprintf(INFO,"ptable_add returns err %d\n", err);
        ...
        if (off < 0) {
            dprintf(INFO,"%s of 0x%lx failed (err %lld)\n",
                   argv[1].str, argv[2].u, off);
        } else {
            dprintf(INFO,"%s of 0x%lx gives [0x%llx, 0x%llx)\n",
                   argv[1].str, argv[2].u, off, off + len);
        }
    } else if (!strcmp(argv[1].str, "write")) {
        dprintf(INFO,"ptable_write result %d\n", ptable_write());
    } else {
        goto usage;
    }
