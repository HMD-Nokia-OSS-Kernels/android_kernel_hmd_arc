...
static int cmd_display_mem(int argc, const cmd_args *argv) {
    ...
    if ((address & (size - 1)) != 0) {
        errorf("unaligned address, cannot display\n");
        return -1;
    }
    ...
    if (vaddr_to_paddr((void *)address) == 0) {
        errorf("ERROR: address 0x%lx is unmapped\n", address);
        return -1;
    }
#endif
        ...
        switch (size) {
            ...
            case 2: {
                uint16_t val = (byte_order != BYTE_ORDER) ?
                               ...
                               *(uint16_t *)address;
                dprintf(INFO,"%04hx ", val);
                break;
            }
            case 1:
                dprintf(INFO,"%02hhx ", *(uint8_t *)address);
                break;
        }
        count += size;
        if (count == 16) {
...
static int cmd_modify_mem(int argc, const cmd_args *argv) {
    ...
    if ((address & (size - 1)) != 0) {
        errorf("unaligned address, cannot modify\n");
        return -1;
    }
...
static int cmd_copy_mem(int argc, const cmd_args *argv) {
    if (argc < 4) {
        errorf("not enough arguments\n");
        errorf("%s <source address> <target address> <len>\n", argv[0].str);
        return -1;
    }
...
static int cmd_memtest(int argc, const cmd_args *argv) {
    ...
    size_t i;
    // write out
    dprintf(INFO,"writing first pass...");
    for (i = 0; i < len / 4; i++) {
        ptr[i] = i;
    }
    dprintf(INFO,"done\n");
    ...
    // verify
    dprintf(INFO,"verifying...");
    for (i = 0; i < len / 4; i++) {
        if (ptr[i] != i)
            dprintf(INFO,"error at %p\n", &ptr[i]);
    }
    dprintf(INFO,"done\n");
