...
usage:
        dprintf(INFO,"%s display_info : output information bout the current display\n", argv[0].str);
        dprintf(INFO,"%s rgb_bars   : Fill frame buffer with rgb bars\n", argv[0].str);
        dprintf(INFO,"%s test_pattern : Fill frame with test pattern\n", argv[0].str);
        dprintf(INFO,"%s fill r g b   : Fill frame buffer with RGB888 value and force update\n", argv[0].str);
        dprintf(INFO,"%s mandelbrot   : Fill frame buffer with Mandelbrot fractal\n", argv[0].str);
    ...
    if (!strcmp(argv[1].str, "display_info")) {
        dprintf(INFO,"display:\n");
        dprintf(INFO,"\tframebuffer %p\n", fb.image.pixels);
        dprintf(INFO,"\twidth %u height %u stride %u\n", fb.image.width, fb.image.height, fb.image.stride);
        dprintf(INFO,"\tformat %u\n", fb.image.format);
