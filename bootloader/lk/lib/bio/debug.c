...
static int cmd_bio(int argc, const cmd_args *argv) {
    ...
    if (argc < 2) {
notenoughargs:
        dprintf(INFO,"not enough arguments:\n");
usage:
        dprintf(INFO,"%s list\n", argv[0].str);
        dprintf(INFO,"%s read <device> <address> <offset> <len>\n", argv[0].str);
        dprintf(INFO,"%s write <device> <address> <offset> <len>\n", argv[0].str);
        dprintf(INFO,"%s dump <device> <offset> <len>\n", argv[0].str);
        dprintf(INFO,"%s erase <device> <offset> <len>\n", argv[0].str);
        dprintf(INFO,"%s ioctl <device> <request> <arg>\n", argv[0].str);
        dprintf(INFO,"%s remove <device>\n", argv[0].str);
        dprintf(INFO,"%s test <device>\n", argv[0].str);
#if WITH_LIB_PARTITION
        dprintf(INFO,"%s partscan <device> [offset]\n", argv[0].str);
#endif
#if WITH_LIB_CKSUM
        dprintf(INFO,"%s crc32 <device> <offset> <len> [repeat]\n", argv[0].str);
#endif
        return -1;
    }
    ...
    if (!strcmp(argv[1].str, "list")) {
        ...
        if (!dev) {
            errorf("error opening block device\n");
            return -1;
        }
        ...
        do {
            ...
            while (pos < offset + len) {
                ...
                if (err <= 0) {
                    dprintf(INFO,"error reading at offset 0x%lx\n", offset + pos);
                    break;
                }
...
static bool sub_erase_test(bdev_t *device, uint32_t n_samples) {
    dprintf(INFO,"Sampling the device %d times.\n", n_samples);
...
static status_t memory_mapped_test(bdev_t *device) {
    ...
    uint8_t *test_buffer = memalign(DMA_ALIGNMENT, device->block_size);
    if (!test_buffer) {
        dprintf(INFO,"Could not allocate %zu bytes for a temporary buffer. "
               "Aborting.\n", device->block_size);
    ...
    if (!reference_buffer) {
        dprintf(INFO,"Could not allocate %zu bytes for a temporary reference "
               "buffer. Aborting.\n", device->block_size);
        free(test_buffer);
        return ERR_NO_MEMORY;
    }
    ...
    if (err < (ssize_t)device->block_size) {
        dprintf(INFO,"Expected to erase at least %zu bytes but only erased %ld. "
    ...
    if (err != (ssize_t)device->block_size) {
        dprintf(INFO,"Error while writing test pattern to device. Expected to write "
    ...
    if (ioctl_result == ERR_NOT_SUPPORTED) {
        dprintf(INFO,"Device does not support linear mode. Aborting.\n");
        retcode = ERR_NOT_SUPPORTED;
        goto finish;
    } else if (ioctl_result != NO_ERROR) {
        dprintf(INFO,"BIO_IOCTL_GET_MEM_MAP returned error %d. Aborting.\n",
               ioctl_result);
    ...
    for (uint i = 0; i < device->block_size; i++) {
        if (*testptr != *devaddr) {
            dprintf(INFO,"Data mismatch at position %d. Expected %d got %d. "
                   "Aborting.\n", i, *testptr, *devaddr);
            goto finish;
        }
        testptr++;
    ...
    if (err != (ssize_t)device->block_size) {
        dprintf(INFO,"Expected to read %zu bytes, actually read %ld. Aborting.\n",
               device->block_size, err);
        retcode = ERR_IO;
        goto finish;
    }
...
static int bio_test_device(bdev_t *device) {
    ...
    if (num_errors < 0) {
        errorf("error %ld performing erase test\n", num_errors);
        return -1;
    }
    dprintf(INFO,"discovered %ld error(s) while testing erase.\n", num_errors);
    if (num_errors) {
        // No point in continuing the tests if we couldn't erase the device.
        errorf("not continuing to test writes.\n");
        return -1;
    }

    num_errors = write_test(device);
    dprintf(INFO,"Discovered %ld error(s) while testing write.\n", num_errors);
    if (num_errors) {
        return -1;
    }
    ...
    if (!success) {
        errorf("Discovered errors while testing sub-erase.\n");
        return -1;
    } else {
        dprintf(INFO,"No errors while testing sub-erase.\n");
    }

    dprintf(INFO,"Testing memory mapped mode...\n");
    status_t test_result = memory_mapped_test(device);
    if (test_result != NO_ERROR) {
        errorf("Memory mapped test returned error %d\n", test_result);
    } else {
        dprintf(INFO,"Memory mapped mode tests returned successfully\n");
    }
