...
static int cmd_repeat(int argc, const cmd_args *argv) {
    ...
    for (int i = 0; i < times; ++i) {
        ...
        if (result != 0) {
            dprintf(INFO,"terminating repeat loop, command exited with status %d\n",
                   result);
            return result;
        }
        thread_sleep(delay);
...
usage:
    dprintf(INFO,"Usage: repeat <times> <delay in ms> <cmd> [args..]\n");
...
static status_t command_loop(int (*get_line)(const char **, void *), void *get_line_cookie, bool showprompt, bool locked) {
    ...
    char *outbuf = NULL;
    char console_project[MAX_NUM_ARGS];
    ...
    memset(console_project, 0, MAX_NUM_ARGS);
    strcpy(console_project, (char const *)CONFIG_SYS_PROMPT);
    while (!exit) {
        // read a new line if it hadn't been split previously and passed back from tokenize_command
        if (continuebuffer == NULL) {
            if (showprompt)
                fputs(console_project, stdout);
...
//      debugf("line = '%s'\n", buffer);
        ...
        if (argc < 0) {
            if (showprompt)
                dprintf(INFO,"syntax error\n");
            continue;
        } else if (argc == 0) {
            continue;
        }

//      debugf("after tokenize: argc %d\n", argc);
...
//          debugf("%d: '%s'\n", i, args[i].str);
        ...
        if (!command) {
            if (showprompt)
                dprintf(INFO,"command not found\n");
            continue;
        }
...
extern int sprd_serial_tstc(void);
#define endtick(seconds) (get_ticks() + (uint64_t)(seconds) * CONFIG_SPRD_TIMER_CLK)

static int shell_check_keyed(int bootdelay)
{
	uint64_t endtime = endtick(bootdelay);
	int c = 0;
...
{
    int bootdelay = CONFIG_LK_SHELL_DELAY;
    int ret = 0;

    dprintf(INFO, "Press q to LK shell cmd in %d seconds!\n", bootdelay);
    ret = shell_check_keyed(bootdelay);
    if (ret == 0)
		console_start();
}

void manual_cmd_shell(void)
{
    shell_loop();
}
#endif
...
static int cmd_test(int argc, const cmd_args *argv) {
    ...
    dprintf(INFO,"argc %d, argv %p\n", argc, argv);
    for (i = 0; i < argc; i++)
        dprintf(INFO,"\t%d: str '%s', i %ld, u %#lx, b %d\n", i, argv[i].str, argv[i].i, argv[i].u, argv[i].b);
