...
{
	/* config the EXT_DCDC_CPU1 voltage initial value */
	__raw_writel(BIT_TOP_DVFS_APB_DCDC_CPU1_VOLTAGE0_I2C(0x8f) |
		     BIT_TOP_DVFS_APB_DCDC_CPU1_VOLTAGE1_I2C(0x94) |
		     BIT_TOP_DVFS_APB_DCDC_CPU1_VOLTAGE2_I2C(0x99) |
		     BIT_TOP_DVFS_APB_DCDC_CPU1_VOLTAGE3_I2C(0x9e),
		     REG_TOP_DVFS_APB_PMIC_3RD_DCDC_CPU1_DVFS_VOLTAGE_VALUE0);

	__raw_writel(BIT_TOP_DVFS_APB_DCDC_CPU1_VOLTAGE4_I2C(0xa3) |
		     BIT_TOP_DVFS_APB_DCDC_CPU1_VOLTAGE5_I2C(0xa8) |
		     BIT_TOP_DVFS_APB_DCDC_CPU1_VOLTAGE6_I2C(0xa8) |
		     BIT_TOP_DVFS_APB_DCDC_CPU1_VOLTAGE7_I2C(0xa8),
		     REG_TOP_DVFS_APB_PMIC_3RD_DCDC_CPU1_DVFS_VOLTAGE_VALUE1);
...
void clock_auto_gate(void)
{
	...
	if (get_bootdevice() == BOOT_DEVICE_UFS) {
		...
		CHIP_REG_OR(REG_AON_APB_MIPI_CSI_POWER_CTRL,
			    BIT_AON_APB_ANALOG_BB_TOP_SINDRV_ENA_AUTO_EN);
	}
#endif
}

static void dcdc_ds_config(void)
{
	uint32_t drop_val;
...
static void pmic_lowpower_config_later(void)
{
	uint32_t avdd18_slp_pd_en = 0;
	uint32_t avdd12_slp_pd_en = 0;

	avdd18_slp_pd_en = !!(ANA_REG_GET(ANA_REG_GLB_SLP_LDO_PD_CTRL1) & BIT_SLP_LDO_AVDD18_PD_EN);
	avdd12_slp_pd_en = !!(ANA_REG_GET(ANA_REG_GLB_SLP_LDO_PD_CTRL1) & BIT_SLP_LDO_AVDD12_PD_EN);

	if (get_bootdevice() == BOOT_DEVICE_UFS) {
		if (!avdd18_slp_pd_en && !avdd12_slp_pd_en)
