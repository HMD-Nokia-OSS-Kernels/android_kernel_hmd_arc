...
void hardfault(struct arm_cm_exception_frame *frame) {
    dprintf(INFO,"hardfault: ");
    dump_frame(frame);

#if     (__CORTEX_M >= 0X03) || (__CORTEX_SC >= 300)
    dprintf(INFO,"HFSR 0x%x\n", SCB->HFSR);
#endif
...
void memmanage(struct arm_cm_exception_frame *frame) {
    ...
    if (mmfsr & (1<<0)) { // IACCVIOL
        dprintf(INFO,"instruction fault\n");
    }
    if (mmfsr & (1<<1)) { // DACCVIOL
        dprintf(INFO,"data fault\n");
    }
    if (mmfsr & (1<<3)) { // MUNSTKERR
        dprintf(INFO,"fault on exception return\n");
    }
    if (mmfsr & (1<<4)) { // MSTKERR
        dprintf(INFO,"fault on exception entry\n");
    }
    if (mmfsr & (1<<5)) { // MLSPERR
        dprintf(INFO,"fault on lazy fpu preserve\n");
    }
    if (mmfsr & (1<<7)) { // MMARVALID
        dprintf(INFO,"fault address 0x%x\n", SCB->MMFAR);
    }
#endif
    platform_halt(HALT_ACTION_HALT, HALT_REASON_SW_PANIC);
}
...
void usagefault(struct arm_cm_exception_frame *frame) {
    dprintf(INFO,"usagefault: ");
    dump_frame(frame);

#if  (__CORTEX_M >= 0x03)
    uint32_t ufsr = BITS_SHIFT(SCB->CFSR, 31, 16);
    dprintf(INFO,"UFSR 0x%x: ", ufsr);

    if (ufsr & (1<<0))
        dprintf(INFO,"undefined instruction\n");
    if (ufsr & (1<<1))
        dprintf(INFO,"ESPR invalid\n");
    if (ufsr & (1<<2))
        dprintf(INFO,"integrity check failed on EXC_RETURN\n");
    if (ufsr & (1<<3))
        dprintf(INFO,"coprocessor access error\n");
    if (ufsr & (1<<8))
        dprintf(INFO,"unaligned error\n");
    if (ufsr & (1<<9))
        dprintf(INFO,"division by zero\n");
#endif
...
void __WEAK _systick(void) {
    dprintf(INFO,"systick\n");
    platform_halt(HALT_ACTION_HALT, HALT_REASON_SW_PANIC);
}

void __WEAK _debugmonitor(void) {
    dprintf(INFO,"debugmonitor\n");
    platform_halt(HALT_ACTION_HALT, HALT_REASON_SW_PANIC);
}
