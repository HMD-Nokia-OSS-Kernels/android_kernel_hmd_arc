...
void arch_mmu_init(void)
{
    ...
    sctlr &= ~(1<<1);  /* disable alignment abort if no sml.*/
...
#ifdef SPRD_ASYNC_SUPPORT
    /*enable HCR.VA and HCR.AMO*/
    if (is_hyp_mode()) {
        uint32_t hcr = arm_read_hcr();
        hcr |= (1<<8)|(1<<5); //enable HCR.VA | HCR.AMO
        arm_write_hcr(hcr);
    }
    arch_enable_async(); //enable serror vector.
#endif
}
