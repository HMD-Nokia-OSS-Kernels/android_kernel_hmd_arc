...
extern int sprd_rng_gen(uint8_t *out, uint32_t out_len);
                             ...
                             unsigned char *out, unsigned long *outlen)
{
    ...
    seed = sprd_pal_malloc(CRYPTO_RNG_MAX_SIZE);
    ...
    sprd_pal_memset(seed, 0, CRYPTO_RNG_MAX_SIZE);
    ...
    /* now choose a random seed (prand in LOAD 0x104, ce_rng in DOWNLOAD 0x102)*/
    switch (boot_role) {
        case BOOTLOADER_MODE_LOAD:
            err = prand_gen(seed, hLen);
            break;
        case BOOTLOADER_MODE_DOWNLOAD:
            err = sprd_rng_gen(seed, hLen);
            break;
        default:
