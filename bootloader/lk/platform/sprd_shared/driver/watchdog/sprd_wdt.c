...
static u32 hw_wdt_int_raw_flag = 0x355e;
/*****************************************************************************/
...
void start_watchdog(uint64_t init_time_ms)
{
	...
	config.val      = init_time_ms <= WDG_REBOOT_MS ? 0x50 : ((init_time_ms
				/ 1000) * WDG_CLK + ((init_time_ms % 1000) * WDG_CLK) / 1000);
...
void stop_watchdog(void)
{
	...
	dprintf(INFO,"Watch Dog Trace: WDG_TimerStop\n");
	  /*clk on*/
	ANA_REG_OR(ANA_REG_GLB_ARM_MODULE_EN, BIT_ANA_WDG_EN); //WDG enable
	ANA_REG_OR(ANA_REG_GLB_RTC_CLK_EN,    BIT_RTC_WDG_EN); //WDG Rtc enable
