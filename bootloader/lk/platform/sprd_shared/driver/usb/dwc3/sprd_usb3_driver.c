...
extern uint32_t	dwc3_get_evt_lpos(void);
 ...
 */
static inline int _handshake(usb3_device_t *dev,
...
static inline void usb3_flush_cache(phys_addr_t addr,
...
void usb3_start_receive_pkt(unsigned char* buf, unsigned int len, unsigned int recv_zlp)
{
	usb3_pcd_t *pcd = &g_usb3_dev.pcd;
	usb3_pcd_ep_t *ep = pcd->out_ep;
	uint8_t tri;
	...
	/* Check whether short Packet unalign*/
	short_p = len % ep->ep.maxpacket;

	/* Check Whether Zero-Length Packet is required */
	zlp = (len && recv_zlp && (0 == short_p)) ? 1 : 0;

	/* Fill in Bulk In TRB */
	usb3_fill_desc(ep->ep.dma_desc, buf,
		len, DSCCTL_TRBCTL_NORMAL,
		(zlp || short_p) ? 0 : DSCCTL_LST_BIT, 1);
	...
	if (short_p)
		usb3_fill_desc(ep->ep.dma_desc + 1, usb_out_endpoint_cmd,
			ep->ep.maxpacket - short_p, DSCCTL_TRBCTL_NORMAL,
			DSCCTL_LST_BIT, 1);
...
{
	unsigned int recv_zlp = (0 == usb_get_dload_sys());
	usb3_start_receive_pkt(buf, len, recv_zlp);
	return 0;
}
...
void usb_core_init(unsigned long reg_base_addr, uint32_t autodl_mode)
{
	...
	/* Prepare to get next OUT transfer */
	if (autodl_mode){
		dprintf(INFO," autodload get next tr\r\n");
		usb_receive(usb_out_endpoint_cmd, (unsigned int)MAX_RECV_LENGTH);
	}
}
