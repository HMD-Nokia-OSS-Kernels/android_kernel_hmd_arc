...
static void usb_enable(void)
{
	...
	unsigned int tmp = readl(REG_ANLG_PHY_G0L_ANALOG_USB20_USB20_TRIMMING);
	...
	*/
	...
	/*config eye pattern*/
	tmp &= ~BIT_ANLG_PHY_G0L_ANALOG_USB20_USB20_TUNEHSAMP(0x3);
	tmp |= BIT_ANLG_PHY_G0L_ANALOG_USB20_USB20_TUNEHSAMP(USB20_TUNEHSAMP);
	tmp &= ~BIT_ANLG_PHY_G0L_ANALOG_USB20_USB20_TUNEEQ(0x7);
	tmp |= BIT_ANLG_PHY_G0L_ANALOG_USB20_USB20_TUNEEQ(USB20_TUNEEQ);
	//tmp &= ~BIT_ANLG_PHY_G0L_ANALOG_USB20_USB20_TFREGRES(0x1f);
	//tmp |= BIT_ANLG_PHY_G0L_ANALOG_USB20_USB20_TFREGRES(USB20_TFREGRES);
	writel(tmp, REG_ANLG_PHY_G0L_ANALOG_USB20_USB20_TRIMMING);
	dprintf(INFO,"eye pattern = 0x%x\n", __raw_readl(REG_ANLG_PHY_G0L_ANALOG_USB20_USB20_TRIMMING));
...
{
	u32 val;
	/* bit0:0 dpu dpi clock
	 * bit1:0 dptx
	 * bit3:0 combphy tca
	 * bit4:0 usb31 pll
	 */
	val = readl(DISPC1_GLB_APB_EB);
	debugf("usb_phy_enabled 0x%x \n", val);
	if ((val & BIT(0)) && (val & BIT(1)) && (val & BIT(3))
		&& (val & BIT(4)))
		return 1;
