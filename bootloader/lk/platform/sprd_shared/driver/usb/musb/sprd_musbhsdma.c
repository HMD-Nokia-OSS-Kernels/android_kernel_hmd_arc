...
static void sprd_dma_channel_release(struct sprd_dma_channel *channel);
...
static struct sprd_dma_channel *sprd_dma_channel_allocate(struct dma_controller *c,
				struct musb_hw_ep *hw_ep, u8 transmit)
{
	...
	struct sprd_dma_channel *channel = NULL;
	u8 bit;
	u16 csr;
	struct musb *musb = controller->private_data;
...
static void sprd_dma_channel_release(struct sprd_dma_channel *sprd_channel)
{
	struct sprd_musb_dma_channel *musb_channel = sprd_channel->private_data;

	sprd_channel->actual_len = 0;

	musb_channel->controller->used_channels &= ~(1 << musb_channel->idx);

	sprd_channel->status = MUSB_DMA_STATUS_UNKNOWN;
}

static void sprd_configure_channel(struct sprd_dma_channel *sprd_channel,
...
static void prepare_linklist_node(struct sprd_dma_channel *channel, int index, u32 list_end, dma_addr_t dma_addr, u32 len)
{
	...
	if(musb_channel->transmit) {
		...
		linklist_tx[index].data_addr = ((u64)dma_addr>>32) & 0x0F;
	} else {
		...
		linklist_rx[index].data_addr = ((u64)dma_addr>>32) & 0x0F;
	}
}

static void prepare_multi_linklist_node(struct sprd_dma_channel *channel, dma_addr_t dma_addr, u32 len)
{
	struct sprd_musb_dma_channel *musb_channel = channel->private_data;
...
static int sprd_dma_channel_program(struct sprd_dma_channel *sprd_channel,
			...
			dma_addr_t dma_addr, u32 len)
{
	...
	dev_vdbg(musb->controller, "ep%d-%s pkt_sz %d, dma_addr 0x%x length %d, mode %d\n",
		musb_channel->ep_num, musb_channel->transmit ? "Tx" : "Rx", packet_sz,
		dma_addr, len, mode);

	if (sprd_channel->status == MUSB_DMA_STATUS_UNKNOWN ||
		sprd_channel->status == MUSB_DMA_STATUS_BUSY)
		return -EINVAL;

	if (musb->ops->adjust_channel_params) {
		int ret = musb->ops->adjust_channel_params(sprd_channel,
			packet_sz, &mode, &dma_addr, &len);
		if (ret)
			return ret;
	}

	if ((musb->hwvers >= SPRD_MUSB_HWVERS_1800) && (dma_addr % 4))
		return -EINVAL;
	...
	prepare_multi_linklist_node(sprd_channel, dma_addr, len);

	musb_channel->blk_len = len;
	musb_channel->ioc = 1;
	musb_channel->list_end = 1;
	musb_channel->sp = 0;
	musb_channel->addr = dma_addr;
...
//		dev_dbg(musb->controller,"%p\n",&musb_channel);
	...
	sprd_configure_channel(sprd_channel, packet_sz, mode, dma_addr, len);

	return 0;
}

static int sprd_dma_channel_abort(struct sprd_dma_channel *dchannel)
{
	struct sprd_musb_dma_channel *mchannel = dchannel->private_data;
	struct sprd_musb_dma_controller *ctrl = mchannel->controller;
	void __iomem *mbase = ctrl->base;
	u8 index = mchannel->idx;
	u16 val;
	int offset;

	if (MUSB_DMA_STATUS_BUSY == dchannel->status) {
		if (mchannel->transmit) {
			offset = 0x100 + mchannel->ep_num * 0x10 + MUSB_TXCSR;

			/*
			 * Programming guide says that we must clear the
			 * DMAENAB bit before the DMAMODE bit...
			 */
			val = musb_readw(mbase, offset);
			val &= ~(MUSB_TXCSR_AUTOSET | MUSB_TXCSR_DMAENAB);
			musb_writew(mbase, offset, val);
			val &= ~MUSB_TXCSR_DMAMODE;
			musb_writew(mbase, offset, val);
		} else {
			offset = 0x100 + mchannel->ep_num * 0x10 + MUSB_RXCSR;
		...
		musb_writel(mbase, MUSB_DMA_CHN_LLIST_PTR(index), 0);
		musb_write_dma_addr(mbase, index, 0);
		dchannel->status = MUSB_DMA_STATUS_FREE;
	}
...
static void sprd_musb_dma_completion(struct musb *musb, u8 epnum, u8 transmit)
{
	...
	struct sprd_musb_request *musb_req;
	...
	if (list_is_empty(queue))
		return;
	musb_req = container_of(queue->next, struct sprd_musb_request, list);
	...
	sprd_musb_g_giveback(musb_ep, request, 0);
}
