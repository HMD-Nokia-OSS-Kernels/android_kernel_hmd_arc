...
extern uint8_t usb_get_dload_sys(void);
...
void prepare_linklist_node (uint32_t dir,int ep, unsigned int transfer_size, unsigned int *buffer, unsigned char list_end, unsigned int index)
{
	...
	if (index > 63) {
		dprintf(INFO,"linklist size overflow\r\n");
		return;
	}
	if (dir) {
		...
		if (list_end) {
			...
			writel(readl(MUSB_DMA_CHN_INTR(rx_ch)) | 0x14,(MUSB_DMA_CHN_INTR(rx_ch)));
			//set linklist pointer
			writel((uint32_t)linklist_ram_rx,(MUSB_DMA_CHN_LLIST_PTR(rx_ch)));
			//enable channel and trigger rx dma transfer
			writel(readl(MUSB_DMA_CHN_CFG(rx_ch)) | 0x1,(MUSB_DMA_CHN_CFG(rx_ch)));
		}
	} else {
		...
		if (transfer_size%512 != 0) {
			...
			if ((index + 1) > 63) {
				dprintf(INFO,"tx linklist size overflow\r\n");
				return;
			}
			dma_linklist_tx[index].addr = buffer;
			dma_linklist_tx[index].blk_len = transfer_size;
...
LOCAL void usb_get_descriptor (USB_REQUEST_1_U *request1, USB_REQUEST_2_U *request2)
{
	...
	{
	case USB_DEVICE_DESCRIPTOR_TYPES:
		send_data = dev_desc;
	...
	case USB_CONFIGURATION_DESCRIPTOR_TYPES:
		if (enum_speed)
		{
...
LOCAL void usb_setup_handle (void)
{
	...
	case USB_REQ_STANDARD://standard
		...
		switch (request1->mBits.brequest)
		{
			...
			case USB_REQUESTS_GET_DESCRIPTOR:
				usb_get_descriptor (request1, request2);
			break;

			case USB_REQUESTS_SET_CONFIGURATION:
				(*(volatile unsigned char*)Mentor_USB_CSR0L) |= (BIT_3 | BIT_6);
				reg= *(volatile unsigned short *)Mentor_USB_INTRTX;
				while((reg & BIT_0) != BIT_0)
					reg= *(volatile unsigned short *)Mentor_USB_INTRTX;
...
LOCAL void USB_EPxSendData (char ep_id,unsigned char* pBuf,int len)
{
	...
	writel(readl(MUSB_DMA_CHN_INTR(tx_ch)) | 0x04,(MUSB_DMA_CHN_INTR(tx_ch)));
	//set linklist pointer
	writel((uint32_t)linklist_ram_tx,MUSB_DMA_CHN_LLIST_PTR(tx_ch));
	//enable channel and trigger rx dma transfer
	writel(readl(MUSB_DMA_CHN_CFG(tx_ch)) | 0x1,(MUSB_DMA_CHN_CFG(tx_ch)));

	//polling linklist end int
	reg32=readl(MUSB_DMA_CHN_INTR(tx_ch));
	while ((reg32 & BIT_18) != BIT_18) {
		reg32=readl(MUSB_DMA_CHN_INTR(tx_ch));
#ifdef CONFIG_DOWNLOAD_USB_CHECK
		if (!is_usb_disconnected()) {
			errorf("usb disconnect when get packet, download end and power down device\n");
	...
	writel(readl(MUSB_DMA_CHN_INTR(tx_ch)) | BIT_26,(MUSB_DMA_CHN_INTR(tx_ch)));
...
LOCAL void usb_EP_out_handle (void)
{
	...
	writel(readl(MUSB_DMA_CHN_INTR(rx_ch)) | 0x14000000,(MUSB_DMA_CHN_INTR(rx_ch)));

	//get dma receive data len
	reg32_blk_len = readl(MUSB_DMA_CHN_LEN(rx_ch));
...
static __inline void usb_handler (void)
{
	...
	reg32 = readl(MUSB_DMA_CHN_INTR(USB_EP6+15));
...
static void usb_core_init (unsigned int autodl_mode)
{
	...
	//Configure RXFifoSize 4096
	c_size = 0x9;
	musb_write_rxfifosz(musb_base, c_size);
	//Configure maxpacket 512 & multi 6
	(*(volatile unsigned short*)Mentor_USB_RXMAXP(USB_EP6)) = 0x3200;
#endif
...
static inline void usb_ldo_switch(int flag)
{
	 if(flag){
...
static struct musb_fifo_cfg sprd_musb_mode_cfg[] = {
	SPRD_MUSB_EP_FIFO_SINGLE(5, FIFO_TX, 512),
	SPRD_MUSB_EP_FIFO_SINGLE(6, FIFO_RX, 512),
};
#else
static struct musb_fifo_cfg sprd_musb_mode_cfg[] = {
	SPRD_MUSB_EP_FIFO_DOUBLE(5, FIFO_TX, 512),
	SPRD_MUSB_EP_FIFO_DOUBLE(6, FIFO_RX, 512),
};
#endif
...
int usb_driver_init(unsigned int max_speed)
{
	...
	retval = musb_register(&sprd_musb, NULL, (void *)SPRD_USB_BASE);
	if (retval) {
		errorf("musb_register failed retval = %d\n",retval);
	}
	return retval;
}
