...
static void sprd_eic_reg_update(unsigned int offset, u16 reg, unsigned int val)
{
	struct eic_info eic_info;
	unsigned eic_id = offset;
	u32 tmp;
...
static int sprd_eica_irq_handler(void)
{
	...
	if (eic_info.base_addr == INVALID_REG) {
		WARN(1, "ERR:Get EIC %d base_addr\n", eic_id);
		return 0;
	}
	...
	while(value){
		...
		if(sprd_eic_cfg.int_table[n] != NULL)
			...
			dprintf(INFO,"eica:%d isr callback is null!\n", eic_id);
		sprd_eic_irq_unmask(eic_id);
	}
	return 0;
}

void EICA_EnableInt(void)
{
	unmask_interrupt(TB_EICA_INT);
}

void EICA_DisableInt(void)
{
	mask_interrupt(TB_EICA_INT);
}
...
{
#ifdef REG_EIC_EB
	CHIP_REG_OR(REG_EIC_EB, BIT_EIC_EB);
#else
	CHIP_REG_OR(REG_AON_APB_APB_EB0, BIT_EIC_EB);

#endif

	CHIP_REG_OR(REG_AON_APB_APB_RTC_EB, BIT_EIC_RTC_EB);
	CHIP_REG_OR(REG_AON_APB_APB_RTC_EB, BIT_EIC_RTCDV5_EB);
	ANA_REG_OR(ANA_REG_GLB_ARM_MODULE_EN, BIT_ANA_EIC_EN);
	ANA_REG_OR(ANA_REG_GLB_RTC_CLK_EN, BIT_RTC_EIC_EN);

#ifdef CONFIG_SPRD_EIC_IRQ
	register_int_handler(TB_EICA_INT, sprd_eica_irq_handler, NULL);
	EICA_EnableInt();
#endif
}
