...
extern struct gpio_section * gpio_get_section_table(u32 * table_size);
...
void sprd_gpio_int_reg(unsigned int id_num, GPIO_CALLBACK gpio_callback)
{
	...
	GPIO_DBG("sprd_gpio_int_reg gpio_id=%d\r\n", id_num);
...
static void sprd_gpio_reg_update(u32 offset, u16 reg, int value)
{
	struct gpio_info gpio_info;
	u32 tmp;
...
int sprd_gpio_get_mis(unsigned offset)
{
	unsigned gpio_id = offset;
	struct gpio_info gpio_info;
...
void sprd_gpio_irq_mask(unsigned offset)
{
	sprd_gpio_reg_update(offset, GPIO_IE, 0);
}

void sprd_gpio_irq_ack(unsigned offset)
{
	sprd_gpio_reg_update(offset, GPIO_IC, 1);
}

void sprd_gpio_irq_unmask(unsigned offset)
{
	sprd_gpio_reg_update(offset, GPIO_IC, 1);
	sprd_gpio_reg_update(offset, GPIO_IE, 1);
}

int sprd_gpio_irq_set_type(unsigned int flow_type, u32 offset)
{
	switch (flow_type) {
	case SPRD_IRQ_TYPE_EDGE_RISING:
		sprd_gpio_reg_update(offset, GPIO_IS, 0);
		sprd_gpio_reg_update(offset, GPIO_IBE, 0);
		sprd_gpio_reg_update(offset, GPIO_IEV, 1);
		sprd_gpio_reg_update(offset, GPIO_IC, 1);
		break;
	case SPRD_IRQ_TYPE_EDGE_FALLING:
		sprd_gpio_reg_update(offset, GPIO_IS, 0);
		sprd_gpio_reg_update(offset, GPIO_IBE, 0);
		sprd_gpio_reg_update(offset, GPIO_IEV, 0);
		sprd_gpio_reg_update(offset, GPIO_IC, 1);
		break;
	case SPRD_IRQ_TYPE_EDGE_BOTH:
		sprd_gpio_reg_update(offset, GPIO_IS, 0);
		sprd_gpio_reg_update(offset, GPIO_IBE, 1);
		sprd_gpio_reg_update(offset, GPIO_IC, 1);
		break;
	case SPRD_IRQ_TYPE_LEVEL_HIGH:
		sprd_gpio_reg_update(offset, GPIO_IS, 1);
		sprd_gpio_reg_update(offset, GPIO_IBE, 0);
		sprd_gpio_reg_update(offset, GPIO_IEV, 1);
		break;
	case SPRD_IRQ_TYPE_LEVEL_LOW:
		sprd_gpio_reg_update(offset, GPIO_IS, 1);
		sprd_gpio_reg_update(offset, GPIO_IBE, 0);
		sprd_gpio_reg_update(offset, GPIO_IEV, 0);
		break;
...
static int sprd_gpio_irq_handler(void)
{
	...
	for (bank = 0; bank * SPRD_GPIO_BANK_NR < NR_D_DIE_GPIOS; bank++) {
		...
		reg = gpio_reg_get(base + GPIO_MIS, D_DIE) & SPRD_GPIO_BANK_MASK;
			while (reg) {
			n = __ffs(reg);
			reg &= ~(1 << n);
			gpio = bank * SPRD_GPIO_BANK_NR + n;
			sprd_gpio_irq_ack(gpio);
			sprd_gpio_irq_mask(gpio);
			state = sprd_gpio_get(gpio);
			if (sprd_gpio_cfg.int_table[gpio] != NULL)
				(*(sprd_gpio_cfg.int_table[gpio]))(gpio, state);
			else
				dprintf(INFO,"gpio:%d isr callback is null!", gpio);
				sprd_gpio_irq_unmask(gpio);
		}
	}
	return 0;
}
...
{
	if (directions)
		sprd_gpio_direction_output(gpio_id, 0);
	else
		sprd_gpio_direction_input(gpio_id);

}

void GPIO_SetInterruptsSense (unsigned int gpio_id, int sensetype)
{
	sprd_gpio_irq_set_type(sensetype, gpio_id);
}

void GPIO_ClearIntStatus (unsigned int gpio_id)
{
	sprd_gpio_irq_ack(gpio_id);
}
#endif

#if 0
static int sprd_gpio_to_irq(unsigned offset)
{
	int i;
...
#ifdef REG_GPIO_EB
	CHIP_REG_OR(REG_GPIO_EB, BIT_GPIO_EB);
#else
	CHIP_REG_OR(REG_AON_APB_APB_EB0, BIT_GPIO_EB);
#endif
...
#ifdef CONFIG_SPRD_GPIO_IRQ
	register_int_handler(TB_GPIO_INT, sprd_gpio_irq_handler, NULL);
	GPIO_EnableInt();
#endif
}
