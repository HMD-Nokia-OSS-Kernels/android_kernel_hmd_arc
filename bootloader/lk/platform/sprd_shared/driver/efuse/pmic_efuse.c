...
/* registers definitions for controller ANA_REGS_EFUSE */
	...
	defined(CONFIG_ADIE_UMP9620) || defined(CONFIG_ADIE_UMP518)
...
u32 sprd_pmic_efuse_read_ump9621(int blk_index)
{
	...
	dprintf(ALWAYS, "pmic efuse read %d,val=0x%x\n", blk_index,val);
	return val;
}
#endif
...
u32 sprd_pmic_efuse_read_bits(int bit_index, int length)
{
	...
	val &= BIT(length) - 1;
	dprintf(ALWAYS, "pmic efuse read bits %d ++ %d 0x%08x\n\n", bit_index, length, val);
	return val;
}
...
{
	int i, idx;
	dprintf(ALWAYS, "pmic efuse blocks dump:\n");
	...
	for (idx = 0; idx < EFUSE_BLOCK_MAX; idx++) {
		...
		dprintf(ALWAYS, "%02d  ", idx);
		for (i = EFUSE_BLOCK_WIDTH - 1; i >= 0; --i)
			dprintf(ALWAYS, "%s ", (val & BIT(i)) ? "1" : "0");
		dprintf(ALWAYS, "\n");
	}
	dprintf(ALWAYS, "\n");
}
