...
#define SPRD_RGB_PD_SW BIT(12)
#define SPRD_RGB_HW_PD BIT(13)
#define SPRD_RGB_V     (0xf8)
...
#else
	ANA_REG_BIC(ANA_BLTC_CTRL, SPRD_RGB_PD_SW);
	ANA_REG_BIC(ANA_BLTC_CTRL, SPRD_RGB_HW_PD);
	ANA_REG_BIC(ANA_BLTC_STS, SPRD_RGB_V);
#endif
	init = 1;
}
...
void sprd_led_disable(LedType type)
{
	...
	dprintf(INFO, "[LED] disable led, type=%d\n", type);
}
...
void sprd_led_set_brightness(LedType type, uint32_t brightness)
{
	...
	dprintf(INFO, "[LED] set brightness, type=%d, brightness=%u\n", type, brightness);
}
...
void sprd_led_pattern_clear(LedType type)
{
	...
	ANA_REG_BIC(ANA_BLTC_CTRL, bits);
	dprintf(INFO, "[LED] pattern clear, type=%d\n", type);
}
...
void sprd_led_pattern_set(LedType type, uint32_t brightness)
{
	...
	ANA_REG_OR(ANA_BLTC_CTRL, bits);
	dprintf(INFO, "[LED] pattern set, type=%d, brightness=%d, rising=%d, high=%d, falling=%d, low=%d\n",
