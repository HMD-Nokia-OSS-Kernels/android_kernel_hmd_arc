...
#if defined(CONFIG_ADIE_SC2720) || defined(CONFIG_ADIE_SC2730) || defined(CONFIG_ADIE_UMP9620) || defined(CONFIG_ADIE_UMP518)
...
#define SPRD_ADC_LOG_LIMIT(fmt, ...)				\
do {								\
	if (!enter_sysdump_flag)				\
		dprintf(CRITICAL, pr_fmt(fmt), ##__VA_ARGS__);	\
} while (0)
...
struct sprd_adc_cal batdet_scale_cal = {
	1400, 3482,
	200, 476,
	SPRD_AUXADC_CAL_NO,
};
...
static struct pmic_adc_para para = {
	...
	.batdet_cal_efuse_blk_1v4 = 35,
	.batdet_cal_efuse_blk_0v2 = 34,
	.blk_width = 16,
	.cal_efuse_bit = 16,
};
...
static struct pmic_adc_para para = {
	.small_cal_efuse_blk_1v0 = 33,
	.small_cal_efuse_blk_0v1 = 34,
	.big_cal_efuse_blk_3v6 = 35,
	.big_cal_efuse_blk_4v2 = 36,
	.batdet_cal_efuse_blk_1v4 = 31,
	.batdet_cal_efuse_blk_0v2 = 32,
	.blk_width = 16,
	.cal_efuse_bit = 16,
};
...
static void sprd_adc_data_qusort(u32 a[], int N)
{
	int i, j, temp = 0;
	for (i = 0; i < N; i++) {
		for (j = i+1; j < N; j++) {
...
static int pmic_adc_set_current(u8 enable, int isen)
{
	...
	dprintf(ALWAYS, "ANA_REG_GLB_BATDET_CUR_CTRL =0x%x\n",sci_adi_read(ANA_REG_GLB_BATDET_CUR_CTRL));
	return 0;
}

#elif defined(CONFIG_ADIE_SC2721) || defined(CONFIG_ADIE_SC2720) || \
	defined(CONFIG_ADIE_SC2730) || defined(CONFIG_ADIE_UMP9620) || defined(CONFIG_ADIE_UMP518)
				   ...
				   int isen)
{
	...
	if (0 == pmic_adc_get_values(channel, scale, num, results)) {
		sprd_adc_data_qusort(results, num);
		values =
	...
	for (i = 0; i < ARRAY_SIZE(results); i++) {
		dprintf(ALWAYS, "%d\t", results[i]);
	}
	dprintf(ALWAYS, "adc[%d] isen: %d, value: %d\n", channel, isen, values);
...
#elif defined(CONFIG_ADIE_UMP9620) || defined(CONFIG_ADIE_UMP518)
...
#if defined(CONFIG_ADIE_UMP9620) || defined(CONFIG_ADIE_UMP518)
...
static int sprd_get_adc_small_scale_cal(uint32_t *p_cal_data)
{
	...
	dprintf(ALWAYS, "adc deta_1v0:0x%x,p_cal_data[0]:0x%x\n", deta_1v0, p_cal_data[0]);

	/* adc 0.1V */
	p_cal_data[1] = (deta_0v1 & 0xfff0) >> 4;
	dprintf(ALWAYS, "adc deta_0v1:0x%x,p_cal_data[1]:0x%x\n", deta_0v1, p_cal_data[1]);
...
static int sprd_get_adc_small_scale_cal(uint32_t *p_cal_data)
{
	...
	dprintf(ALWAYS, "get pmic efuse block %u, deta: 0x%08x\n",
...
int sprd_get_adc_bat_cal(uint32_t *p_cal_data)
{
	...
	dprintf(ALWAYS, "adc bat deta_4v2: 0x%x, p_cal_data[0]: 0x%x\n", deta_4v2, p_cal_data[0]);
	/* adc 3.6V */
	p_cal_data[1] = (deta_3v6 & 0xfff0) >> 4;
	dprintf(ALWAYS, "adc bat deta_3v6: 0x%x, p_cal_data[1]: 0x%x\n", deta_3v6, p_cal_data[1]);
...
#if defined(CONFIG_ADIE_UMP9620) || defined(CONFIG_ADIE_UMP518)
static int sprd_get_adc_batdet_scale_cal(uint32_t *p_cal_data)
{
	uint32_t deta_1v4 = 0, deta_0v2 = 0;

	if (NULL == p_cal_data) {
		sprd_err("ERR!adc batdet cal p_cal_data is NULL\n");
		return -1;
	}
	/* adc 1.4V */
	deta_1v4 = sprd_pmic_efuse_read_bits(BITSINDEX(para.batdet_cal_efuse_blk_1v4, 0),
					     para.cal_efuse_bit);
	if (!deta_1v4) {
		sprd_err("ERR! adc batdet cal deta_1v4:0x%x\n", deta_1v4);
		return -1;
	}
	/* adc 0.2V */
	deta_0v2 = sprd_pmic_efuse_read_bits(BITSINDEX(para.batdet_cal_efuse_blk_0v2, 0),
					     para.cal_efuse_bit);
	if (!deta_0v2) {
		sprd_err("ERR! adc bat cal deta_0v2:0x%x\n", deta_0v2);
		return -1;
	}

	/* adc 1.4V */
	p_cal_data[0] = (deta_1v4 & 0xfff0) >> 4;
	dprintf(ALWAYS, "adc batdet deta_1v4: 0x%x, p_cal_data[0]: 0x%x\n", deta_1v4, p_cal_data[0]);
	/* adc 0.2V */
	p_cal_data[1] = (deta_0v2 & 0xfff0) >> 4;
	dprintf(ALWAYS, "adc bat deta_0v2: 0x%x, p_cal_data[1]: 0x%x\n", deta_0v2, p_cal_data[1]);

	return 0;

}
#else
static int sprd_get_adc_batdet_scale_cal(uint32_t *p_cal_data){return 0;}
#endif
...
static int sprd_bat_scale_adc_efuse_get(void)
{
	...
	if (!sprd_get_adc_bat_cal(efuse_cal_data)) {
		...
		dprintf(ALWAYS, "bat scal adc,p0_vol:%d, p0_adc:%d, p1_vol:%d, p1_adc:%d, cal_type:%d\n",
...
static int sprd_small_scale_adc_efuse_get(void)
{
	...
	if (!sprd_get_adc_small_scale_cal(efuse_cal_data)) {
		...
		dprintf(ALWAYS, "small scal adc p0_vol:%d, p0_adc:%d, p1_vol:%d, p1_adc:%d, cal_type:%d\n",
...
static int sprd_batdet_scale_adc_efuse_get(void)
{
	uint32_t efuse_cal_data[2] = { 0 };

	if (!sprd_get_adc_batdet_scale_cal(efuse_cal_data)) {
		batdet_scale_cal.p0_adc = efuse_cal_data[0];
		batdet_scale_cal.p1_adc = efuse_cal_data[1];
		batdet_scale_cal.cal_type = SPRDLIT_AUXADC_CAL_CHIP;
		dprintf(ALWAYS, "batdet scal adc p0_vol:%d, p0_adc:%d, p1_vol:%d, p1_adc:%d, cal_type:%d\n",
			batdet_scale_cal.p0_vol, batdet_scale_cal.p0_adc, batdet_scale_cal.p1_vol,
			batdet_scale_cal.p1_adc, batdet_scale_cal.cal_type);
		return 0;
	}
	return -1;
}
...
static int sprd_bat_scale_adc_efuse_get(void)
{
	...
	if (!sprd_get_adc_bat_cal(efuse_cal_data)) {
		...
		big_scale_cal.cal_type = SPRDBIG_AUXADC_CAL_CHIP;
		dprintf(ALWAYS, "adc efuse big cal %d,%d,%d,%d,cal_type:%d\n",
...
static int sprd_small_scale_adc_efuse_get(void)
{
	...
	if (!sprd_get_adc_small_scale_cal(efuse_cal_data)) {
		...
		small_scale_cal.cal_type = SPRDLIT_AUXADC_CAL_CHIP;
		dprintf(ALWAYS, "adc efuse small cal %d,%d,%d,%d,cal_type:%d\n",
...
static int sprd_batdet_scale_adc_efuse_get(void){ return 0;}
#endif
...
void pmic_adc_Init(void)
{
	...
	if (0 == sprd_small_scale_adc_efuse_get())
		dprintf(ALWAYS, "sprd small scale efuse have cal\n");
	if (0 == sprd_bat_scale_adc_efuse_get())
		dprintf(ALWAYS, "sprd big scale efuse have cal\n");
	if (0 == sprd_batdet_scale_adc_efuse_get())
		dprintf(ALWAYS, "sprd batdet scale efuse have cal\n");
	...
	dprintf(ALWAYS, "ADC_CTL1 value of it 0x%x\n", ANA_REG_GET(ADC_CTL1));
#endif
}
...
static int32_t sprd_small_scale_to_vol(int32_t adcvalue)
{
	...
	SPRD_ADC_LOG_LIMIT("small scale: (%d, %d), (%d, %d), (0x%x, %d)\n", small_scale_cal.p0_vol,
		small_scale_cal.p0_adc, small_scale_cal.p1_vol, small_scale_cal.p1_adc,
		adcvalue, vol);
...
static int32_t sprd_bat_scale_adc_to_vol(int32_t adcvalue)
{
	...
	SPRD_ADC_LOG_LIMIT("big scale: (%d, %d), (%d, %d), (0x%x, %d)\n", big_scale_cal.p0_vol,
		big_scale_cal.p0_adc, big_scale_cal.p1_vol, big_scale_cal.p1_adc,
		adcvalue, vol);
...
#if defined(CONFIG_ADIE_UMP9620) || defined(CONFIG_ADIE_UMP518)
static int32_t sprd_batdet_scale_adc_to_vol(int32_t adcvalue)
{
	int32_t vol;

	vol = batdet_scale_cal.p0_vol - batdet_scale_cal.p1_vol;
	vol = vol * (adcvalue - batdet_scale_cal.p1_adc);
	vol = vol / (batdet_scale_cal.p0_adc - batdet_scale_cal.p1_adc);
	vol = vol + batdet_scale_cal.p1_vol;

	SPRD_ADC_LOG_LIMIT("batdet scale: (%d, %d), (%d, %d), (0x%x, %d)\n", batdet_scale_cal.p0_vol,
		batdet_scale_cal.p0_adc, batdet_scale_cal.p1_vol, batdet_scale_cal.p1_adc,
		adcvalue, vol);
...
static int32_t sprd_batdet_scale_adc_to_vol(int32_t adcvalue)
{
	return sprd_small_scale_to_vol(adcvalue);
}
#endif
				  ...
				  int32_t adcvalue)
{
	...
	SPRD_ADC_LOG_LIMIT("big scale adc %d vol %d\n", adcvalue, result);
				    ...
				    int32_t adcvalue)
{
	...
	SPRD_ADC_LOG_LIMIT("small adc to vol, numerators: %d, denominators: %d, adc: %d, vol: %d\n",
		numerators, denominators, adcvalue, result);
	return result;
}

int32_t sprd_chan_batdet_adc_to_vol(uint16_t channel, int scale, int mux,
				int32_t adcvalue)
{
	int32_t result;
	int32_t vol;
	...
	int32_t bat_numerators, bat_denominators;
	int32_t numerators, denominators;
	vol = sprd_batdet_scale_adc_to_vol(adcvalue);
	bat_numerators = 1;
	bat_denominators = 1;
	ratio = pmic_adc_get_ratio(channel, scale, mux);
	numerators = ratio >> 16;
	...
	SPRD_ADC_LOG_LIMIT("batdet adc to vol, numerators: %d, denominators: %d, adc: %d, vol: %d\n",
		numerators, denominators, adcvalue, result);
	return result;
}
			      ...
			      int cal_type)
{
	...
	if (0 == pmic_adc_get_values(channel, scale, num, adc_value)) {
		sprd_adc_data_qusort(adc_value, num);
		adc_average =
	...
	} else if (cal_type == ADC_BATDET_SCALE) {
		vol = sprd_chan_batdet_adc_to_vol(channel, scale, mux, adc_average);
	} else {
