...
static void dcdc_zcd_cali_cfg(void)
{
...
static void dcdc_ripple_cali_cfg(void)
{
...
struct regulator_desc *regulator_get(const char *id)
{
	...
	dprintf(INFO,"The regulator %s is not found.\n", id);
...
unsigned int regulator_get_voltage(const char con_id[])
{
	...
	vol /= 1000;
	dprintf(INFO,"Get %s voltage: %dmv, reg_val: 0x%08x.\n", con_id, vol, reg_val);
...
int regulator_set_voltage(const char con_id[], int vol_mv)
{
	...
	reg_val = ANA_REG_GET(desc->vol_trm);
	dprintf(INFO,"Set %s voltage: %dmv, reg_val: 0x%08x.\n", con_id, vol_mv, reg_val);
