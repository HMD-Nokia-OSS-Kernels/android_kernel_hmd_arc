...
static void umb9230s_reg_write(struct phy_tx_context *ctx, uint16_t reg, uint8_t val)
{
    u32 buf[2] = {};
    ...
    iic2cmd_write(ctx->i2c_bus, ctx->i2c_addr, buf, 2);
}
...
static int dphy_set_pll_reg(struct phy_tx_context *ctx, struct dphy_pll *pll)
{
	if (!pll || !pll->fvco)
		goto FAIL;
	...
	umb9230s_reg_write(ctx, 0x02, regs._02.val);
...
static int dphy_pll_config(struct phy_tx_context *ctx)
{
	int ret;
	...
	umb9230s_reg_write(ctx, 0x25, regs._25.val);
#endif
...
static int umb9230s_dphy_set_timing_regs(struct phy_tx_context *ctx, int type, u8 val[])
{
	switch (type) {
	case REQUEST_TIME:
...
static int dphy_timing_config(struct phy_tx_context *ctx)
{
	u8 val[2];
	u32 tmp = 0;
...
static int dphy_hop_start(struct phy_tx_context *ctx)
{
	/* start hopping */
...
static int dphy_hop_stop(struct phy_tx_context *ctx)
{
...
static int dphy_hop_config(struct phy_tx_context *ctx, int delta, int period)
{
	dphy_hop_stop(ctx);
...
static int dphy_ssc_start(struct phy_tx_context *ctx)
{
	pll.mod_en = true;
	pll.hop_en = false;
...
static int dphy_ssc_stop(struct phy_tx_context *ctx)
{
	pll.mod_en = false;
...
static int dphy_ssc_en(struct phy_tx_context *ctx, bool en)
{
	if (en) {
 ...
 */
static void dphy_force_pll(struct phy_tx_context *ctx, int force)
{
	u8 data;
...
{
	return &umb9230s_dphy_tx_pll_ops;
}
