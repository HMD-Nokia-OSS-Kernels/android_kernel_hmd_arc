...
struct dpi_video_param {
	...
	u16 vfp;
	/* Video porch enter lp mode config */
...
struct dsi_core_ops {
	...
	void (*dpi_vporch_lp_en)(struct dsi_context *ctx, int enable, uint32_t video_lp_config);
...
__attribute__((weak)) struct dsi_glb_ops *dsi_s_glb_ops_attach(void);
