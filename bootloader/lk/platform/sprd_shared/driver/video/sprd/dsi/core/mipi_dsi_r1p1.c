...
static bool dsi_check_version(struct dsi_context *ctx)
{
	...
	if (read32(&reg->DSI_VERSION) == 0x100)
		...
		return true;
	else if (read32(&reg->DSI_VERSION) == 0x303)
		return true;
	else
		return false;
}
...
static void dsi_dpi_vfp(struct dsi_context *ctx, u16 lines)
{
	...
	dpi_vfp.val = read32(&reg->DPI_VFP);
	dpi_vfp.bits.vfp_lines = lines;

	write32(dpi_vfp.val, &reg->DPI_VFP);
}
/**
 ...
 * @param enable (1) - disable (0)
 */
static void dsi_dpi_vporch_lp_en(struct dsi_context *ctx, int enable, uint32_t video_lp_config )
{
	struct dsi_reg *reg = (struct dsi_reg *)ctx->base;
	union _0x38 vid_mode_cfg;
