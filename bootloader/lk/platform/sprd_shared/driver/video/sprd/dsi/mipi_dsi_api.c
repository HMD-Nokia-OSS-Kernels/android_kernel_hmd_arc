...
int mipi_dsi_dpi_video(struct sprd_dsi *dsi, struct dpi_video_param *param)
{
	...
	dsi_hal_dpi_vporch_lp_en(dsi, 1, param->video_lp_config);
...
{
	u8 payload[] = {
		SPRD_MIPI_DCS_SET_COLUMN_ADDRESS,
		start >> 8,
		start & 0xff,
		end >> 8,
		end & 0xff
...
{
	u8 payload[] = {
		SPRD_MIPI_DCS_SET_PAGE_ADDRESS,
		start >> 8,
		start & 0xff,
		end >> 8,
		end & 0xff
...
{
	u8 type = SPRD_MIPI_DCS_READ_MEMORY_START;
...
{
	u8 type = SPRD_MIPI_DCS_READ_MEMORY_CONTINUE;
...
void mipi_dsi_set_work_mode(struct sprd_dsi *dsi, u8 mode)
{
	 ...
	 */
	if (!(dsi_hal_wait_tx_cmd_fifo_empty(dsi) &&
	      dsi_hal_wait_tx_payload_fifo_empty(dsi)))
