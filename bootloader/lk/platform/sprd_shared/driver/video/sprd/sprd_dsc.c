					...
					int pixelsPerGroup, int numSsps)
{
	...
	if (dsc_init->first_linebpg_ofs < 0) {
		if (dsc_cfg->slice_height >= 8)
			dsc_init->first_linebpg_ofs = 12 + ((int)(9/100 * SPRD_MIN(34,
	...
	if (dsc_cfg->slice_height == 1) {
		...
		pr_err("The bits/pixel allocation too low ");
	if (final_scale > 9) {
...
static void write_bits(int val, int size, unsigned char *buf, int *bit_count)
{
	...
	if (size > 32)
		pr_err("error: supports max of 32 bits\n");
...
int calc_dsc_params(struct dsc_init_param *dsc_init)
{
	struct dispc_context *ctx =
		(struct dispc_context *)container_of(dsc_init, struct dispc_context, dsc_init);
	...
	set_defaults(&ctx->dsc_init, panel->output_bpc);

	memset(&ctx->dsc_cfg, 0, sizeof(struct dsc_cfg));

	ctx->dsc_init.init_pic_width = panel->width;
	ctx->dsc_init.init_pic_height = panel->height;
	ctx->dsc_init.init_slice_width = panel->slice_width;
	ctx->dsc_init.init_slice_height = panel->slice_height;
	...
	if (calc_rc_params(&ctx->dsc_init, &ctx->dsc_cfg, pixelsPerGroup, numSsps))
		pr_err("One or more PPS parameters exceeded their allowed bit depth.");
		...
		struct panel_info *panel)
{
	...
	set_defaults(dsc_init, panel->umb9230s_output_bpc);

	memset(dsc_cfg, 0, sizeof(struct dsc_cfg));

	dsc_init->init_pic_width = panel->width;
	dsc_init->init_pic_height = panel->height;
	dsc_init->init_slice_width = panel->umb9230s_slice_width;
	dsc_init->init_slice_height = panel->umb9230s_slice_height;
	memset(pps, 0, PPS_SIZE);
