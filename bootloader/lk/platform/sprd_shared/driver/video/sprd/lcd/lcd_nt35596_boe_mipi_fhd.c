/*
 *  <lcd_nt35596_boe_mipi_fhd.c> - <lcd nt35596 boe mipi fhd>
 *
...
static int nt35596_readid(struct panel_info *info)
{
	struct sprd_dsi *dsi = &dsi_device;
	...
	if (read_buf[0] == 0x96) {
		pr_info("nt35596 read id success! readbuf: 0x%02x\n", read_buf[0]);
		return 0;
	}

	pr_err("nt35596 read id failed!readbuf: 0x%02x\n", read_buf[0]);
	return -1;
}
...
static int nt35596_sleep_in(void)
{
	struct sprd_dsi *dsi = &dsi_device;

	mipi_dsi_lp_cmd_enable(dsi, true);
	mipi_dsi_send_cmds(dsi, sleep_in_data);
	pr_info("nt35596_sleep_in end\n");
	return 0;
}
