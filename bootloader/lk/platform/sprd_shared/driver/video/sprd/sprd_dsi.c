...
static int32_t dsi_dpi_video(struct sprd_dsi *dsi)
{
	...
	param.hact = (panel->width >> panel->bv3_en) / dual_dsi_para;
	param.hsync = (timing->hsync) / dual_dsi_para;
	param.hbp = (timing->hbp) / dual_dsi_para;
	param.hline = (panel->width
		+ timing->hsync + timing->hbp + timing->hfp) / dual_dsi_para;
