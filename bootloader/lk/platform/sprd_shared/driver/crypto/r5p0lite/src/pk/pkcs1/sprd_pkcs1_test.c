...
int pkcs1_pss_emsa_test(void)
{
	...
	SPRD_CRYPTO_LOG_ERR("----------pkcs1 pkcs1 pss emsa test ----------\n");
	...
	if (!ret && !res) {
		SPRD_CRYPTO_LOG_ERR("----------pkcs1 pss emsa pass2, ret = (%x) res = (%x)-----------\n", ret, res);
	} else {
		SPRD_CRYPTO_LOG_ERR("----------pkcs1 pss emsa fail2, ret = (%x) res = (%x)-----------\n", ret, res);
		return -2;
	}
	...
	if (!ret) {
		SPRD_CRYPTO_LOG_ERR("----------pkcs1 pss emsa pass3, ret = (%x) outlen = (%d)-----------\n", ret, outlen);
	} else {
		SPRD_CRYPTO_LOG_ERR("----------pkcs1 pss emsa fail3, ret = (%x) outlen = (%d)-----------\n", ret, outlen);
		return -3;
	}
	ret = sprd_pkcs1_pss_decode(test_message_sha1_hash, 20,
				out, outlen, 0, SPRD_CRYPTO_HASH_SHA1, SPRD_CRYPTO_HASH_SHA1, 1024, &res);
	if (!ret && !res) {
		SPRD_CRYPTO_LOG_ERR("----------pkcs1 pss emsa pass4, ret = (%x) res = (%x)-----------\n", ret, res);
	} else {
		SPRD_CRYPTO_LOG_ERR("----------pkcs1 pss emsa fail4, ret = (%x) res = (%x)-----------\n", ret, res);
		return -4;
	}
