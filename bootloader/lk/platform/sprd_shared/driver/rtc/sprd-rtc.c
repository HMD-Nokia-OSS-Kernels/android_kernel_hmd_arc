 ...
 *
 *  sprd-rtc.c - Support bootloader rtc operation interface
 *
 *  History:
 *      2021/7/29 qinjiang.ye@unisoc.com
 *      Add bootloader rtc operation interface
 */
...
int sprd_clean_rtc(void)
{
	...
	dprintf(INFO,"now time sec %lu\n", sprd_rtc_get_sec());
	dprintf(INFO,"now alarm sec %lu\n", sprd_rtc_get_alarm_sec());
	return 0;
}
...
{
	//dprintf(INFO,"ANA_RTC_INT_RSTS is 0x%x\n", ANA_RTC_INT_RSTS);
