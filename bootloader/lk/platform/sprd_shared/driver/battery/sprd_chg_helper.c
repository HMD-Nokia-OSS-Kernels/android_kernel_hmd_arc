...
uint16_t sprdbat_auxadc2vbatvol(uint16_t adcvalue)
{
	...
	dprintf(INFO,"sprd_chg: uboot battery voltage:%d,adc4200:%d,adc3600:%d\n",
...
uint32_t sprdbat_get_aux_vbatvol(void)
{
	...
	if (pmic_adc_get_values(ADC_CHANNEL_VBAT_SENSE, false, SAMPLE_CNT, results)) {
		dprintf(ALWAYS,"sprd_chg: ERR: read vbat adc fail!!!\n");
		return 0;
	}

	for (i = 0; i < SAMPLE_CNT; i++) {
		dprintf(INFO,"sprd_chg: vbat adc%d:%d\n",i,results[i]);
		value += results[i];
	}

	value /= SAMPLE_CNT;
	dprintf(INFO,"sprd_chg: vbat avg adc:%d\n", value);
	value = sprdbat_auxadc2vbatvol(value);
	dprintf(ALWAYS,"sprd_chg: vbat vol:%d\n", value);
	return value;
}
...
void sprdbat_get_vbatauxadc_caldata(void)
{
	...
	if (adc_cal_flag == ADC_CAL_TYPE_NO) {
		if (!sprd_get_adc_bat_cal(adc_data)) {
			dprintf(INFO,"sprd_chg: adc_para from efuse is 0x%x 0x%x \n",
...
int sprdbat_is_battery_connected(int log_flag)
{
	...
	ret = sprd_eic_get(EIC_BATDET);
	if (log_flag)
		dprintf(ALWAYS,"sprd_chg: eica status bat%x\n", ret);
