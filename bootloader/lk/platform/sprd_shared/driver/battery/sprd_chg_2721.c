...
#if DEBUG
#define debugf(fmt, args...) do { dprintf(INFO,"chg: %s(): ", __func__); dprintf(INFO,fmt, ##args); } while (0)
#else
...
static void sprdchg_cmd(enum sprdchg_cmd cmd,int value)
{
	switch (cmd) {
...
int sprdchg_charger_is_adapter(void)
{
	...
	dprintf(ALWAYS,"chg_status=%d, ret = %d\n", charger_status, ret);
	return ret;
}
