...
static int bq25890_read_reg(u8 reg, u8 *value)
{
	...
	dprintf(ALWAYS,"bq25890_read_reg reg = 0x%x, value = %d/0x%x\n", reg, *value, *value);
...
static u8 BQ25890_get_value(u8 reg, u8 reg_bit, u8 reg_shift)
{
	...
	bq25890_read_reg(reg, &reg_value);
	reg_value = (reg_value & reg_bit) >> reg_shift;

	return reg_value;
}

static void chg_bq25890_set_chg_cur(u32 cur)
{
	u8 reg_value;
...
static void chg_bq25890_set_limit_cur(u32 limit)
{
	u8 reg_value;

	bq25890_set_value(BQ25890_REG_00, REG00_EN_ILIM_MASK,
...
static int chg_bq25890_set_prechg(u32 ichg)
{
	u8 reg_value;
	...
	bq25890_set_value(BQ25890_REG_05, REG05_IPRECHG_MASK,
			  REG05_IPRECHG_SHIFT, reg_value);

	return 0;
}

static void chg_bq25890_cmd(enum sprdchg_cmd cmd, int value)
{
	switch (cmd) {
...
static int bq25890_charger_get_vendor_id_part_value(void)
{
	int ret = 0;
	u8 reg_part_val = 0;

	ret = bq25890_read_reg(BQ25890_REG_14, &reg_part_val);
	if (ret < 0) {
...
void sprdchg_bq25890_init(void)
{
	...
	dprintf(INFO,"bq25890 init\n");

	ret = bq25890_charger_get_vendor_id_part_value();
	if (ret) {
		printf("[%s]l=%d: bq25890 is not found, not register ops\n", __FUNCTION__, __LINE__);
		return;
	}

	dprintf(INFO,"bq25890 register charge ops!\n");
