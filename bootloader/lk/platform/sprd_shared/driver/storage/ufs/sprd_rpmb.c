/*
 * * Copyright 2022 Unisoc (Shanghai) Technologies Co., Ltd.
 * * Licensed under the Unisoc General Software License, version 1.0 (the License);
 * * you may not use this file except in compliance with the License.
 * * You may obtain a copy of the License at
 * * https://www.unisoc.com/en_us/license/UNISOC_GENERAL_LICENSE_V1.0-EN_US
 * * Software distributed under the License is distributed on an "AS IS" BASIS,
 * * WITHOUT WARRANTIES OF ANY KIND, either express or implied.
 * * See the Unisoc General Software License, version 1.0 for more details.
 * */

#include <config.h>
#include <asm/arch/common.h>
#include <sprd_common_rw.h>
#include <linux/types.h>
#include <malloc.h>
#include <sprd_rpmb.h>
#include <sprd_ufs.h>
#include <lk_sec_drv.h>
#include <tee_smc_call.h>
#include <sec_rpmb.h>
#include <linux/byteorder/little_endian.h>
#include <linux/byteorder/generic.h>

#include "protocol_util.h"


struct lu_info_tbl rpmb_lu_info = {0};

static void u16_to_bytes(uint16_t u16, uint8_t *bytes)
{
	*bytes = (uint8_t) (u16 >> 8);
	*(bytes + 1) = (uint8_t) u16;
}

static void bytes_to_u16(uint8_t *bytes, uint16_t *u16)
{
	*u16 = (uint16_t) ((*bytes << 8) + *(bytes + 1));
}

static void bytes_to_u32(uint8_t *bytes, uint32_t *u32)
{
	*u32 = (uint32_t) ((*(bytes) << 24) +
			   (*(bytes + 1) << 16) +
			   (*(bytes + 2) << 8) + (*(bytes + 3)));
}

static void u32_to_bytes(uint32_t u32, uint8_t *bytes)
{
	*bytes = (uint8_t) (u32 >> 24);
	*(bytes + 1) = (uint8_t) (u32 >> 16);
	*(bytes + 2) = (uint8_t) (u32 >> 8);
	*(bytes + 3) = (uint8_t) u32;
}

static void bytes_to_u64(uint8_t *bytes, uint64_t *u64)
{
	uint32_t u32_l;
	uint32_t u32_u;

	bytes_to_u32(bytes, &u32_u);
	bytes_to_u32(bytes + 4, &u32_l);

	*u64 = (uint64_t)((uint64_t)u32_u << 32 | u32_l);
}

static void u64_to_bytes(uint64_t u64, uint8_t *bytes)
{
	*bytes = (uint8_t) (u64 >> 56);
	*(bytes + 1) = (uint8_t) (u64 >> 48);
	*(bytes + 2) = (uint8_t) (u64 >> 40);
	*(bytes + 3) = (uint8_t) (u64 >> 32);
	*(bytes + 4) = (uint8_t) (u64 >> 24);
	*(bytes + 5) = (uint8_t) (u64 >> 16);
	*(bytes + 6) = (uint8_t) (u64 >> 8);
	*(bytes + 7) = (uint8_t) u64;
}

int prepare_rpmb_lu(void)
{
	struct ufs_rpmb_unit_desc_tbl *rpmb_unit_desc;
	uint64_t block_count = 0;
	uint8_t ret = 0;

	ret = get_descriptor_data((void **)(&rpmb_unit_desc),
                                   RPMB_UNIT_DESC, 0xc4, 0);
	if (ret != UFS_SUCCESS)
		return ret;
	block_count = be64_to_cpu(rpmb_unit_desc->qLogicalBlockCount);
	if (block_count) {
		rpmb_lu_info.lu_index = 0xc4;
		rpmb_lu_info.blkcnt = block_count;
		rpmb_lu_info.log2blksz = rpmb_unit_desc->bLogicalBlockSize;
		rpmb_lu_info.memory_type =rpmb_unit_desc->bMemoryType;
		rpmb_lu_info.bootable = rpmb_unit_desc->bBootLunID;
		rpmb_lu_info.write_protect = rpmb_unit_desc->bLUWriteProtect;
	}
	/*
	 * write key,read counter,write data,
	 * need this test_unit_ready operation.
	 */
	ret = test_unit_ready(&(rpmb_lu_info));

	return ret;
}

/* @retrun 0 get rpmb package successful */
int check_ufs_rpmb_key(uint8_t *package, int package_size)
{
	/* Must match in tos */
	uint8_t nonce[RPMB_NONCE_SIZE] = {
		0xa5, 0x5a, 0xff, 0x00, 0xbe, 0xef, 0xbe, 0xef,
		0xbe, 0xef, 0xbe, 0xef, 0x00, 0xff, 0x5a, 0xa5
	};
	struct rpmb_data_frame *data_frame;
	uint16_t msg_type;
	uint16_t op_result;
	uint32_t writecount;
	lbaint_t transfer_blkcnt = 0;
	int ret = 0;

	if (rpmb_lu_info.log2blksz == 0) {
		ret = prepare_rpmb_lu();
		if(ret != UFS_SUCCESS)
			return ret;
	}

	/* rpmb_lu_info.log2blksz=0x08,256B */
	transfer_blkcnt = RPMB_DATA_FRAME_SIZE >> (rpmb_lu_info.log2blksz);
	if (!package || package_size < sizeof(struct rpmb_data_frame)) {
		errorf("%s parameter package is invalid\n", __func__);
		return -1;
	}

	data_frame = package;
	msg_type = RPMB_MSG_TYPE_REQ_WRITE_COUNTER_VAL_READ;
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	u16_to_bytes(msg_type, data_frame->msg_type);
	memcpy(data_frame->nonce, nonce, RPMB_NONCE_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_IN, DMA_FROM_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	return 0;
}

/* @retrun 0 rpmb key unwritten */
int is_wr_ufs_rpmb_key(void)
{
	uint8_t result=0;
	struct rpmb_data_frame *data_frame;
	uint16_t msg_type;
	uint16_t op_result;
	uint32_t writecount;
	uint8_t nonce[RPMB_NONCE_SIZE] = {
		0xa5, 0x5a, 0xff, 0x00, 0xbe, 0xef, 0xbe, 0xef,
		0xbe, 0xef, 0xbe, 0xef, 0x00, 0xff, 0x5a, 0xa5
	};
	lbaint_t transfer_blkcnt = 0;

	if (rpmb_lu_info.log2blksz == 0) {
		result = prepare_rpmb_lu();
		if(UFS_SUCCESS != result)
			return result;
	}
	/* rpmb_lu_info.log2blksz=0x08,256B */
	transfer_blkcnt = RPMB_DATA_FRAME_SIZE >> (rpmb_lu_info.log2blksz);

	msg_type = RPMB_MSG_TYPE_REQ_WRITE_COUNTER_VAL_READ;
	data_frame = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE);
	if (!data_frame) {
		errorf("%s malloc error\n", __func__);
		return -1;
	}

	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	u16_to_bytes(msg_type, data_frame->msg_type);
	memcpy(data_frame->nonce, nonce, RPMB_NONCE_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_IN, DMA_FROM_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	/* result check */
	bytes_to_u16(data_frame->op_result, &op_result);
	bytes_to_u16(data_frame->msg_type, &msg_type);
	if (op_result == RPMB_RES_NO_AUTH_KEY) {
		dprintf(INFO,"%s rpmb key not write\n", __func__);
		result = 0;
	} else {
		errorf("%s rpmb key has been written\n", __func__);
		result = -1;
	}
	free(data_frame);

	return result;
}

uint32_t ufs_rpmb_read_writecount(void)
{
	struct rpmb_data_frame *data_frame;
	uint16_t msg_type;
	uint16_t op_result;
	uint32_t writecount;
	uint8_t nonce[RPMB_NONCE_SIZE] = {
		0xa5, 0x5a, 0xff, 0x00, 0xbe, 0xef, 0xbe, 0xef,
		0xbe, 0xef, 0xbe, 0xef, 0x00, 0xff, 0x5a,0xa5
	};
	lbaint_t transfer_blkcnt = 0;
	uint8_t ret = 0;

	if (rpmb_lu_info.log2blksz == 0) {
		ret = prepare_rpmb_lu();
		if (ret != UFS_SUCCESS) {
			debugf("prepare rpmb unit failed!\n");
			return ret;
		}
	}
	/* rpmb_lu_info.log2blksz =0x08 */
	transfer_blkcnt = RPMB_DATA_FRAME_SIZE >> (rpmb_lu_info.log2blksz);

	msg_type = RPMB_MSG_TYPE_REQ_WRITE_COUNTER_VAL_READ;
	data_frame = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE);
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	u16_to_bytes(msg_type, data_frame->msg_type);
	memcpy(data_frame->nonce, nonce, RPMB_NONCE_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_IN, DMA_FROM_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	/* result check */
	bytes_to_u16(data_frame->op_result, &op_result);
	bytes_to_u16(data_frame->msg_type, &msg_type);
	if ((op_result == RPMB_RESULT_OK) &&
	    (msg_type == RPMB_MSG_TYPE_RESP_WRITE_COUNTER_VAL_READ)) {
		bytes_to_u32(data_frame->write_counter, &writecount);
		debugf("read write count successed\n");
		free(data_frame);
		return writecount;
	}
	else
		debugf(" read write count:0x%x ,msg_type:0x%x\n",
		       op_result,msg_type);
	free(data_frame);

	return 1;
}

/*
 * blk_data: for save read data;
 * blk_index: the block index for read;
 * block_count: the read count;
 * success return 0;
 */
uint8_t ufs_rpmb_blk_read(char *blk_data, uint16_t blk_index,
			  uint8_t block_count)
{
	struct rpmb_data_frame *data_frame;
	struct rpmb_data_frame *resp_buf;
	uint16_t msg_type;
	uint16_t op_result;
	uint8_t nonce[RPMB_NONCE_SIZE] = {
		0xa5, 0x5a, 0xff, 0x00, 0xbe, 0xef, 0xbe, 0xef,
		0xbe, 0xef, 0xbe, 0xef, 0x00, 0xef, 0x5a,0xa5
	};
	lbaint_t transfer_blkcnt = 0;
	uint8_t ret = 0;

	if (rpmb_lu_info.log2blksz == 0) {
		ret = prepare_rpmb_lu();
		if (ret != UFS_SUCCESS)
		return ret;
	}
	/* rpmb_lu_info.log2blksz=0x08,256B */
	transfer_blkcnt = RPMB_DATA_FRAME_SIZE >> (rpmb_lu_info.log2blksz);

	if (!blk_data) {
		errorf("rpmb_blk_read null \n");
		return 1;
	}
	msg_type = RPMB_MSG_TYPE_REQ_AUTH_DATA_READ;
	data_frame = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE);
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	u16_to_bytes(msg_type, data_frame->msg_type);
	u16_to_bytes(blk_index, data_frame->address);
	u16_to_bytes(block_count, data_frame->block_count);
	memcpy(data_frame->nonce, nonce, RPMB_NONCE_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	resp_buf = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE * block_count);
	memset(resp_buf, 0, RPMB_DATA_FRAME_SIZE * block_count);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_IN, DMA_FROM_DEVICE,
		      &rpmb_lu_info, (void*)resp_buf, 0,
		      transfer_blkcnt*block_count, 0);

	/* result check */
	bytes_to_u16((resp_buf + block_count - 1)->op_result, &op_result);
	bytes_to_u16((resp_buf + block_count - 1)->msg_type, &msg_type);
	if ((op_result == RPMB_RESULT_OK) &&
            (msg_type == RPMB_MSG_TYPE_RESP_AUTH_DATA_READ)) {
		uint8_t i = 0;
		for (i = 0; i < block_count; i++)
			memcpy((blk_data + i * RPMB_DATA_SIZE),
			       ((uint8_t *) (resp_buf + i) +
				RPMB_STUFF_DATA_SIZE + RPMB_KEY_MAC_SIZE),
			       RPMB_DATA_SIZE);
		debugf("read  successed\n");
		free(resp_buf);
		free(data_frame);
		return 0;
	}
	else
		debugf("read write count:0x%x ,msg_type:0x%x\n",
		       op_result, msg_type);
	free(resp_buf);
	free(data_frame);

	return 1;
}

/*
 * write_data: data for write
 * blk_index: the block will be write to;
 * success return 0
 */

/* for single data */
uint8_t ufs_rpmb_blk_write(char *write_data, uint8_t *key, uint16_t blk_index)
{
	struct rpmb_data_frame *data_frame;
	uint16_t msg_type;
	uint16_t op_result;
	uint16_t block_count = 1;
	uint32_t writecount;
        lbaint_t transfer_blkcnt = 0;
	uint8_t ret = 0;

	if(rpmb_lu_info.log2blksz == 0) {
		ret = prepare_rpmb_lu();
		if (ret != UFS_SUCCESS)
			return ret;
	}
	/* rpmb_lu_info.log2blksz=0x08,256B */
	transfer_blkcnt = RPMB_DATA_FRAME_SIZE >>(rpmb_lu_info.log2blksz);

	if (!write_data)//|| (RPMB_DATA_SIZE != strlen(write_data)))
		return 1;

	/* TODO: The following codes is multiple block write
	 * for(int i=0;i<(strlen(write_data)/RPMB_DATA_SIZE),i++){
	 */

	msg_type = RPMB_MSG_TYPE_REQ_AUTH_DATA_WRITE;
	data_frame = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE);
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	writecount = ufs_rpmb_read_writecount();
	u16_to_bytes(msg_type, data_frame->msg_type);
	u16_to_bytes(blk_index, data_frame->address);
	u16_to_bytes(block_count, data_frame->block_count);
	u32_to_bytes(writecount, data_frame->write_counter);
	memcpy(data_frame->data, write_data, RPMB_DATA_SIZE);

	/* TODO: The following codes is key mac caculate.
	 * int key_len = 32;
	 * int mac_data_len = 284;
	 * data_frame->key_mac = HMAC_SHA256(key, data_frame->data, key_len,
	 *                                   mac_data_len);
	 */
#if defined(CONFIG_BORINGSSL)
	//for key_mac calc;
	HMAC(EVP_sha256(), key, 32, data_frame->data, 284, data_frame->key_mac, 32);
#endif


	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	/* for read result req */
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	msg_type = RPMB_MSG_TYPE_REQ_RESULT_READ;
	u16_to_bytes(msg_type, data_frame->msg_type);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_IN, DMA_FROM_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	/* result check */
	bytes_to_u16(data_frame->op_result, &op_result);
	bytes_to_u16(data_frame->msg_type, &msg_type);
	if ((op_result == RPMB_RESULT_OK) &&
	    (msg_type == RPMB_MSG_TYPE_RESP_AUTH_DATA_WRITE)) {
		dprintf(INFO," data  write successed\n");
		free(data_frame);
		return 0;
	} else {
		errorf(" data write fail op_result:0x%x ,msg_type:0x%x\n",
		       op_result, msg_type);
	}

	free(data_frame);

	return 1;
}

/*
 * key: the key will write must 32 len;
 * len :must be 32;
 * reutrn 0 success
 */
uint8_t ufs_rpmb_write_key(uint8_t * key, uint8_t len)
{
	struct rpmb_data_frame *data_frame = NULL;
	uint16_t msg_type;
	uint16_t op_result;
	uint8_t ret = 0;
	lbaint_t transfer_blkcnt = 0;

	if (rpmb_lu_info.log2blksz == 0) {
		ret = prepare_rpmb_lu();
		if (ret != UFS_SUCCESS)
			return ret;
	}

	/* rpmb_lu_info.log2blksz=0x08,256B */
	transfer_blkcnt = RPMB_DATA_FRAME_SIZE >> (rpmb_lu_info.log2blksz);

	if (!key || len != RPMB_KEY_MAC_SIZE)
		return 1;

	/* for write rpmb key req */
	msg_type = RPMB_MSG_TYPE_REQ_AUTH_KEY_PROGRAM;
	data_frame = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE);
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	u16_to_bytes(msg_type, data_frame->msg_type);
	memcpy(data_frame->key_mac, key, RPMB_KEY_MAC_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	/* for read result req */
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	msg_type = RPMB_MSG_TYPE_REQ_RESULT_READ;
	u16_to_bytes(msg_type, data_frame->msg_type);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_IN, DMA_FROM_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	/* result check */
	bytes_to_u16(data_frame->op_result, &op_result);
	bytes_to_u16(data_frame->msg_type, &msg_type);
	if ((op_result == RPMB_RESULT_OK) &&
	    (msg_type == RPMB_MSG_TYPE_RESP_AUTH_KEY_PROGRAM)) {
		dprintf(INFO," key write successed\n");
		free(data_frame);
		return 0;
	} else {
		errorf(" key write fail op_result:0x%x ,msg_type:0x%x\n",
		       op_result, msg_type);
	}

	free(data_frame);

	return 1;
}

int ufs_rpmb_write_pac(uint8_t *pac_data, uint16_t blk_index)
{
	struct rpmb_data_frame *data_frame;
	uint16_t msg_type;
	uint16_t op_result;
	uint16_t block_count = 1;
	uint32_t writecount;
	lbaint_t transfer_blkcnt = 0;
	int ret = 0;

	if(rpmb_lu_info.log2blksz == 0) {
		ret = prepare_rpmb_lu();
		if (ret != UFS_SUCCESS)
			return ret;
	}

	/* rpmb_lu_info.log2blksz=0x08,256B */
	transfer_blkcnt = RPMB_DATA_FRAME_SIZE >>(rpmb_lu_info.log2blksz);

	if (pac_data == NULL)
		return -1;

	data_frame = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE);
	if (data_frame == NULL) {
		errorf("erro data_frame is NULL!\n");
		return -1;
	}
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	memcpy(data_frame, pac_data, RPMB_DATA_FRAME_SIZE);

	/*for write req */
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	/*for read result req */
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	msg_type = RPMB_MSG_TYPE_REQ_RESULT_READ;
	u16_to_bytes(msg_type, data_frame->msg_type);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

      /*read result */
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_IN, DMA_FROM_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	/* result check */
	bytes_to_u16(data_frame->op_result, &op_result);
	bytes_to_u16(data_frame->msg_type, &msg_type);
	if (op_result == RPMB_RESULT_OK) {
		dprintf(INFO," rpmb package  write successed\n");
		free(data_frame);
		return 0;
	} else {
		errorf(" rpmb package write fail op_result:0x%x ,msg_type:0x%x\n",
		       op_result, msg_type);
	}


	free(data_frame);

	return -1;		//should not run here;
}

/*
 * swp_config_data: The secure Write Protect Configuration Block;
 * success return 0;
 */
int ufs_rpmb_swp_config_read(char *swp_config_data)
{
	struct rpmb_data_frame *data_frame;
	struct rpmb_data_frame *resp_buf;
	uint16_t msg_type;
	uint16_t op_result;
	uint8_t nonce[RPMB_NONCE_SIZE] = {
		0xa5, 0x5a, 0xff, 0x00, 0xbe, 0xef, 0xbe, 0xef,
		0xbe, 0xef, 0xbe, 0xef, 0x00, 0xef, 0x5a,0xa5
	};
	lbaint_t transfer_blkcnt = 0;
	int ret = 0;

	if (rpmb_lu_info.log2blksz == 0) {
		ret = prepare_rpmb_lu();
		if (ret != UFS_SUCCESS)
		return ret;
	}
	/* rpmb_lu_info.log2blksz=0x08,256B */
	transfer_blkcnt = RPMB_DATA_FRAME_SIZE >> (rpmb_lu_info.log2blksz);

	if (!swp_config_data) {
		errorf("swp_config_data is null \n");
		return -1;
	}
	msg_type = RPMB_MSG_TYPE_REQ_SWP_CFG_BLK_READ;
	data_frame = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE);
	if (data_frame == NULL)
		return -1;
	memset(data_frame, 0, RPMB_DATA_FRAME_SIZE);
	u16_to_bytes(msg_type, data_frame->msg_type);
	u16_to_bytes(0, data_frame->address);
	u16_to_bytes(1, data_frame->block_count);
	memcpy(data_frame->nonce, nonce, RPMB_NONCE_SIZE);
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_OUT, DMA_TO_DEVICE,
		      &rpmb_lu_info, (void*)data_frame, 0, transfer_blkcnt, 0);

	resp_buf = (uint8_t *)memalign(ARCH_DMA_MINALIGN, RPMB_DATA_FRAME_SIZE );
	if (resp_buf == NULL) {
		free(data_frame);
		return -1;
	}
	memset(resp_buf, 0, RPMB_DATA_FRAME_SIZE );
	send_scsi_cmd(UFS_OP_SECURITY_PROTOCOL_IN, DMA_FROM_DEVICE,
		      &rpmb_lu_info, (void*)resp_buf, 0,
		      transfer_blkcnt, 0);

	/* result check */
	bytes_to_u16(resp_buf->op_result, &op_result);
	bytes_to_u16(resp_buf->msg_type, &msg_type);
	if ((op_result == RPMB_RESULT_OK) &&
            (msg_type == RPMB_MSG_TYPE_RESP_SWP_CFG_BLK_READ)) {
			memcpy(swp_config_data,
			       ((uint8_t *) (resp_buf) + RPMB_STUFF_DATA_SIZE + RPMB_KEY_MAC_SIZE),
			       RPMB_DATA_SIZE);
		dprintf(INFO,"read Secure Write Protect Configuration Block successed\n");
		free(resp_buf);
		free(data_frame);
		return 0;
	} else {
		debugf("read Secure Write Protect Configuration Block:0x%x ,msg_type:0x%x\n", op_result, msg_type);
	}
	free(resp_buf);
	free(data_frame);

	return -1;
}

/*
 * Set authenticated secure write protect entry
 * @ptn_name: partition name
 * @offset: offset of partition which we want to set
 * @bytes: this entry ranges in bytes unit
 * success return 0
 */
int storage_write_protect_set(const char *ptn_name, uint64_t offset, uint64_t bytes)
{
	int lu_index = 0;
	block_dev_desc_t *dev_desc = NULL;
	disk_partition_t part_info;
	uint64_t start_blk, offset_blk, bytes_blk;
	uint64_t blk_addr;
	uint32_t blk_num;
	int entry_num_r = 0;
	int entry_num_w = 0;
	int ret = 0;
	int i = 0;

	struct sec_wp_cfg_blk *cfg_blk_write = NULL;
	struct sec_wp_cfg_blk *cfg_blk_read = NULL;
	uint8_t data_blk[RPMB_DATA_SIZE];

	if (bytes % LOGICAL_BLK_SIZE || offset % LOGICAL_BLK_SIZE) {
		debugf("Swp entry not aligned to 4k bytes!\n");
		return -1;
	}
	offset_blk = offset >> (ufs_info.lu_info[lu_index].log2blksz);
	bytes_blk = bytes >> (ufs_info.lu_info[lu_index].log2blksz);

	dev_desc = ufs_get_dev(lu_index);
	ret = get_partition_info_by_name(dev_desc, ptn_name, &part_info);
	if (ret) {
		debugf("Get partition fail!\n");
		return -1;
	}

	start_blk = (offset >> (ufs_info.lu_info[lu_index].log2blksz)) + part_info.start_blk;
	if (offset_blk + bytes_blk > part_info.blk_cnt) {
		debugf("secure write protect range overflow partition size!\n");
		return -1;
	}

	memset(data_blk, 0x0, RPMB_DATA_SIZE);
	data_blk[0] = lu_index;
	ret = sec_rpmb_swp_config_read(data_blk, RPMB_DATA_SIZE);
	if (ret) {
		debugf("secure write protect config block read fail!\n");
		return -1;
	}

	cfg_blk_read = (struct sec_wp_cfg_blk *)malloc(RPMB_DATA_SIZE);
	if (!cfg_blk_read) {
		debugf("cfg_blk_read malloc failed!\n");
		ret = -1;
		goto out;
	}
	memset(cfg_blk_read, 0x0, RPMB_DATA_SIZE);
	memcpy((void *)cfg_blk_read, data_blk, RPMB_DATA_SIZE);
	entry_num_r = cfg_blk_read->data_length / 16;

	cfg_blk_write = (struct sec_wp_cfg_blk *)malloc(RPMB_DATA_SIZE);
	if (!cfg_blk_write) {
		debugf("cfg_blk_write malloc failed!\n");
		ret = -1;
		goto out;
	}
	memset(cfg_blk_write, 0, RPMB_DATA_SIZE);
	if (entry_num_r == MAX_ENTRY_COUNT) {
		debugf("secure write protect max entry num is 4!\n");
		ret = -1;
		goto out;
	} else if (entry_num_r == 1 && cfg_blk_read->entry[0].wpt_wpf == 0x0){
		cfg_blk_write->entry[0].wpt_wpf = 0x1;
		u64_to_bytes(start_blk, cfg_blk_write->entry[0].blk_address);
		u32_to_bytes(bytes_blk, cfg_blk_write->entry[0].blk_num);
		entry_num_w = 1;
	} else {
		memcpy((void *)cfg_blk_write->entry, (void *)cfg_blk_read->entry,
		       RPMB_ENTRY_SIZE * entry_num_r);
		cfg_blk_write->entry[entry_num_r].wpt_wpf = 0x1;
		u64_to_bytes(start_blk, cfg_blk_write->entry[entry_num_r].blk_address);
		u32_to_bytes(bytes_blk, cfg_blk_write->entry[entry_num_r].blk_num);
		entry_num_w = entry_num_r + 1;
	}

	cfg_blk_write->lun = lu_index;
	cfg_blk_write->data_length = 16 * entry_num_w;

#if DEBUG
	debugf("lun:0x%lx\n", cfg_blk_write->lun);
	debugf("data_length:%d\n", cfg_blk_write->data_length);
	for (i = 0; i < 4; i++) {
		debugf("write:entry[%d].wpt_wpf:0x%x\n", i, cfg_blk_write->entry[i].wpt_wpf);
		bytes_to_u64(cfg_blk_write->entry[i].blk_address, &blk_addr);
		debugf("write:entry[%d].blk_address_real:0x%llx\n", i, blk_addr);
		bytes_to_u32(cfg_blk_write->entry[i].blk_num, &blk_num);
		debugf("write:entry[%d].blk_num_real:0x%llx\n", i, blk_num);
	}
#endif

	ret = sec_rpmb_swp_config_write((void *)cfg_blk_write, RPMB_DATA_SIZE);
	if (ret)
		debugf("secure write protect config block write fail!\n");
	else
		debugf("Set swp entry: partition:%s, offset:0x%llx, bytes:0x%llx  successed!\n",
		       ptn_name, offset, bytes);
out:
	if (cfg_blk_read)
		free(cfg_blk_read);
	if (cfg_blk_write)
		free(cfg_blk_write);
	return ret;
}

/*
 * Remove authenticated secure write protect entry
 * @ptn_name: partition name
 * @offset: offset of partition which we want to remove
 * @bytes: this entry ranges in bytes unit
 * success return 0
 */
int storage_write_protect_remove(const char *ptn_name, uint64_t offset, uint64_t bytes)
{
	int lu_index = 0;
	block_dev_desc_t *dev_desc = NULL;
	disk_partition_t part_info;
	lbaint_t start_blk, offset_blk, bytes_blk;
	uint64_t blk_addr;
	uint32_t blk_num;
	int entry_num_r = 0;
	int entry_num_w = 0;
	int ret = 0;
	int i = 0;

	struct sec_wp_cfg_blk *cfg_blk_write = NULL;
	struct sec_wp_cfg_blk *cfg_blk_read = NULL;
	uint8_t data_blk[RPMB_DATA_SIZE];

	if (bytes % LOGICAL_BLK_SIZE || offset % LOGICAL_BLK_SIZE) {
		debugf("Swp entry not aligned to 4k bytes!\n");
		return -1;
	}
	offset_blk = (offset >> (ufs_info.lu_info[lu_index].log2blksz));
	bytes_blk = (bytes >> (ufs_info.lu_info[lu_index].log2blksz));

	dev_desc = ufs_get_dev(lu_index);
	ret = get_partition_info_by_name(dev_desc, ptn_name, &part_info);
	if (ret) {
		debugf("Get partition fail!\n");
		return -1;
	}

	start_blk = (offset >> (ufs_info.lu_info[lu_index].log2blksz)) + part_info.start_blk;
	if (offset_blk + bytes_blk > part_info.blk_cnt) {
		debugf("secure write protect range overflow partition size!\n");
		return -1;
	}

	memset(data_blk, 0x0, RPMB_DATA_SIZE);
	data_blk[0] = lu_index;
	ret = sec_rpmb_swp_config_read(data_blk, RPMB_DATA_SIZE);
	if (ret) {
		debugf("secure write protect config block read fail!\n");
		return -1;
	}

	cfg_blk_read = (struct sec_wp_cfg_blk *)malloc(RPMB_DATA_SIZE);
	if (!cfg_blk_read) {
		debugf("cfg_blk_read malloc failed!\n");
		ret = -1;
		goto out;
	}
	memset(cfg_blk_read, 0x0, RPMB_DATA_SIZE);
	memcpy((void *)cfg_blk_read, data_blk, RPMB_DATA_SIZE);
	entry_num_r = cfg_blk_read->data_length / 16;
	if (!entry_num_r) {
		debugf("There is no secure write protect entry!\n");
		ret = -1;
		goto out;
	}

	cfg_blk_write = (struct sec_wp_cfg_blk *)malloc(RPMB_DATA_SIZE);
	if (!cfg_blk_write) {
		debugf("cfg_blk_write malloc failed!\n");
		ret = -1;
		goto out;
	}
	memset(cfg_blk_write, 0, RPMB_DATA_SIZE);

	for (i = 0; i < entry_num_r; i++) {
		bytes_to_u64(cfg_blk_read->entry[i].blk_address, &blk_addr);
		bytes_to_u32(cfg_blk_read->entry[i].blk_num, &blk_num);
		/* remove entry  equal or larger than entry region */
		if (start_blk <= blk_addr && (start_blk + bytes_blk >= blk_addr + blk_num)) {
			continue;
		} else {
			memcpy((void *)&cfg_blk_write->entry[entry_num_w],
			       (void *)&cfg_blk_read->entry[i], RPMB_ENTRY_SIZE);
			entry_num_w++;
		}
	}

	if (entry_num_w == entry_num_r) {
		debugf("This region is not secure write protect entry!\n");
		ret = -1;
		goto out;
	}

	if (entry_num_w == 0) {
		debugf("data_length change to 0x1!\n");
		entry_num_w++;
	}

	cfg_blk_write->lun = lu_index;
	cfg_blk_write->data_length = 16 * entry_num_w;

#if DEBUG
	debugf("lun:0x%lx\n", cfg_blk_write->lun);
	debugf("data_length:%d\n", cfg_blk_write->data_length);
	for (i = 0; i < 4; i++) {
		debugf("write:entry[%d].wpt_wpf:0x%x\n", i, cfg_blk_write->entry[i].wpt_wpf);
		bytes_to_u64(cfg_blk_write->entry[i].blk_address, &blk_addr);
		debugf("write:entry[%d].blk_address_real:0x%llx\n", i, blk_addr);
		bytes_to_u32(cfg_blk_write->entry[i].blk_num, &blk_num);
		debugf("write:entry[%d].blk_num_real:0x%llx\n", i, blk_num);
	}
#endif
	ret = sec_rpmb_swp_config_write((void *)cfg_blk_write, RPMB_DATA_SIZE);
	if (ret)
		debugf("secure write protect config block remove fail!\n");
	else
		debugf("remove swp entry: partition:%s, offset:0x%llx, bytes:0x%llx  successed!\n",
		       ptn_name, offset, bytes);
out:
	if (cfg_blk_read)
		free(cfg_blk_read);
	if (cfg_blk_write)
		free(cfg_blk_write);
	return ret;
}

/*
 * Check authenticated secure write protect entry
 * @ptn_name: partition name
 * @offset: offset of partition which we want to check
 * @bytes: this entry ranges in bytes unit
 * enable return 0
 */
int storage_write_protect_check(const char *ptn_name, uint64_t offset, uint64_t bytes)
{
	int lu_index = 0;
	block_dev_desc_t *dev_desc = NULL;
	disk_partition_t part_info;
	lbaint_t start_blk, offset_blk, bytes_blk;
	uint64_t blk_addr;
	uint32_t blk_num;
	int entry_num = 0;
	int ret = 1;
	int i = 0;

	struct sec_wp_cfg_blk *cfg_blk_read = NULL;
	uint8_t data_blk[RPMB_DATA_SIZE];

	if (bytes % LOGICAL_BLK_SIZE || offset % LOGICAL_BLK_SIZE) {
		debugf("Swp entry not aligned to 4k bytes!\n");
		return -1;
	}
	offset_blk = (offset >> (ufs_info.lu_info[lu_index].log2blksz));
	bytes_blk = (bytes >> (ufs_info.lu_info[lu_index].log2blksz));

	dev_desc = ufs_get_dev(lu_index);
	ret = get_partition_info_by_name(dev_desc, ptn_name, &part_info);
	if (ret) {
		debugf("Get partition fail!\n");
		return -1;
	}

	start_blk = (offset >> (ufs_info.lu_info[lu_index].log2blksz)) + part_info.start_blk;
	if (offset_blk + bytes_blk > part_info.blk_cnt) {
		debugf("secure write protect range overflow partition size!\n");
		return -1;
	}

	memset(data_blk, 0x0, RPMB_DATA_SIZE);
	data_blk[0] = lu_index;
	ret = sec_rpmb_swp_config_read(data_blk, RPMB_DATA_SIZE);
	if (ret) {
		debugf("secure write protect config block read fail!\n");
		return -1;
	}

	cfg_blk_read = (struct sec_wp_cfg_blk *)malloc(RPMB_DATA_SIZE);
	if (!cfg_blk_read) {
		debugf("cfg_blk_read malloc failed!\n");
		return -1;
	}
	memset(cfg_blk_read, 0x0, RPMB_DATA_SIZE);
	memcpy((void *)cfg_blk_read, data_blk, RPMB_DATA_SIZE);
	entry_num = cfg_blk_read->data_length / 16;
#if DEBUG
	debugf("lun:0x%lx\n", cfg_blk_read->lun);
	debugf("data_length:%d\n", cfg_blk_read->data_length);
	for (i = 0; i < 4; i++) {
		debugf("write:entry[%d].wpt_wpf:0x%x\n", i, cfg_blk_read->entry[i].wpt_wpf);
		bytes_to_u64(cfg_blk_read->entry[i].blk_address, &blk_addr);
		debugf("write:entry[%d].blk_address_real:0x%llx\n", i, blk_addr);
		bytes_to_u32(cfg_blk_read->entry[i].blk_num, &blk_num);
		debugf("write:entry[%d].blk_num_real:0x%llx\n", i, blk_num);
	}
#endif
	if (!entry_num) {
		debugf("Partition:%s, offset:0x%llx, bytes:0x%llx is not swp entry!\n",
		       ptn_name, offset, bytes);
		free(cfg_blk_read);
		return 1;
	}

	for (i = 0; i < entry_num; i++) {
		bytes_to_u64(cfg_blk_read->entry[i].blk_address, &blk_addr);
		bytes_to_u32(cfg_blk_read->entry[i].blk_num, &blk_num);

		/* check region equal or smaller than entry region */
		if ((start_blk >= blk_addr && (start_blk + bytes_blk <= blk_addr + blk_num))
			&& (cfg_blk_read->entry[i].wpt_wpf != 0x0)) {
			debugf("Partition:%s, offset:0x%llx, bytes:0x%llx is swp entry!\n",
			       ptn_name, offset, bytes);
			free(cfg_blk_read);
			return 0;
		}
	}

	debugf("Partition:%s, offset:0x%llx, bytes:0x%llx is not swp entry!\n",
	       ptn_name, offset, bytes);
	free(cfg_blk_read);
	return 1;
}
