			   ...
			   struct pin_info *pinctr_info, int pins_num, u32 base_addr)
{
	...
	for (i = 0; i < pins_num; i++) {
		...
		if (ret == 0)
			writel(dev_info->val, pinctr_info[i].offset + base_addr);
	}
			   ...
			   struct pin_info *pinctr_info, int pins_num, u32 base_addr)
{
	...
	for (i = 0; i < pins_num; i++) {
		ret = strcmp(pinctr_info[i].name, dev_info->pins);
		if (ret == 0) {
			val = readl(pinctr_info[i].offset + base_addr);
			...
			writel(val, pinctr_info[i].offset + base_addr);
		}
	}
				 ...
				 struct pin_info *pinctr_info, int pins_num, u32 base_addr)
{
	...
	if (pin_nodeoffset < 0)
		...
		if (pinctrl.type < 0) {
			...
			else if (pinctrl.type == COMMON_REG) {
				pinctrl.pins = fdt_getprop(fdt, pin_nodeoffset, "sprd,pins", NULL);
				if (!pinctrl.pins) {
					errorf("fdt_find_pinctrl_node: cann't find sprd,pins property\n");
					return 0;
