...
//#define EFUSE_DEBUG
#ifdef EFUSE_DEBUG
#define UBOOT_EFUSE_LOG(format, arg...) dprintf(INFO, format, ## arg)
#else
...
int efuse_rollback_version_write(int type,int value)
{
	...
	if(type)//no_sec_rollback
	{
		...
		dprintf(INFO,"efuse_unsec_prog ret =%x,version=%x\n",ret,version);
		value-=31;
		write_block--;
		}
	...
	else
	{
		...
		dprintf(INFO,"efuse_sec_prog ret=%x,version=%x\n",ret,value);
	}
        return ret;
}
...
int secure_efuse_program_sprd(void)
{
	...
	dprintf(INFO,"###### secure_efuse_program_sprd ######\n");
	/**read spl image**/
	//Emmc_Read(PARTITION_BOOT1, 0, SPL_EMMC_SECTOR_NUM, (unsigned char *)VERIFY_BASE);
	if (common_raw_read("splloader",SPL_IMAGE_SIZE, 0, (unsigned char *)VERIFY_BASE)) {
		UBOOT_EFUSE_LOG("%s()->Line:%d; common_raw_read fail\n", __FUNCTION__, __LINE__);
		return -1;
	}
	sprd_get_pubk(VERIFY_BASE,sprd_pubkey, 0);
	...
	//g_imgToVerInfo.img_addr = VERIFY_BASE;
