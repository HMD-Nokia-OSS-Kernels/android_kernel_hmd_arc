...
#if defined(PLATFORM_QOGIRN6PRO)
...
unsigned int sprd_get_version(sw_version_id_t counterId, unsigned int* swVersion)
{
    ...
    case SW_VERSION_COUNTER0:
        return sec_efuse_read(SEC_VERSION0_BLOCK, swVersion);
#endif
    case SW_VERSION_COUNTER1:
...
#if defined(PLATFORM_QOGIRN6L)
    case SW_VERSION_COUNTER0:
        return sec_efuse_write_and_check(SEC_VERSION0_BLOCK, swVersion);
#endif
    case SW_VERSION_COUNTER1:
...
unsigned int is_secure_boot_enable(void)
{
...
unsigned int sprd_check_gid_kce(void)
{
	if (is_kce_gid_write())
		return 0;//kce is locked
	else
		return 1;//kce is not lock
}

unsigned int sprd_write_gid_kce(unsigned int* WRdata)
{
    unsigned int ret = SEC_EFUSE_OK;
    unsigned int block_id;
    unsigned int* write_ptr = NULL;
    ...
    for (block_id = KCE_BLOCK_START; block_id <= KCE_BLOCK_START + 3; block_id++)
    {
        ret = sprd_ce_efuse_program(block_id, *write_ptr);
        if (ret)
            return ret;
