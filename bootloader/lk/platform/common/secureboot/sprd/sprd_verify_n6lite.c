...
#define RSA_ALGS                1      //QOGIRN6L use rsa algorithm
#define ECC_ALGS                0      //QOGIRN6L use ECC algorithm
...
{
	int ret = 0;
	static uint32_t ver_slota = 0;
	static uint32_t ver_slotb = 0;

	secf("image type = %d\n", certtype);
	if (certtype == TRUSTED_FIRMWARE) {
		if (Trusted_rollback_version == SEC_VERSION_INIT_VALUE){
			sprd_get_version(SW_VERSION_COUNTER1, &ver_slota);
			sprd_get_version(SW_VERSION_COUNTER0, &ver_slotb);
			secf("read efuse version slota: 0X%x, version slotb: 0X%x\n",
				 ver_slota, ver_slotb);
		}
		Trusted_rollback_version = ver_slota > ver_slotb ? ver_slota : ver_slotb;
 ...
 void sprd_get_hash_key(uint8_t * load_buf,uint8_t *hash_key)
{
	...
	if (load_buf == IRAM_BEGIN) {
		...
		if (sec_memcmp(hash_key, (uint8_t *)imghdr->magic - HASH_BYTE_LEN, HASH_BYTE_LEN)) {
			...
			secf("next hash key different, pls check!\n");
			while(1);
		} else {
...
void sprd_dl_vboot_verify(uint8_t *name,uint8_t *tocheck_name,uint8_t * header,uint8_t *code)
{
	...
	if (name) {
		if (strcmp("fdl2",name) == 0) {
			...
			if (vb_ret != 0) {
				errorf("[%s]:check image failed. vb_ret = %d\n", __func__, vb_ret);
				while(1);
			}
		}
	}
...
void sprd_dl_verify(uint8_t *name,uint8_t * header,uint8_t *code)
{
	...
	if (code && (strcmp("trustos", code) == 0)) {
		...
		if (!sprd_kce_aes_decrypt(&header, &payload)) {
			secf("sprd_dl_verify: aes decrypt error !!\n");
			while(1);
		}
	}
#endif
