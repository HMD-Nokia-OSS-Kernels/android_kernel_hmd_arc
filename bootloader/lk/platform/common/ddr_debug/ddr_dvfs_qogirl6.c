...
#define MEM_DDR_RETENTION_AND_DFS	(0x65014800)
#define DRAM_INFO_ADDR			(MEM_DDR_RETENTION_AND_DFS + 0xc)
#define IRAM_DFS_DEBUG_ADDR			(MEM_DDR_RETENTION_AND_DFS + 0x100)
#define IRAM_DFS_SWITCH				(IRAM_DFS_DEBUG_ADDR + 0x4)
#define IRAM_DFS_HW_DIS_FREQ		(IRAM_DFS_DEBUG_ADDR + 0x8)
...
static DDRC_FREQ_INFO_T ddrc_freq_info_lp4[]=
{	//ddr_clk ,freq_sel,ratio,clk_mode,ratio_d2,harf_freq_mode, vol
	{256,	0,	0x44,	0x0,	0x9,	0x1, 750},
	{384,	1,	0x45,	0x0,	0x9,	0x1, 750},
	{512,	2,	0x44,	0x1,	0x4,	0x1, 750},
	{768,	3,	0x43,	0x1,	0x0,	0x0, 750},
	{1024,	4,	0x44,	0x1,	0x0,	0x0, 750},
	{1333,	5,	0x08,	0x1,	0x0,	0x0, 800},
	{1536,	6,	0x45,	0x1,	0x0,	0x0, 860},// 860 fix sharkl6 ab chip
	{1866,	7,	0x08,	0x1,	0x0,	0x0, 900},
};
...
{
	writel(0xffff, REG_PMU_APB_PUB_AUTO_LIGHT_SLEEP_ENABLE);
	reg_bit_set(REG_PMU_APB_DDR_SLEEP_CTRL, 0, 1, 0);
}

static u32 ddr_lightsleep_shut(void)
{
	reg_bit_set(REG_PMU_APB_DDR_SLEEP_CTRL, 0, 1, 1);
	writel(0, REG_PMU_APB_PUB_AUTO_LIGHT_SLEEP_ENABLE);

	while(readl(REG_PMU_APB_DDR_SLP_STATUS)!=0);
...
{
	g_cur_fn = (readl((SPRD_PUB_RF_DDR_CTRL_BASE+0x12c))>>8)& 0x7;
...
{
	writel(((retention_dis << 1) | dvfs_dis), IRAM_DFS_SWITCH);
