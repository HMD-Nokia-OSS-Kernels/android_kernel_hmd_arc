...
struct hwfeature hwf;
extern int fdt_chosen_bootargs_append(void *fdt, const char *append_args, int force);
...
#else
	phwf->format_buf[strlen(phwf->format_buf)] = '\n';
	memcpy(hwf.hwfeature_bootconfig+hwf.hwfeature_bootconfig_offset, phwf->format_buf, strlen(phwf->format_buf));
	hwf.hwfeature_bootconfig_offset += strlen(phwf->format_buf);
#endif
    }

    // Append to dts file
    if (phwf->fdt_generate(phwf))
        errorf("hwfeature: Unable to add node to DT\n");
...
static int hwfeature_fdt_header(struct hwfeature *phwf)
{
    ...
    if (root_off < 0) {
        errorf("hwfeature: No / node in DT\n");
        return -1;
    }
...
static int hwfeature_fdt_mknodes(struct hwfeature *phwf, char *path, int *dt_node_off)
{
    ...
    while (path) {
        ...
        if (node_off < 0) {
            ...
            if (node_off < 0) {
                errorf("hwfeature: Unable to set node to upper: %d\n", node_off);
                return -1;
            }
        }
...
static int hwfeature_fdt_generate(struct hwfeature *phwf)
{
    ...
    errorf("Unable to set prop node: %s,%s\n", prop_key, prop_value);
    return -1;
}
...
static void hwfeature_init(struct hwfeature *phwf, void *fdt)
{
    ...
    if (phwf->hwf_off < 0) {
        errorf("hwfeature: fdt: Unable to add header to DT: %d\n", phwf->hwf_off);
    }
}
...
struct hwfeature hwf = {
    .param = CONFIG_BOOTLOADER_HWFEATURE,
...
char *bootconfig_get_hwfeature(void)
{
    return hwf.hwfeature_bootconfig;
}
...
int fdt_fixup_soc_module(void *fdt)
{
	...
	nodeoffset = fdt_path_offset(fdt, "/");
	if (nodeoffset < 0) {
		debugf("%s: Failed to find nodeoffset : %s\n", __func__, fdt_strerror(nodeoffset));
		return 0;
	}
	...
	if (strcmp((const char *) soc_module, "UMS512") == 0) {
		if (get_soc_bonding(&hwf) == 0) {
			...
			if (err < 0) {
				errorf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
				return err;
			}
		} else {
		...
		unsigned int cpu = get_soc_bonding(&hwf);
		char cpu_version[16];

		memset(cpu_version, '\0', sizeof(cpu_version));

		switch (cpu) {
		case 0 :
			strcpy(cpu_version, "T760");
			break;
		case 1 :
			strcpy(cpu_version, "T770");
			break;
		case 2 :
			strcpy(cpu_version, "T820");
			break;
		case 3 :
			strcpy(cpu_version, "UTS6560");
			break;
		case 4 :
			strcpy(cpu_version, "UTS6560S");
			break;
		default:
			strcpy(cpu_version, "T820");
			break;
		...
		err = fdt_setprop(fdt, nodeoffset, "soc-module", cpu_version, strlen(cpu_version) + 1);
		if (err < 0) {
			errorf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
			return err;
		}

	} else if (strcmp((const char *) soc_module, "UMS9621") == 0) {
		if (get_soc_bonding(&hwf) == 1) {
			const char t750[] = "T750";
			err = fdt_setprop(fdt, nodeoffset, "soc-module", t750, strlen(t750) + 1);
			if (err < 0) {
				errorf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
				return err;
			}
		} else {
			const char t765[] = "T765";
			err = fdt_setprop(fdt, nodeoffset, "soc-module", t765, strlen(t765) + 1);
			if (err < 0) {
				errorf("ERROR: could not setprop soc-module %s.\n",fdt_strerror(err));
				return err;
			}
...
char *bootconfig_get_hwfeature(void) {return 0;}
void hwfeature_hook_late_initcall(void (*late_initcall)(struct hwfeature *phwf, void *fdt)) { };
#undef HW_GET_HOOK_MODE
#define HW_GET_HOOK_MODE(mode) \
    void hwfeature_hook_get_##mode(long (*get)(struct hwfeature *phwf)) { }
