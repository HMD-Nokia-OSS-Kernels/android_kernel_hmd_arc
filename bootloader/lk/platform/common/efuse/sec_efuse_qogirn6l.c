...
int is_kce_gid_write(void)
{
	...
	for (i = KCE_BLOCK_START; i <= KCE_BLOCK_START + 3; i++)
	{
		block_id = pmap[i];
		ret = ret & (is_bit_set(bitmap, (block_id << 1)));
	}
...
Efuse_Result_Ret sprd_ce_efuse_program(unsigned int block_id, unsigned int WriteData)
{
	...
	if (block_id == CYCLE_STATE_BLOCK || block_id == SEC_VERSION_BLOCK || block_id == SEC_VERSION0_BLOCK)
	{
		real_block = pmap[block_id];
