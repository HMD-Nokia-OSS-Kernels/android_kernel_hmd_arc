...
typedef struct{
	...
	uint64_t kce_decrypt_flag; //1 :need decrypt; 0: don't need
}VbootVerifyInfo;
...
typedef enum {
	SECBOOT_DISPLAY_ENABLEED = 0,
	SECBOOT_DISPLAY_DISABLED
} secboot_display_info;

extern unsigned g_dmverity_flag;//for save dm-verity error
extern unsigned g_dmverity_corrupt_flag;//for save dm-verity corrupt

typedef struct {
	uint32_t error_flag;	//0x03: error; other: no error
	uint8_t hashdata[32];	// used to store os's digest (libavb)
} dmverity_info;

typedef enum {
	DMVERITY_IO_RESULT_OK = 0,
	DMVERITY_IO_RESULT_ERROR_IO,
	DMVERITY_IO_RESULT_ERROR_HASHTREE_CORRUPTION,
} DmVerityIOResult;
...
#define EFUSE_VERSION_32_BIT 0
#define EFUSE_VERSION_64_BIT 1
#define SPL_SINGLE_SLOT 0
#define SPL_DOUBLE_SLOT 1
#define SLOT_A_EFUSE_VERSION 1
#define SLOT_B_EFUSE_VERSION 0

typedef struct{
	uint32_t swVersion[2];
	uint32_t version_type; //0: 32 bits; 1: 64 bits
	uint32_t ab_slot_flag; //0: single slot; 1:spl double slot
}secure_version_info;
...
#elif defined (PLATFORM_SHARKL5) || defined (PLATFORM_SHARKL5PRO)
#define IRAM_BEGIN   0x5500
#elif defined (PLATFORM_PIKE2)
#define IRAM_BEGIN 0x50002e00
#elif defined (PLATFORM_QOGIRL6) || defined (PLATFORM_QOGIRN6PRO) || defined (PLATFORM_QOGIRN6L)
...
VERIFY_RESULT dl_secure_process_flow(unsigned long * strip, unsigned char * part_name,
				uint32_t rcv_size, uint32_t total_size, unsigned char * buf);

VERIFY_RESULT fb_secure_process_flow(unsigned long * strip, unsigned char * part_name,
			    uint32_t rcv_size, uint32_t total_size, unsigned char * buf);
