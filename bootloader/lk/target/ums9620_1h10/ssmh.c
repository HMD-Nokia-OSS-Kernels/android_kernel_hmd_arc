...
#define MAX_LEVEL_NUM 8
#define ERROR_ADC_LEVEL -1

enum BOOT_HARDWARE_MODE {
	NOT_SIM_ONLY_MODE = 0,
	NOT_DATA_ONLY_MODE = 0,
	NOT_WIFI_ONLY_MODE = 0,
	SIM_ONLY_MODE = 1,
	DATA_ONLY_MODE = 1,
	WIFI_ONLY_MODE = 1,
};
...
enum ADC_VALUE_MIN_MAP {
	...
	ADC_VALUE_MIN_5 = 731,
	ADC_VALUE_MIN_6 = 883,
	ADC_VALUE_MIN_7 = 1054,
};

enum ADC_VALUE_MAX_MAP {
	ADC_VALUE_MAX_0 = 31,
	ADC_VALUE_MAX_1 = 220,
	ADC_VALUE_MAX_2 = 325,
	ADC_VALUE_MAX_3 = 514,
...
static const struct adc2level adc2level_table[MAX_LEVEL_NUM] ={
	{ADC_VALUE_MIN_0,ADC_VALUE_MAX_0,VOL_LEVEL_0},
	{ADC_VALUE_MIN_1,ADC_VALUE_MAX_1,VOL_LEVEL_1},
	{ADC_VALUE_MIN_2,ADC_VALUE_MAX_2,VOL_LEVEL_2},
	{ADC_VALUE_MIN_3,ADC_VALUE_MAX_3,VOL_LEVEL_3},
	{ADC_VALUE_MIN_4,ADC_VALUE_MAX_4,VOL_LEVEL_4},
	{ADC_VALUE_MIN_5,ADC_VALUE_MAX_5,VOL_LEVEL_5},
	{ADC_VALUE_MIN_6,ADC_VALUE_MAX_6,VOL_LEVEL_6},
	{ADC_VALUE_MIN_7,ADC_VALUE_MAX_7,VOL_LEVEL_7}
};
...
static int get_adc_value_to_level(int vol)
{
	int i = 0;
	while (i < MAX_LEVEL_NUM)
	{
		if ((vol >= adc2level_table[i].min_vol) && (vol <= adc2level_table[i].max_vol))
		{
			debugf("level = %d\n",adc2level_table[i].level);
			return adc2level_table[i].level;
		}
		i++;
	}

	errorf("vol is out of reasonable ranger \n");
	return ERROR_ADC_LEVEL;
}
...
static int get_adc_value_board(int channel)
{
	...
	level = get_adc_value_to_level(vol);
...
/*modified according to the actual hardware*/
	...
	if (boardid == 0) {
		return WIFI_ONLY_MODE;
	} else {
		return NOT_WIFI_ONLY_MODE;
	}
#else
/* fixed to Wifionly */
...
/*modified according to the actual hardware*/
	...
	if (boardid == 1) {
		return DATA_ONLY_MODE;
	} else {
		return NOT_DATA_ONLY_MODE;
	}
#else
/* fixed to dataonly */
...
int sprd_get_sim(void)
{
	...
	if (boardid == 3) {
		return SIM_ONLY_MODE;
	} else {
		return NOT_SIM_ONLY_MODE;
	}
#else
/* fixed to SINGLESIM */
