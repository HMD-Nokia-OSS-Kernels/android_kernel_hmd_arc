...
extern int sprd_get_hr_version(void);
int hr_state = -1;
...
void umb9230s_set_isppll(void)
{
	writel(BIT_PMU_APB_ISPPLL_AP_SEL,
			REG_PMU_APB_ISPPLL_REL_CFG + 0x1000);
}

void umb9230s_set_xbuf_pd(void)
{
	sprd_gpio_request(172);
	sprd_gpio_direction_output(172, 1);
}
...
static void battery_init(void)
{
	...
	#elif defined(CONFIG_CHARGER_UPM6920_SUPPORT)
	sprdchg_upm6920_init();
	#elif defined(CONFIG_CHARGER_SC8960X_SUPPORT)
	sprdchg_sc8960x_init();
	#endif	
	sprdbat_init();
...
void target_init(void) {
	...
	/*FPGA forbiden*/
	sprd_gpio_init();
	hr_state = sprd_get_hr_version();
	if (hr_state == 1){
		umb9230s_pin_init();
		umb9230s_set_isppll();
		umb9230s_set_xbuf_pd();
	}
	misc_init();
	sprd_eic_init();
	sprd_led_init();
