...
extern int sprd_get_crystal(void);
extern int sprd_get_wcn_crystal(void);
...
void pmic_lowpower_config(uint32_t ddie_id,uint32_t adie_id)
{
	...
	ANA_REG_SET(REG_ANA_UMP9621_SLP_DCDC_PD_CTRL,
		    ...
		    BIT_ANA_UMP9621_SLP_DCDCAI_PD_EN |     //DCDCAI(VDDSRAM_CPU_BIG2)
	...
	ANA_REG_SET(ANA_REG_GLB_SLP_LDO_LP_CTRL1,
		    ...
		    //BIT_SLP_LDO_VDDLDO1_LP_EN |     //not used
		    0);
	...
	ANA_REG_SET(ANA_REG_GLB_DCDC_XTL_EN5,
		    ...
		    BIT_DCDC_MODEM_EXT_XTL1_EN |     //DCDCMODEM(VDDMM)
	...
	ANA_REG_SET(ANA_REG_GLB_DCDC_XTL_EN6,
		    ...
		    //BIT_DCDC_MM_EXT_XTL2_EN |     //DCDCMM(VDDMODEM)
...
void change_cdac_from_miscdata(void)
{
#ifdef  CONFIG_ADIE_UMP9622
        char buf_p[32] = {0};
        unsigned int cdac = 99, min = 0, max = 255, ret = 0;

	if (common_raw_read("miscdata", (uint64_t)TSX_CALI_DATA_LEN,
				TSX_CALI_DATA_OFFSET, buf_p)){
		errorf("read tsx_cdac data error from miscdata...\n");
	} else {
		ret = hexval(buf_p[0]);
		dprintf(INFO,"tsx_cdac assigned by miscdata is:0x%x\n", ret);
		if(ret > min && ret <= max)
			cdac = ret;
	}

	debugf("cdac=%d\n", cdac);
	ANA_REG_SET(REG_ANA_UMP9622_TSX_CTRL10, cdac);
#endif
}
