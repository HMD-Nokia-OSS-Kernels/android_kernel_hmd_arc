...
/* sub to sp uart */
#define USB2SPUART_CFG_VALUE	(0x58A90C84)
#define USB2SPUART_CFG1_VALUE	(0x000016E2)
#define PIN_UART_CLR	(~BIT(2))
...
static void sprd_sp_uart_inf_sel(void)
{
	CHIP_REG_SET(CTL_PIN_BASE + REG_PIN_UART_MATRIX_MTX_CFG, USB2SPUART_CFG_VALUE);
	CHIP_REG_SET(CTL_PIN_BASE + REG_PIN_UART_MATRIX_MTX_CFG1, USB2SPUART_CFG1_VALUE);
}
...
void usb_mux_sp_uart_config_enable(void)
{
	debugf("usb switch to sp uart!\n");
	/* switch uart function */
	CHIP_REG_SET(REG_AON_APB_USB_UART_JTAG_MUX, PIN_UART_SEL);
	sprd_usb_power_init();
	sprd_sp_uart_inf_sel();
}
...
/* load part to init usb2spuart */
#ifdef CONFIG_USB2SPUART
int sprd_usb2spuart_config(void)
{

	/* read 5 bytes from MISCDATA_UBOOT_BASE + 896 + 10*/
	char buf[10];
	int ret = 0;
	int error = -1;
	/* clear buf */
	...
	/* read enable/disable of tring from miscdata, sizeof(disable) is 7  */
	if (common_raw_read("miscdata", (uint64_t)USB2SPUART_LENGTH_DISABLE,
			    (uint64_t)USBMUX_DATA_OFFSET, buf)){
		errorf("read USB2SPUART data error from miscdata...\n");
		return error;
	}
	else {
		/*get data successfully */
		if (!strncmp(buf,"enable", strlen("enable"))) {
			debugf("turn on the confition of usb2spuart by miscdata\n");
			usb_mux_sp_uart_config_enable();
		} else if (!strncmp(buf,"disable", strlen("disable"))) {
			debugf("turn off the confition of usb2spuart by miscdata\n");
