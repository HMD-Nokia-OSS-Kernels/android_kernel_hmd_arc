...
/* set uart inf8 as ap_uart1 and inf1 as auddsp_uart0  */
#define PIN_UART_MATRIX_MTX_CFG_VALUE	(0x5B97088A)
#define PIN_UART_MATRIX_MTX_CFG1_VALUE	(0x00000016)

#define REG_PIN_CTRL_REG2_FUNC_CFG	(0x642E0034)
#define REG_CS_DBG_APB_DBG_CTRL		(0x7800A00C)
#define PIN_UART_SEL	BIT(31)
#define PIN_JTAG_SEL	(BIT(31) | BIT(30) | BIT(29))
#define PIN_JTAG_APWDG_SEL	BIT(26)

static void sprd_usb_power_init(void)
{
	CHIP_REG_AND(REG_AON_APB_MIPI_CSI_POWER_CTRL,
			~(BIT_AON_APB_C2G_ANALOG_USB20_USB20_PS_PD_L | BIT_AON_APB_C2G_ANALOG_USB20_USB20_PS_PD_S));
	CHIP_REG_AND(REG_AON_APB_AON_SOC_USB_CTRL, ~BIT_AON_APB_USB20_ISO_SW_EN);
}

static void sprd_uart_inf_sel(void)
{
	CHIP_REG_SET(REG_PIN_UART_MATRIX_MTX_CFG, PIN_UART_MATRIX_MTX_CFG_VALUE);
	CHIP_REG_SET(REG_PIN_UART_MATRIX_MTX_CFG1, PIN_UART_MATRIX_MTX_CFG1_VALUE);
}
...
{
	if(CHIP_REG_GET(REG_PIN_CTRL_REG2_FUNC_CFG))
		return 1;
	else
		return 0;
}
...
{
	if (CHIP_REG_GET(REG_PIN_CTRL_REG2_FUNC_CFG) == PIN_UART_SEL)
		sprd_uart_inf_sel();
}

/* usb switch to uart/jtag by keys detection  */
void usb_uart_key_config(void)
{
	if (check_keys_mode() == 1) {
		CHIP_REG_SET(REG_PIN_CTRL_REG2_FUNC_CFG, PIN_UART_SEL);
		sprd_usb_power_init();
		sprd_uart_inf_sel();
...
void usb_jtag_key_config(void)
{
	if (check_keys_mode() == 1) {
		...
		CHIP_REG_SET(REG_PIN_CTRL_REG2_FUNC_CFG, PIN_JTAG_SEL);
		sprd_usb_power_init();
	}
	else {
		dprintf(INFO, "keys_detection: keep default mode!\n");
...
/* jtag_apwdg function is not used, need to be fixed */
void usb_mux_jtag_apwdg_config(void)
{
	CHIP_REG_SET(REG_CS_DBG_APB_DBG_CTRL, PIN_JTAG_APWDG_SEL);
	sprd_usb_power_init();
