...
define MAKECONFIGHEADER
	...
	$(eval _var := ) \
	$(eval _uncompleted := false) \
	$(foreach var,$($(2)), \
		$(eval _keep := true ) \
		$(eval _var += $(var)) \
		$(if $(findstring =",$(_var)), \
			$(if $(findstring ",$(subst =",$(SPACE),$(_var))), \
				$(if $(filter $(_uncompleted),true), \
					$(eval _uncompleted := false) \
					echo \#define '$(strip $(subst =,$(SPACE),$(_var)))' >> $1.tmp; \
					$(eval _var := ) \
					$(eval _keep := false) \
					, \
					$(eval _keep := true) \
				...
				$(eval _keep := false) \
				$(eval _uncompleted := true) \
			) \
		) \
		$(if $(filter $(_keep),true), \
			echo \#define \
