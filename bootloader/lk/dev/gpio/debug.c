...
static int cmd_gpio(int argc, const cmd_args *argv) {
    if (argc == 4 && !strcmp(argv[1].str,"set")) {
        ...
        dprintf(INFO,"gpio set <gpio #> <value>\n");
        dprintf(INFO,"gpio get <gpio #>\n");
        dprintf(INFO,"gpio cfg <gpio #> [flags: ");
        for (unsigned int i = 0; i < countof(gpio_flag_labels); i++) {
            dprintf(INFO,"%s ", gpio_flag_labels[i].label);
        }
        putchar(']');
        putchar('\n');
    }
