...
#define EXCEPTION_NAME_MAX 30
#define EXTEND_STRING "extend"

#define MINI_STRING "mini"

#define TMP_SIZE (4 * 1024)
#define COMPRESSED_LEN_MALLOC_MAX   (SECTION_NAME_MAX * MINIDUMP_MEM_MAX + TMP_SIZE)
#define COMPRESSED_LEN_NAME	"compressed_info"

#ifdef SPRD_SYSDUMP_TEST
#define NUM_TEST	1
#endif
...
#define SP_IRAM_LK		"sp_iram"

#define MINIDUMP_MEM_INFO "mini_mem_info"	/*struct sysdump_mem minidump_mem*/
...
struct regs_info_native{
        int arch;       /* 32bit reg unsigned int and 64bit unsigned long, we need use type to mark*/
        int num;        /* the num of regs will save memery amount */
        unsigned long long vaddr;    /* struct pt_regs vaddr */
        unsigned long long paddr;    /* struct pt_regs paddr */
        int size;       /* sizeof(struct pt_regs)  */
        int size_comp;  /* size after compressed  */
};

struct regs_memory_info_native{
        unsigned long long reg_vaddr[REGS_NUM_MAX - 1];      /* if vaddr invalid set it as 0*/
        unsigned long long reg_paddr[REGS_NUM_MAX - 1];      /* if paddr invalid set it as 0*/
        int per_reg_memory_size;                /* memory size amount reg */
        int per_mem_size_comp[REGS_NUM_MAX - 1];                        /* size after compressed  */
        int valid_reg_num;                      /* maybe some regs value not a valid addr */
        int size;                               /* memory size amount reg */
};
struct section_info_native{
        char section_name[SECTION_NAME_MAX];
        /*Get teh value in kernel use to record elfhdr info in uboot*/
        unsigned long long section_start_vaddr;
        unsigned long long section_end_vaddr;
        /*Get the value in kernel by __pa  use to get memory contents in uboot */
        unsigned long long section_start_paddr;
        unsigned long long section_end_paddr;
        int section_size;
        int section_size_comp;                  /* size after compressed  */
};

struct section_info_total_native{
        struct section_info_native section_info_native[SECTION_NUM_MAX];
        int total_size;
        int total_num;
};

/* the new struct to save minidump all information */
struct minidump_info_native{
	char kernel_magic[6];                           		/* make sure minidump data valid */
        struct regs_info_native regs_info_native;			/* | struct pt_regs |                   */
        struct regs_memory_info_native regs_memory_info_native;         /* | memory amount regs |  , need paddr and size, if paddr invalid set it as 0  */
        struct section_info_total_native section_info_total_native;   	/* | sections | , text,rodata,page_table ....,may be logbuf in here */
        int minidump_elfhdr_size;                       		/* minidump elfhdr data size: update in uboot  */
        int minidump_elfhdr_size_comp;                  		/* minidump elfhdr data size,after compressed  */
        struct minidump_data_desc  desc;                		/* minidump contents description */
        int minidump_data_size;                         		/* minidump data total size: regs_all_size + reg_memory_all_size + section_all_size  */
