...
int gpt_key_cmd = 0;
extern uint32_t ascii_to_hex(uint8_t *src, uint8_t *dst, uint16_t len);
gpt_cmd_data gpt_data __attribute__((aligned(4096))) = {{0}, {0}, {0}};
#endif

#ifdef CONFIG_DOWNLOAD_RECORD
extern record_partition_t record_partition_tab[GPT_ENTRY_NUMBERS];
uint32_t dload_index = 0;
uint32_t erase_index = 0;
static uint32_t download_cnt = 0;
#endif
...
static __inline dl_cmd_type_t convert_operate_status(int err)
{
	switch (err)
	{
		...
		case OPERATE_MAGIC_ERROR:
			return BSL_REP_MAGIC_ERROR;
		case OPERATE_REPARTITION_ERROR:
			return BSL_REP_REPARTITION_ERROR;
		case OPERATE_READ_ERROR:
			return BSL_READ_ERROR;
		case OPERATE_MALLOC_ERROR:
			return BSL_REP_MALLOC_ERROR;
		case OPERATE_VERIFY_ERROR:
...
int dl_pre_record_flag(void)
{
	...
	/* Erase record flag */
	if (0 != common_raw_erase("miscdata", DOWNLOAD_RECORD_LEN, DOWNLOAD_RECORD_OFFSET))
		errorf("erase download record flag fail\n");
	else
		dprintf(INFO,"erase download record flag success\n");
	...
	dprintf(INFO,"Get total partition nums:%u\n", total);

	free(info);
	return 0;
}

uint32_t get_partition_index(uchar *name)
{
	uint32_t index_temp = 0;
	uint32_t i;

	for (i = 0; i < GPT_ENTRY_NUMBERS; i++) {
		if (!strcmp(record_partition_tab[i].partition_name, (const char *)name)) {
			index_temp = record_partition_tab[i].index;
			break;
		}
	}
...
int _parse_repartition_header(uchar * data, REPARTITION_TABLE_INFO * info, uchar ** list)
{
	...
	pointer += 2;
	dprintf(INFO,"%s: version(%d),unit(%d), table_count(%d), table_tag(%d), table_offset(%d), table_size(%d)\n",
...
{
	dprintf(INFO,"enable write flash\n");
...
int dl_cmd_write_start (dl_packet_t *packet, void *arg)
{
...
#ifdef CONFIG_MMC_POWP_SUPPORT
	if (!strcmp((char *)partition_name, g_part_name) && ENABLE_WRITE_PROTECT == g_part_protected) {
		if (common_set_powp(g_part_name)) {
			errorf("%s can't be protected, pls check it\n", g_part_name);
			return -1;
		}
		debugf("write start powp success\n");
		g_part_protected += 2;
	}
#endif
	...
	dload_index = get_partition_index(partition_name);
	dprintf(INFO,"download_index = %d\n", dload_index);
#endif
...
int dl_cmd_write_end (dl_packet_t *packet, void *arg)
{
	...
	if (op_res == OPERATE_SUCCESS)
		...
		download_flag = 2;
	if (0 != common_raw_write("miscdata", 0x1, 0, DOWNLOAD_RECORD_OFFSET + dload_index, (char *)&download_flag))
		errorf("write miscdata download partition error\n");
	else
		dprintf(INFO,"write miscdata download partition dload_index:%d, download_flag:%d success\n", dload_index, download_flag);
#endif
	_send_reply(op_res);
	return 0;
}
...
int dl_cmd_read_start(dl_packet_t *packet, void *arg)
{
	...
	if(0 == strcmp(partition_name,"uboot") || 0 == strcmp(partition_name,"splloader"))
		spl_read_size = size;
#endif
	op_res = dl_read_start(partition_name, size, packet);
...
int dl_cmd_read_midst(dl_packet_t *packet, void *arg)
{
	...
	if (size > MAX_PKT_SIZE) {
		debugf("Size:0x%x beyond MAX_PKT_SIZE(0x%x)\n",size,MAX_PKT_SIZE);
...
int dl_cmd_erase(dl_packet_t *packet, void *arg)
{
	...
	erase_index = get_partition_index(partition_name);
	dprintf(INFO,"erase partition:%s, erase_index = %d\n", partition_name, erase_index);
#endif

	op_res = dl_erase((const char *)partition_name, size);
	...
	if (op_res == OPERATE_SUCCESS)
		...
		erase_flag = 4;
	if (0 != common_raw_write("miscdata", 0x1, 0, DOWNLOAD_RECORD_OFFSET + erase_index, (char *)&erase_flag))
		errorf("write miscdata download partition flag  error\n");
	else
		dprintf(INFO,"write miscdata download partition:%s, erase_index:%d,  erase_flag:%d success\n", partition_name, erase_index, erase_flag);
#endif
...
int dl_cmd_repartition(dl_packet_t *pakcet, void *arg)
{
	...
	if (0 != (size % part_cell_length)) {
		dprintf(INFO,"%s:recvd packet size(%d) error \n", __FUNCTION__,size);
	...
	if (ENABLE_WRITE_PROTECT == g_part_protected) {
		...
		debugf("repartition powp success\n");
		g_part_protected += 1;
	}
#endif
...
{
	uchar * raw_data = packet->body.content;
	uint16_t size = packet->body.size;
	int i;
	int32_t op_res = 1;
	int32_t len = 0;

	gpt_key_cmd = 0;
	char *get_gpt_data = memalign(SZ_4K, size);
	char *data_to_hex = memalign(SZ_4K, size);

	dprintf(ALWAYS, "Entry dl_cmd_get_gpt\n");
	if (get_gpt_data == NULL) {
		errorf("malloc get_gpt_data error!\n");
		return -1;
	}
	memset(get_gpt_data, 0, size);

	if(data_to_hex == NULL) {
		errorf("malloc data_to_hex error!\n");
		return -1;
	}
	memset(data_to_hex, 0, size);

	memcpy(get_gpt_data, raw_data, size);
	dprintf(INFO, "size is %d\n", size);
	len = ascii_to_hex(get_gpt_data, data_to_hex, size);
	dprintf(INFO, "len is %d\n", len);

	memcpy(gpt_data.header, data_to_hex, SZ_H);
	memcpy(gpt_data.key, data_to_hex+SZ_H, SZ_K);
...
void dl_set_wdtflag(void)
{
	char buf[WDTEN_DATA_LEN] = {0};
	char dsbuf[DSWDTEN_DATA_LEN] = {"enabled"};
#if DEBUG
		strcpy(buf, "disable");
#else
		strcpy(buf, "enabled");
	...
	if (0 != common_raw_write("miscdata", WDTEN_DATA_LEN, 0, WDTEN_DATA_OFFSET, buf)) {
		errorf("write default WDT %s flag fail\n", buf);
		return;
	} else {
		debugf("write default WDT %s flag success\n", buf);
	}

	if (0 != common_raw_write("miscdata", DSWDTEN_DATA_LEN, 0, DSWDTEN_DATA_OFFSET, dsbuf)) {
		errorf("write default DSWDT disable flag fail\n");
		return;
	} else {
		debugf("write default DSWDT disable flag success\n");
	}

	if (!strcmp("autodloader", g_env_bootmode)) {
		if (0 != common_raw_write("miscdata", DL_MODE_DATA_LEN, 0, DL_MODE_OFFSET, "auto")) {
			errorf("write autodloader mode flag fail\n");
			return;
		} else {
			debugf("write autodloader mode flag success\n");
		}
	}
}
...
{
	if (0 != common_raw_read("miscdata", DL_PROCESS_LEN, DL_PROCESS_OFFSET, buf)) {
		errorf("get enter_download flag fail\n");
		return;
	} else {
		dprintf(ALWAYS, "get enter_download flag :%s\n", buf);
	}
}

static void dl_set_downloadflag(void)
{
	char buf[DL_PROCESS_LEN] = {"enable"};
	if (0 != common_raw_write("miscdata", DL_PROCESS_LEN, 0, DL_PROCESS_OFFSET, buf)) {
		errorf("set enter_download flag fail\n");
		return;
	} else {
		dprintf(ALWAYS, "set enter_download flag to %s success\n", buf);
	}
}

void dl_clear_downloadflag(void)
{
	char buf[DL_PROCESS_LEN] = {"disable"};
	if (0 != common_raw_write("miscdata", DL_PROCESS_LEN, 0, DL_PROCESS_OFFSET, buf)) {
		errorf("clear enter_download flag fail\n");
		return;
	} else {
		dprintf(ALWAYS, "clear enter_download flag to %s success\n", buf);
	}
}

void dl_clear_dvfs_set_paras(void)
{
	unsigned int dvfs_set_default = 0;

	if (0 != common_raw_write("miscdata", (uint64_t)DVFS_SET_LEN, (uint64_t)0,
				  (uint64_t)DVFS_SET_OFFSET, (char*)(&dvfs_set_default)))
		errorf("clear dvfs_set flag fail\n");
	else
		debugf("clear dvfs_set flag to %d success\n", dvfs_set_default);
}

#ifdef CONFIG_USBPINMUX
extern void usbmux_miscdata_first_write(void);
#endif


#ifdef CONFIG_DOWNLOAD_RECORD
void dl_dump_record_flag(void)
{
	uint8_t recordbuf[DOWNLOAD_RECORD_LEN] = {0};
	uint32_t i;

	if (0 != common_raw_read("miscdata", DOWNLOAD_RECORD_LEN, DOWNLOAD_RECORD_OFFSET, (char *)recordbuf))
		errorf("read download record partition fail\n");
	else
		dprintf(ALWAYS, "read download record partition success\n");

	dprintf(INFO, "print the record partition data\n");
	for (i = 0; i < DOWNLOAD_RECORD_LEN; i++) {
		dprintf(INFO, "0x%x\n", recordbuf[i]);
	}
	dprintf(INFO,"\n");
}
#endif
extern int erase_fdl_download_auth_data(void);
int dl_cmd_reboot (dl_packet_t *pakcet, void *arg)
{
	int32_t op_res = OPERATE_SUCCESS;
	dl_set_wdtflag();
	char cons_baudrate[CONS_BAUDRATE_LEN] = "disable";

	if (0 != common_raw_write("miscdata", CONS_BAUDRATE_LEN, (uint64_t)0, (uint64_t)CONS_BAUDRATE_OFFSET, cons_baudrate)) {
		errorf("write default baudrate fail\n");
		op_res = OPERATE_WRITE_ERROR;
	} else {
		debugf("write default baudrate success\n");
	}

#ifdef CONFIG_USBPINMUX
	/* write default_val to miscdata for usbpinmux in engineer mode */
	usbmux_miscdata_first_write();
#endif
	/*erase ddr paras writed by engineer mode*/
	ddr_debug_paras_erase();

	/* erase dvfs set paras */
	dl_clear_dvfs_set_paras();
#ifdef CONFIG_DOWNLOAD_RECORD
	dl_dump_record_flag();
#endif
	...
	if(erase_fdl_download_auth_data())
	{
...
int dl_cmd_read_ref_info(dl_packet_t *packet, void *arg)
{
	...
	if (buf == NULL) {
		...
		debugf("memcpy\n");
		dl_read_ref_info("miscdata", size, (uint64_t)REF_INFO_OFF, buf,(char *)packet->body.content);
	}
	dl_send_packet (packet);
	if(buf != NULL)
		free(buf);
...
int dl_cmd_write_datetime(dl_packet_t *packet, void *arg)
{
	...
	char *raw_data = (char *) packet->body.content;
...
int dl_cmd_set_debuginfo(dl_packet_t *packet, void *arg)
{
	...
	if (size == 0) {
		...
		dprintf(ALWAYS, "get debuginfo {%s}\n", debuginfo_buf);
		if (0 != common_raw_write("miscdata", debuginfo_size, (uint64_t)0, DEBUG_INFO_OFFSET, debuginfo_buf)) {
			op_res = OPERATE_WRITE_ERROR;
		}
	} else {
...
/*same to raw_cur_buf*/
ALTER_BUFFER_ATTR *sparse_cur_buf = NULL;
...
{
	if (NULL == pdata) {
		dprintf(INFO,"%s raw data buf pointer is NULL\r\n", __func__);
	...
	sparse_cur_buf = &raw_buf1;//for handle sparse
...
int dl_cmd_check_partition(dl_packet_t *packet, void *arg)
{
	...
	disk_partition_t* info = NULL;
	tool_partition_t* tool_info = NULL;
	...
	tool_info = malloc(sizeof(tool_partition_t) * GPT_ENTRY_NUMBERS);
	if (tool_info == NULL) {
		errorf("No space to malloc tool_info\n");
		free(info);
		dl_send_ack(BSL_REP_OPERATION_FAILED);
		return 0;
	}
	...
	for (i = 0; i < total; i++) {
		for (j = 0; j < strlen(info[i].part_name); j++) {
			tool_info[i].name[2 * j] = info[i].part_name[j];
		}

		tool_info[i].size = (info[i].blk_cnt * info[i].blksz) / SZ_1K;
	}
...
int dl_cmd_write_flush_raw_data(dl_packet_t *packet, void *arg)
{
	...
	if (op_res != OPERATE_SUCCESS) {
		dprintf(INFO,"Sha256 check err!!!\r\n");
		_send_reply(op_res);
	} else {
		op_res = dl_download_midst(raw_cur_buf->used, (char *)raw_cur_buf->addr);
		if (op_res != OPERATE_SUCCESS) {
			dprintf(INFO,"Download write err!!!, stop!!!\r\n");
		}
		_send_reply(op_res);
		raw_cur_buf->used = 0x0;
...
int dl_cmd_write_raw_data_enable(dl_packet_t *packet, void *arg)
{
	...
	if (packet->body.size == MUSB_DLOAD_FOR_LINUX_FLAG) {
		dprintf(INFO,"========download usb linux tools========\n");
		usb_set_dload_sys(MUSB_DLOAD_FOR_LINUX);
	}
...
int dl_set_selinux(char* selinux_buf) {
	...
	if (selinux_buf != NULL
		&& strlen(selinux_buf) <= SELINUX_INFO_LEN) {
...
int dl_cmd_disable_selinux(dl_packet_t *packet, void *arg)
{
	...
	ret = dl_set_selinux(selinux_buf);
	if (ret) {
		op_res = OPERATE_WRITE_ERROR;
...
{
	int op_res = OPERATE_SUCCESS;
	u8 buf[DL_IQ_DYN_DATA_LEN];
	char* tmp_buf = (char *)(packet->body.content + 0x28);

	if (strlen(tmp_buf) > DL_IQ_DYN_DATA_LEN)
		goto fail;

	memset(buf, 0, DL_IQ_DYN_DATA_LEN);
	memcpy(buf, tmp_buf, strlen(tmp_buf));
	if (common_raw_write("miscdata", (u64)sizeof(buf), (u64)0,
				(u64)DL_IQ_DYN_OFFSET, buf)) {
		goto fail;
	}
	...
	return 0;
fail:
	_send_reply(OPERATE_WRITE_ERROR);
	return -1;
}
...
#ifdef CONFIG_SPRD_LOG
int get_dl_cmd_log2pc_status(void)
{
	return dl_log2pc_not_ready;
}
...
int dl_cmd_dump_log(dl_packet_t *packet, void *arg)
{
	...
	if ((p_log_buffer->addr < p_log_buffer->pointer)) {
		len = p_log_buffer->pointer - p_log_buffer->addr;
		log = p_log_buffer->addr;
	} else {
		len = (p_log_buffer->end - p_log_buffer->addr) +
			(p_log_buffer->pointer - (uint8_t *)LOG_BUFFER_START_ADDR);
		log = malloc(len);
		if (log == NULL)
			return -3;
		memcpy(log, p_log_buffer->addr, (p_log_buffer->end - p_log_buffer->addr));
		memcpy(log + (p_log_buffer->end - p_log_buffer->addr),
			(uint8_t *)LOG_BUFFER_START_ADDR,
			(p_log_buffer->pointer - (uint8_t *)LOG_BUFFER_START_ADDR));
		cycle_flag = 1;
	}
	...
	do {
		i = len > DL_RESP_MAX_PKT_SIZE ? DL_RESP_MAX_PKT_SIZE : len;
		memcpy(packet_rsp->body.content + 4, (log+sendout) , i);
		/* adjuct length to even and fill pad if necessary */
		if (i % 2) {
...
{
	void *ptr = &(packet->body.content[0]);
	uint32_t timeout = *(uint32_t *)(ptr);
	dprintf(INFO,"start wdg timeout %dms\n", timeout);
	dl_set_downloadflag();
...
extern int check_download_status(void);
#if defined(CONFIG_DL_PCTOOL_AUTHORIZE) || defined(CONFIG_BLOCK_ENTER_MODE)
/**
 * - double check if we need to authorize pctool
 * Return: 0, no; other yes
 */
#ifdef CONFIG_DL_PCTOOL_AUTHORIZE
...
{
#ifdef SPRD_SECBOOT
	static unsigned int lcs = ~0;

	if (5 == lcs) {
		debugf("efuse secboot was enabled\n");
		return 1;
	} else if (~0 != lcs) {
		debugf("efuse secboot was disabled(lcs: %d)\n", lcs);
		return 0;
	}

	if (sprd_get_lcs(&lcs) || (5 != lcs)) {
		debugf("check efuse secboot was disabled(lcs: %d)\n", lcs);
		return 0;
	}
	debugf("check efuse secboot was enabled\n");
	return 1;	/* need authorize */
#else
	return 0;	/* secboot not support */
#endif
}
...
{
		debugf("Write efuse\n");
		if (sprd_dl_write_efuse()){
			errorf("Write efuse fial\n");
			_send_reply(OPERATE_WRITE_ERROR);
			return 0;
		}
		debugf("Write efuse success, send ack 0x%x\n", BSL_REP_ACK);
		_send_reply(OPERATE_SUCCESS);
		return 0;
}
#endif
