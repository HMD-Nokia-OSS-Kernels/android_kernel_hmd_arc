...
{
	dl_packet_t *ptr;
//dprintf(INFO, "bfgetpk,time=%u  \n", SCI_GetTickCount());

	// waiting for a packet
	while (NULL == packet_completed_list) {
		FDL_PacketDoIdle();
	}
//dprintf(INFO, "aftgetpk,time=%u \n ", SCI_GetTickCount());
	...
	while (1) {
		if (gFdlUsedChannel == FDL_USBChannel()) {
			...
			if (crc) {
				...
				dprintf(ALWAYS, "%4x%4x", packet_ptr->body.type,
				       packet_ptr->body.size);
				for (k = 0;	k  < packet_ptr->data_size - 4; k++)
					dprintf(ALWAYS, "%2x", packet_ptr->body.content[k]);
		...
		*/
		...
		if (packet_ptr->data_size > MAX_PKT_SIZE) {
			packet_receiving = NULL;
			dl_free_packet (packet_ptr);
			dprintf(ALWAYS, "data_size error : datasize = %d  MAX_PKT_SIZE = %d\n",
				packet_ptr->data_size, MAX_PKT_SIZE);
			dprintf(ALWAYS, "\n\n\n%s %s %d\n\n\n", __FILE__, __FUNCTION__, __LINE__);
			dprintf(ALWAYS, "data_size error");
			dprintf(ALWAYS, "\n\n\n%s %s %d\n\n\n", __FILE__, __FUNCTION__, __LINE__);
			SEND_ERROR_RSP (BSL_REP_VERIFY_ERROR)
			//return;
		}
...
void dl_send_ack (dl_cmd_type_t  pkt_type)
{
	...
	send_len = FDL_DataProcess (packet_ptr, tmp_packet_ptr);
	//dprintf(INFO, "pkt->body.type=%d\n" , packet_ptr->body.type);
	//dprintf(INFO, "before send_len = %d\n" , send_len);
	FDL_WritePacket ( (char *) (& (tmp_packet_ptr->body)), send_len);
	//dprintf(INFO, "send_len = %d" , send_len);
...
{
	unsigned int usb_check_count = 0;

	for (usb_check_count = 0; usb_check_count < 3; usb_check_count++) {
...
void dl_first_init_test(void)
{
	dprintf(INFO, "dl_first_init_test\n");
	dl_packet_t *packet;
	...
	//dprintf(INFO,"ack 7E before send_len = %d\n" , packet->body.size);
	//dprintf(INFO,"ack 7E before send_type = %d\n" , packet->body.type);
