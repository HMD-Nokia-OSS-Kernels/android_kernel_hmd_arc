...
int autodloader_initusb(void)
{
	...
	while(!usb_is_configured()){
		...
		now++;
		if(now > USB_ENUM_MS) {
			debugf("USB SERIAL CONFIGED failed \n");
			return -1;
		}
	}
	debugf("USB SERIAL CONFIGED\n");

	debugf("Waiting For USB SERIAL PORT OPEN ... ... \n");
	while(!usb_is_port_open());
	debugf("USB SERIAL PORT OPENED\n");

	//gs_open();

	debugf(" in [%s] has been Hidden \n",__func__);
	return 0;
}
