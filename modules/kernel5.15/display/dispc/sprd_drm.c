						...
						struct drm_atomic_state *state)
{
	...
	for_each_old_crtc_in_state(state, crtc, old_crtc_state, i) {
		...
		if (to_sprd_crtc(crtc)->type == SPRD_DISPLAY_TYPE_DPI) {
...
#ifdef CONFIG_DRM_SPRD_SPI
	if (sprd_panel_find_node_by_name()) {
		for (i = 0; ; i++) {
			port = of_parse_phandle(dev->of_node, "spi_ports", i);
			if (!port)
				break;
		...
		if (i == 0) {
			dev_err(dev, "missing 'spi_ports' property\n");
			return -ENODEV;
		}
...
#ifdef CONFIG_DRM_SPRD_SPI
static struct platform_driver *sprd_drm_spi_drivers[]  = {
	&sprd_spi_swdispc_driver,
	&sprd_spi_driver,
};
#endif
...
#ifdef CONFIG_DRM_SPRD_SPI
	if (sprd_panel_find_node_by_name()) {
		ret = platform_register_drivers(sprd_drm_spi_drivers,
						ARRAY_SIZE(sprd_drm_spi_drivers));
		if (ret)
			return ret;
	}
#endif
...
#ifdef CONFIG_DRM_SPRD_SPI
	if (sprd_panel_find_node_by_name()) {
		spi_register_driver(&sprd_spi_panel_driver);
	}
#endif
...
{
#ifdef CONFIG_DRM_SPRD_SPI
	spi_unregister_driver(&sprd_spi_panel_driver);
#endif
...
#ifdef CONFIG_DRM_SPRD_SPI
	platform_unregister_drivers(sprd_drm_spi_drivers,
				    ARRAY_SIZE(sprd_drm_spi_drivers));
#endif
