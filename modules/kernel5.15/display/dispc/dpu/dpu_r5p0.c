...
static bool enhance_en_cfg(struct dpu_context *ctx, int mask)
{
	if (DPU_REG_RD(ctx->base + REG_DPU_ENHANCE_CFG) & mask)
		return false;

	DPU_REG_SET(ctx->base + REG_DPU_ENHANCE_CFG, mask);
	return true;
}
...
static void enhance_update(struct dpu_context *ctx, bool enhance_en_update_flag)
{
	struct dpu_enhance *enhance = ctx->enhance;
	...
	if ((ctx->if_type == SPRD_DPU_IF_DPI) && !ctx->stopped) {
		 ...
		 */
		if (!(enhance->pq_update_by_flip) || enhance_en_update_flag) {
			DPU_REG_SET(ctx->base + REG_DPU_CTRL, BIT(2));
			dpu_wait_update_done(ctx);
			pr_debug("update by self\n");
		}
...
static void dpu_enhance_set(struct dpu_context *ctx, u32 id, void *param, size_t count)
{
	...
	bool dpu_stopped;
	bool enhance_en_update_flag = true;
	u32 *p;
	int i;
	...
	case ENHANCE_CFG_ID_HSV:
		...
		enhance_en_update_flag = enhance_en_cfg(ctx, BIT(2));
		pr_info("enhance hsv set\n");
		break;
	case ENHANCE_CFG_ID_CM:
		memcpy(&enhance->cm_copy, param, sizeof(enhance->cm_copy));
		dpu_cm_set(ctx, CM_PQ);
		enhance_en_update_flag = enhance_en_cfg(ctx, BIT(3));
	...
	case ENHANCE_CFG_ID_LTM:
		...
		enhance_en_update_flag = enhance_en_cfg(ctx, BIT(6));
	...
	case ENHANCE_CFG_ID_SLP:
		...
		enhance_en_update_flag = enhance_en_cfg(ctx, BIT(4));
	...
	case ENHANCE_CFG_ID_GAMMA:
		...
		enhance_en_update_flag = enhance_en_cfg(ctx, BIT(5) | BIT(10));
	...
	case ENHANCE_CFG_ID_LUT3D:
		...
		#ifndef ZCFG_POWERON_LOGO_FLICKER
			dpu_stop(ctx);
		#endif
		...
		enhance_en_update_flag = enhance_en_cfg(ctx, BIT(9));
	...
	enhance_update(ctx, enhance_en_update_flag);
	enhance->enhance_en = DPU_REG_RD(ctx->base + REG_DPU_ENHANCE_CFG);
}
