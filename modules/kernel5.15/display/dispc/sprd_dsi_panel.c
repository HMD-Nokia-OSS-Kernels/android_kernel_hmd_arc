...
#include "../../input/udc/udc_lcd.h"
#endif
#define SPRD_MIPI_DSI_FMT_DSC 0xff

#if defined(ZCFG_MK_TP_GESTURE)
extern u8 set_gesture_status_info(void);
extern u8 get_gesture_status_info(void);
int gesture_status=0;
#endif

#ifdef ZCFG_CTP_NEAR_CLOSE_BACKLIGHT 
static struct drm_panel *ctp_panel;
#endif
...
#if defined(CONFIG_LCM_POWER_TPS65132_SUPPORT)
extern void tps65132_config_voltage(void);
#endif
/*add by jinqiang for zyt_info*/
static char s_zyt_peripherals_LCD_name[32] = "Unknow";
extern void zyt_get_lcm_info(void *name);

static void zyt_set_lcm_info(void)
{
	//printk("zyt_set_lcm_info s_zyt_peripherals_LCD_name=%s\n", s_zyt_peripherals_LCD_name);
	zyt_get_lcm_info(s_zyt_peripherals_LCD_name);
}
/*add by jinqiang for zyt_info*/
...
struct device_node *sprd_get_panel_node_by_name(void)
{
	...
	/*add by jinqiang for zyt_info start*/
	strncpy(s_zyt_peripherals_LCD_name, lcd_name, sizeof(s_zyt_peripherals_LCD_name)-1);
	zyt_set_lcm_info();
	/*add by jinqiang for zyt_info end*/
...
void incellctp_set_sprd_panel_phone_call_flag(bool enable){
	 ...
	 printk("cxp incellctp_phone_call_switch_flag = %d\n",incellctp_phone_call_switch_flag);
}
...
static int sprd_panel_unprepare(struct drm_panel *p)
{
	...
	if (panel->info.panel_type == SPRD_PANEL_TYPE_AMOLED) {
		...
		#if defined(ZCFG_MK_TP_GESTURE)
		gesture_status=get_gesture_status_info();
...
#ifdef GCORE_PROXIMIC_CONFIG
else
{
	sprd_panel_send_cmds(panel->slave,
					 panel->info.cmds[CMD_CODE_PROX_SLEEP_IN],
					 panel->info.cmds_len[CMD_CODE_PROX_SLEEP_IN]);
		}	
#endif		 
#endif
...
#ifdef ZCFG_CTP_NEAR_CLOSE_BACKLIGHT 
void incellctp_near_close_backlight(bool enable)
{
	//DRM_INFO("%s()\n", __func__);

	if(ctp_panel != NULL ){
		struct sprd_panel *panel = to_sprd_panel(ctp_panel);
		if(enable){
			if(panel->backlight){
			panel->backlight->props.power = FB_BLANK_UNBLANK;
			panel->backlight->props.state &= ~BL_CORE_FBBLANK;
			backlight_update_status(panel->backlight);
			DRM_INFO("%s(): backlight is on\n", __func__);

			}
		}else{
		//DRM_INFO("wab enable = %d\n", enable);
			if (panel->backlight) {
			panel->backlight->props.power = FB_BLANK_POWERDOWN;
			panel->backlight->props.state |= BL_CORE_FBBLANK;
			backlight_update_status(panel->backlight);
			DRM_INFO("%s(): backlight is off\n", __func__);
			}
		}
	}else{
...
EXPORT_SYMBOL(incellctp_near_close_backlight);
#endif
...
{
	sprd_panel_send_cmds(panel->slave,
			     panel->info.cmds[CMD_CODE_PROX_INIT],
			     panel->info.cmds_len[CMD_CODE_PROX_INIT]);
}
#endif
#endif
...
static int sprd_oled_set_brightness(struct backlight_device *bdev)
{
    ...
    #ifdef ZCFG_BACKLIGHT_OLED_DELAY
	...
	if (oled->cmds_total == 1) {
		...
		if(b_light==1)
		{
			mdelay(ZCFG_BACKLIGHT_OLED_DELAY);
...
#ifdef GCORE_PROXIMIC_CONFIG
	p = of_get_property(lcd_node, "sprd,face-out-command-gesture", &bytes);
	if (p) {
		info->cmds[CMD_CODE_PROX_INIT] = p;
		info->cmds_len[CMD_CODE_PROX_INIT] = bytes;
	} else
		DRM_ERROR("can't find sprd,face-out-command-gesture property\n");

	p = of_get_property(lcd_node, "sprd,face-in-command-gesture", &bytes);
	if (p) {
		info->cmds[CMD_CODE_PROX_SLEEP_IN] = p;
		info->cmds_len[CMD_CODE_PROX_SLEEP_IN] = bytes;
	} else
		DRM_ERROR("can't find sprd,face-in-command-gesture property\n");
...
static int sprd_panel_probe(struct mipi_dsi_device *slave)
{
	...
	if (!panel)
		return -ENOMEM;
    //DRM_ERROR("zjx sprd_panel_probe sprd_get_panel_node_by_name\n");
	...
	if (udc_lcd_create(SEC_LCD0, panel)){
	 	strncpy(s_zyt_peripherals_LCD_name, panel->lcd_name,sizeof(s_zyt_peripherals_LCD_name)-1);
	  	DRM_WARN("jinqiang sprd_panel_probe: s_zyt_peripherals_LCD_name:%s\n", s_zyt_peripherals_LCD_name);
	 	zyt_set_lcm_info();
	}
