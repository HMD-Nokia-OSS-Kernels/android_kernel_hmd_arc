...
#define I2C_MAX_UMB9230S_1M7_FREQ	1700000
...
static void sprd_i2c_set_clk(void __iomem *addr, u32 freq)
{
	...
	if (freq == I2C_MAX_UMB9230S_1M7_FREQ) {
		tmp = readl(addr + I2C_CTL);
		writel(tmp | I2C_TRIM_OPT, addr + I2C_CTL);
		high = ((i2c_dvd << 1) * 16) / 50;
		low = ((i2c_dvd << 1) * 34) / 50;
	...
	if (freq == I2C_MAX_FAST_MODE_FREQ)
		...
		writel(0x80, addr + ADDR_STA0_DVD);
}
