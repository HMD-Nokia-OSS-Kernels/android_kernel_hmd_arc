...
//#define SPREADTRUM_PLATFORM          //?a????¨¬o?SC8810|?¡ì??
#define STK_ALS_FIR
// #define STK_IRS
//#define STK_CHK_REG
//#define STK_GES
...
#define PS_NAME			"stk3x3x-ps"
#define ALS_NAME 		"stk3x3x-als"
...
extern int CDC_Plsensor_Device_Id(int id);
...
uint32_t stk_alscode2lux(struct stk3x3x_data *ps_data, uint32_t alscode)
{
	...
	if(alscode < 10 )
		ps_data->als_transmittance = 1125; 
	else if(alscode < 125)
		ps_data->als_transmittance = 266; 
	else 
		ps_data->als_transmittance = 219;
	
	lux_value = (alscode * ps_data->als_transmittance)/1000;
	
	if( lux_count%10 == 0)
	printk("stk test alscode=%d, correct =%d,lux=%d\n",alscode,ps_data->als_transmittance,lux_value);
...
static int32_t stk3x3x_init_all_reg(struct stk3x3x_data *ps_data)
{
    ...
    ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, STK_ALSCTRL_REG, ps_data->alsctrl_reg+0x01);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
	...
	ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, 0x04, 0x00);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write 0x04 error\n", __func__);
        return ret;
    }
    ...
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write 0xFA error\n", __func__);
        return ret;
    }
    ...
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write 0xA0 error\n", __func__);
        return ret;
    }
    ...
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write 0xDB error\n", __func__);
        return ret;
    }
	
	ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, 0xAA, 0x05);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write 0xAA, error\n", __func__);
        return ret;
    }	
...
//#endif	
	...
	/*
    ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, 0x87, 0x60);
    if (ret < 0)
	{
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
...
static int32_t stk3x3x_read_otp25(struct stk3x3x_data *ps_data)	
{
	int32_t ret, otp25;
	
    ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, 0x0, 0x2);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, 0x90, 0x25);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, 0x92, 0x82);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    ...
    ret = stk3x3x_i2c_smbus_read_byte_data(ps_data->client,0x91);
    if (ret < 0)
    {
   ...
   ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client, 0x0, 0x0);
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
...
static int32_t stk3x3x_check_pid(struct stk3x3x_data *ps_data)
{
	...
	if(stk3x3x_read_otp25(ps_data) == 1)
	{
...
#ifdef STK_GES			
static int32_t stk3x3x_set_flag2(struct stk3x3x_data *ps_data, uint8_t org_flag2_reg, uint8_t clr)
{
	uint8_t w_flag2;
	int ret;
    ...
    ret = stk3x3x_i2c_smbus_write_byte_data(ps_data->client,STK_FLAG2_REG, w_flag2);	
	if(ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
}

static int32_t stk3x3x_get_flag2(struct stk3x3x_data *ps_data)
{	
	int ret;
    ret = stk3x3x_i2c_smbus_read_byte_data(ps_data->client,STK_FLAG2_REG);	
	if(ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
}
						...
						unsigned int enabled)
{
	...
	err = stk3x3x_enable_als(als_data, enabled);
...
static int stk_ps_val(struct stk3x3x_data *ps_data)
{
	...
	ret = stk3x3x_i2c_read_data(ps_data->client, 0x20, 4, value);
	if(ret < 0)
	{
		printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
		return ret;
...
static void stk_ps_poll_work_func(struct work_struct *work)
{
	...
	if(ps_data->ges_enabled == 2)
	{
		...
		if(disable_flag2)
		{
			ret = stk3x3x_set_flag2(ps_data, org_flag2_reg, disable_flag2);		
			if(ret < 0)
				goto err_i2c_rw;
		}			
	}	
...
static int stk3x3x_set_wq(struct stk3x3x_data *ps_data)
{
	...
	hrtimer_init(&ps_data->als_timer, CLOCK_BOOTTIME, HRTIMER_MODE_REL);
	ps_data->als_poll_delay = ns_to_ktime(110 * NSEC_PER_MSEC);		
	...
	hrtimer_init(&ps_data->ps_timer, CLOCK_BOOTTIME, HRTIMER_MODE_REL);
...
static int stk3x3x_set_input_devices(struct stk3x3x_data *ps_data)
{
	...
	input_set_abs_params(ps_data->ps_input_dev, ABS_MISC, 0,10001, 0, 0);
	...
	kobject_uevent(&ps_data->als_input_dev->dev.kobj, KOBJ_ADD);
	kobject_uevent(&ps_data->ps_input_dev->dev.kobj, KOBJ_ADD);
                        ...
                        const struct i2c_device_id *id)
{
    ...
    ps_data->ps_wakelock = wakeup_source_create("stk_input_wakelock");
    wakeup_source_add(ps_data->ps_wakelock);
#ifdef STK_POLL_PS			
	//wake_lock_init(&ps_data->ps_nosuspend_wl,WAKE_LOCK_SUSPEND, "stk_nosuspend_wakelock");
	ps_data->ps_nosuspend_wl = wakeup_source_create("stk_nosuspend_wakelock");
	wakeup_source_add(ps_data->ps_nosuspend_wl);
#endif	
...
//#ifdef CONFIG_HAS_EARLYSUSPEND
	...
	{
		err = -ENODEV;
		printk("Fail to find stk3x3x");
		return err;
	} else {
		extern void zyt_info_s2(char* ,char*);
		zyt_info_s2("[PLSensor] : ","STK3X3X");
	}
...
err_als_input_allocate:
   ...
   // wake_lock_destroy(&ps_data->ps_nosuspend_wl);	
    wakeup_source_remove(ps_data->ps_nosuspend_wl);
    wakeup_source_destroy(ps_data->ps_nosuspend_wl);
#endif	
   // wake_lock_destroy(&ps_data->ps_wakelock);	
    wakeup_source_remove(ps_data->ps_wakelock);
...
static int __init stk3x3x_init(void)
{
	...
	{
		printk("CTP(0x%x)Exist!", CDC_Plsensor_Device_Id(0));
		return -ENODEV;
	}
