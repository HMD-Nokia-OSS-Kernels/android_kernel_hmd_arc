...
#define AW9610X_DRIVER_VERSION "v2.3.0"
...
#define SAR_SENSOR0 	62
#define SAR_SENSOR1 	61
int sar_sent_flag = 0;	
...
static int32_t aw9610x_filedata_deal(struct aw9610x *aw9610x)
{
	...
	fp = filp_open_block(cali_file_name, O_RDWR | O_CREAT, 0644);
	if (IS_ERR(fp)) {
						...
						struct aw_bin *aw_bin_data_s)
{
	...
	for (i = 0; i < aw_bin_data_s->header_info[0].valid_data_len;
		...
		if (reg_addr == REG_SCANCTRL0) {
			aw9610x->channel_use_flag = (reg_data & 0x1f);
			AWLOGI(aw9610x->dev, "channel_use_flag:%d", aw9610x->channel_use_flag);
		}
	...
	if (aw9610x->power_on_load == AW_TURE) {
		aw9610x->power_on_load = AW_FALSE;
...
static int32_t aw9610x_para_loaded(struct aw9610x *aw9610x)
{
	...
	for (i = 0; i < len; i = i + 2) {
		if (aw9610x_reg_default[i] == REG_SCANCTRL0) {
			aw9610x->channel_use_flag = (aw9610x_reg_default[i+1] & 0x1f);
		}
...
static int shub_report_Sar_event(u16 Sar_handle,u32 value)
{
  struct shub_event_params event;
  event.Cmd = 129;
  event.HandleID = Sar_handle;//传入的handle值要与SENSOR_HANDLE_SAR_CH0，SENSOR_HANDLE_SAR_CH1一样
  event.fdata[0] = value;
  event.fdata[1] = 0;
  event.fdata[2] = 0;
...
static int shub_report_Sar_flush_event(u16 Sar_handle,u32 value)
{
  struct shub_event_params event;
  event.Cmd = 130;
  event.HandleID = Sar_handle;//传入的handle值要与SENSOR_HANDLE_SAR_CH0，SENSOR_HANDLE_SAR_CH1一样
...
static ssize_t aw9610x_status_set(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	int enable = 0,ret = 0;
...
static ssize_t aw9610x_sar0_flush_set(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	shub_report_Sar_flush_event(SAR_SENSOR0,0);
	return count;	
}

static ssize_t aw9610x_sar1_flush_set(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	shub_report_Sar_flush_event(SAR_SENSOR1,0);
	return count;	
}
...
static DEVICE_ATTR(enable, 0664, NULL,aw9610x_status_set);
static DEVICE_ATTR(sar0_flush, 0664, NULL,aw9610x_sar0_flush_set);
static DEVICE_ATTR(sar1_flush, 0664, NULL,aw9610x_sar1_flush_set);
...
static struct attribute *aw9610x_sar_attributes[] = {
	...
	&dev_attr_sar0_flush.attr,
	&dev_attr_sar1_flush.attr,
	NULL
};
...
static void aw9610x_irq_handle(struct aw9610x *aw9610x)
{
	...
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
		...
		case FAR:
			...
			if(sar_sent_flag){
				if(i == 0){
					shub_report_Sar_event(SAR_SENSOR0,0);
				}else if(i == 1){
					shub_report_Sar_event(SAR_SENSOR1,0);
				}
			}
			break;
		case TRIGGER_TH0:
			...
			if(sar_sent_flag){
				if(i == 0){
					shub_report_Sar_event(SAR_SENSOR0,0x3f800000);	//float -> int 
				}else if(i == 1){
					shub_report_Sar_event(SAR_SENSOR1,0x3f800000);	//float -> int
				}		
			}
			break;
		case TRIGGER_TH1:
