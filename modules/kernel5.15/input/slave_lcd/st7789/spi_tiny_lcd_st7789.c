...
/*************wyx add*****************/
#ifdef AGN_TFT_LCD_ST7789
static inline void initial_st7789(void)
 {	
	spidev_sync_write_cmd(0x11); //sleep out
	
	mdelay(120);
	spidev_sync_write_cmd(0x36); //Memory Data Access Control 从左到右 从上到下 RGB模式
	spidev_sync_write_data(0x00);

	spidev_sync_write_cmd(0x3A); //Interface Pixel Format 
	spidev_sync_write_data(0x05); //‘101’ = 16bit/pixel 设置成16位
	...
	spidev_sync_write_cmd(0x44); //Set Tear Scanline
	spidev_sync_write_data(0x00);
	spidev_sync_write_data(0x20);

	spidev_sync_write_cmd(0xB2);// Porch Setting  //st7789_write_datas
	spidev_sync_write_data(0x0C);
	spidev_sync_write_data(0x0C);
	spidev_sync_write_data(0x00);
	spidev_sync_write_data(0x33);
	spidev_sync_write_data(0x33);
	
	spidev_sync_write_cmd(0xB7);// Gate Control
	spidev_sync_write_data(0x75);
	
	spidev_sync_write_cmd(0xBB);// VCOM Setting
	spidev_sync_write_data(0x1F);

	spidev_sync_write_cmd(0xC0);//  LCM Control
	spidev_sync_write_data(0x2C); 

	spidev_sync_write_cmd(0xC2);//  VDV and VRH Command Enable
	spidev_sync_write_data(0x01); 
	
	spidev_sync_write_cmd(0xC3);//  VRH Set
	spidev_sync_write_data(0x13); 
	
	spidev_sync_write_cmd(0xC4);// VDV Set
	spidev_sync_write_data(0x20);

	spidev_sync_write_cmd(0xC6);//  Frame Rate Control in Normal Mode
	spidev_sync_write_data(0x0F);// 60hz

	spidev_sync_write_cmd(0xD0);// Power Control 1
	spidev_sync_write_data(0xA4);  
	spidev_sync_write_data(0xA1); 
	
	spidev_sync_write_cmd(0xD6);// Gate Output Selection in Sleep in Mode
	spidev_sync_write_data(0xA1);

	spidev_sync_write_cmd(0x21);// Display Inversion On
	spidev_sync_write_cmd(0xE0);//  Positive Voltage Gamma Control
	spidev_sync_write_data(0xD0);
	spidev_sync_write_data(0x08);
	spidev_sync_write_data(0x10);
	spidev_sync_write_data(0x0D);
	spidev_sync_write_data(0x0C);
	spidev_sync_write_data(0x07);
	spidev_sync_write_data(0x37);
	spidev_sync_write_data(0x53);
	spidev_sync_write_data(0x4C);
	spidev_sync_write_data(0x39);
	spidev_sync_write_data(0x15);
	spidev_sync_write_data(0x15);
	spidev_sync_write_data(0x2A);
	spidev_sync_write_data(0x2D);
	spidev_sync_write_cmd(0xE1);//  Negative Voltage Gamma Control
	spidev_sync_write_data(0xD0);
	spidev_sync_write_data(0x0D);
	spidev_sync_write_data(0x12);
	spidev_sync_write_data(0x08);
	spidev_sync_write_data(0x08);
	spidev_sync_write_data(0x15);
	spidev_sync_write_data(0x34);
	spidev_sync_write_data(0x34);
	spidev_sync_write_data(0x4A);
	spidev_sync_write_data(0x36);
	spidev_sync_write_data(0x12);
	spidev_sync_write_data(0x13);
	spidev_sync_write_data(0x2B);
	spidev_sync_write_data(0x2F);
	spidev_sync_write_cmd(0x29); //Display On
	mdelay(120);
 }
