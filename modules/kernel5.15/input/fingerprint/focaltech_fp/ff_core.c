...
// static int fts_notifier_callback_init(ff_context_t *ff_ctx); //wbk
// static int fts_notifier_callback_exit(ff_context_t *ff_ctx); //wbk
...
static int ff_set_irq(ff_context_t *ff_ctx, bool enable)
{
    ...
    FF_LOGD("set irq:%d->%d", ff_ctx->b_irq_enabled, enable);
...
static int ff_set_reset(ff_context_t *ff_ctx, bool high)
{
    ...
    FF_LOGD("set reset pin to %s,use_pinctrl:%d", high ? "high" : "low", ff_ctx->b_use_pinctrl);
    if (ff_ctx->b_use_pinctrl && ff_ctx->pinctrl) {
        FF_LOGD("use pinctrl to control reset pin");
    ...
    if (!ff_ctx->b_use_pinctrl && gpio_is_valid(ff_ctx->reset_gpio)) {
        FF_LOGD("use gpio(%d) to control reset pin", ff_ctx->reset_gpio);
...
static int ff_power_on(ff_context_t *ff_ctx, bool on)
{
    ...
    if (ff_ctx->b_power_always_on) {
        FF_LOGD("power is always on, no control power");
        return 0;
    }

    FF_LOGD("set power:%d->%d,use_regulator:%d", ff_ctx->b_power_on, on, ff_ctx->b_use_regulator);
    if (ff_ctx->b_power_on ^ on) {
        if (ff_ctx->b_use_regulator && ff_ctx->ff_vdd) {
            FF_LOGD("use regulator to control power");
        ...
        if (!ff_ctx->b_use_regulator) {
            if (ff_ctx->b_use_pinctrl && ff_ctx->pinctrl) {
                FF_LOGD("use pinctrl to control power pin");
            ...
            if (!ff_ctx->b_use_pinctrl && gpio_is_valid(ff_ctx->vdd_gpio)) {
                FF_LOGD("use gpio(%d) to control power", ff_ctx->vdd_gpio);
...
static int ff_ctl_init_driver(ff_context_t *ff_ctx)
{
    ...
    if (ff_ctx->b_screen_onoff_event) {
        // ret = fts_notifier_callback_init(ff_ctx); //wbk
        if (ret) {
...
static int ff_ctl_free_driver(ff_context_t *ff_ctx)
{
    ...
    if (ff_ctx->b_screen_onoff_event) {
        // fts_notifier_callback_exit(ff_ctx);//wbk
    }
...
#ifdef ZCFG_FINGERPRINT_TAKE_PHOTO_KEY
    if(kevent.code == 87 && kevent.value == 0){
        input_report_key(ff_ctx->input, kevent.code, 1);
        input_sync(ff_ctx->input);
        input_report_key(ff_ctx->input, kevent.code, 0);
        input_sync(ff_ctx->input);
    }
#else
...
static int ff_ctl_sync_config(ff_context_t *ff_ctx, unsigned long arg)
{
    ...
    if (ff_ctx->b_gesture_support) {
        FF_LOGD("gesture is supported.");
    }

    /* Take logging level effect. */
    FF_LOGD("set log:%d->%d", g_ff_log_level, driver_config.log_level);
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FF_IOC_FREE_DRIVER:
        FF_LOGD("ioctrl:FF_IOC_FREE_DRIVER");
...
int fts_power_sequence(bool power_on)
{
    ...
    if (power_on) {
        ...
        FF_LOGD("power sequence off, delay 100ms");
        ff_set_reset(ff_ctx, 0);
        msleep(5);
        ff_power_on(ff_ctx, 0);
        msleep(100);
    ...
    struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    FF_LOGD("'%s' enter.", __func__);
    sscanf(buf, "%d", &value);
    FF_LOGD("log level:%d->%d", g_ff_log_level, value);
...
static void ff_parse_dt(struct device *dev, ff_context_t *ff_ctx)
{
    ...
    FF_LOGD("irq gpio:%d, reset gpio:%d, vdd gpio:%d", ff_ctx->irq_gpio,
...
static int ff_init_pins(ff_context_t *ff_ctx)
{
    ...
    if (ff_ctx->b_use_pinctrl) {
        ...
        FF_LOGD("get irq-reset-power pinctrl success");
    } else {
        /* request irq gpio */
        if (gpio_is_valid(ff_ctx->irq_gpio)) {
            FF_LOGD("init irq gpio:%d", ff_ctx->irq_gpio);
        ...
        /* request reset gpio */
        if (gpio_is_valid(ff_ctx->reset_gpio)) {
            FF_LOGD("init reset gpio:%d", ff_ctx->reset_gpio);
        ...
        FF_LOGD("get irq and reset gpio success");
    }
...
static int ff_init_power(ff_context_t *ff_ctx)
{
    ...
    if (ff_ctx->b_power_always_on) {
        FF_LOGD("power is always on, no init power");
        return 0;
    }
    ...
    if (ff_ctx->b_use_regulator) {
        ...
        FF_LOGD("get power regulator success");
    } else if (ff_ctx->b_use_pinctrl) {
        FF_LOGD("get power pinctrl success");
    } else {
        if (gpio_is_valid(ff_ctx->vdd_gpio)) {
            ...
            FF_LOGD("get power gpio(%d) success", ff_ctx->vdd_gpio);
        }
    }
...
static int ff_register_irq(ff_context_t *ff_ctx)
{
    ...
    FF_LOGD("irq_gpio:%d,irq_num:%d", ff_ctx->irq_gpio, ff_ctx->irq_num);
    if (np && ff_ctx->b_use_pinctrl && (ff_ctx->irq_num <= 0)) {
        ff_ctx->irq_num = irq_of_parse_and_map(np, 0);
    }

    if (ff_ctx->irq_num > 0) {
        FF_LOGD("irq_num:%d", ff_ctx->irq_num);
...
#endif
        FF_LOGD("notifier,event:%lu,blank:%d", event, blank_value);
...
static int fts_notifier_callback_init(ff_context_t *ff_ctx)
{
    ...
    FF_LOGD("'%s' enter.", __func__);
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
    FF_LOGD("init notifier with mtk_disp_notifier_register");
...
#elif IS_ENABLED(CONFIG_FB)
    FF_LOGD("init notifier with fb_register_client");
    ff_ctx->fb_notif.notifier_call = fb_notifier_callback;
...
static int drm_check_dt(ff_context_t *ff_ctx)
{
    ...
    if (ff_ctx && ff_ctx->pdev->dev.of_node) {
        ...
        for (i = 0; i < count; i++) {
            ...
            of_node_put(node);
            if (!IS_ERR(panel)) {
                FF_LOGD("find drm_panel successfully");
                active_panel = panel;
                return 0;
            }
        }
...
#if IS_ENABLED(CONFIG_DRM_PANEL)
    FF_LOGD("init notifier with drm_panel_notifier_register");
...
#else
    FF_LOGD("init notifier with msm_drm_register_client");
...
static int ff_probe(struct platform_device *pdev)
{
    ...
    FF_LOGD("'%s' ff driver probe succeeds.", __func__);
    return 0;
