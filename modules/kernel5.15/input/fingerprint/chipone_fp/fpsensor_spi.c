...
static int fpsensor_get_gpio_dts_info(fpsensor_data_t *fpsensor)
{
    ...
    fpsensor_debug("fpsensor_get_gpio_dts_info entry\n");
	...
	fpsensor_debug("fpsensor_get_gpio_dts_info irq_gpio = %d\n", fpsensor->irq_gpio);
	fpsensor_debug("fpsensor_get_gpio_dts_info reset_gpio = %d\n", fpsensor->reset_gpio);
...
static irqreturn_t fpsensor_irq(int irq, void *handle)
{
    ...
    ** since this is interrupt context (other thread...) */
    smp_rmb();
#if FPSENSOR_WAKEUP_SOURCE && CHIPONE_NF7312BL_A1
	__pm_wakeup_event(fpsensor_dev->ttw_wl, 1000);	
#elif FPSENSOR_WAKEUP_SOURCE 
    ...
    case FPSENSOR_IOC_REMOVE:
	  ...
	  //wakeup_source_trash(&fpsensor_dev->ttw_wl);
		if (g_fpsensor->ttw_wl) {
			wakeup_source_unregister(g_fpsensor->ttw_wl);
			g_fpsensor->ttw_wl = NULL;
		}
      #else
      wake_lock_destroy(&fpsensor_dev->ttw_wl);
       #endif
...
static int fpsensor_dev_setup(fpsensor_data_t *fpsensor)
{
    ...
    fpsensor_debug("fpsensor_dev_setup entry\n");
...
static int fpsensor_probe(struct platform_device *pdev)
{
    ...
    fpsensor_debug("fpsensor_probe entry\n");
...

	{
		...
		#if defined(ZCFG_FS170_TRX_HAIFEI_NT7337)
			zyt_info_s2("[FP] : ","Huate:NT7337");	
        #elif defined(ZCFG_FS311L6_TRX_CHIPONE_ICNF7318)
            zyt_info_s2("[FP] : ","ICNF7318");
        #elif defined(ZCFG_FS322L6_TRX_DISPLAY_CHANGE)
            zyt_info_s2("[FP] : ","ICNF7316");
		#else
			zyt_info_s2("[FP] ","chipone");
		#endif
	}
