...
//udc=0x93000000, 0xcb0, 0x2
static int  udc_start()
{ 
    uint32_t i = 0; 
	...
	printk("%s: cmdline =%s\n", __func__,cmd_line);
    str = strstr(cmd_line, "udc=");
    str += sizeof("udc=")-1;
	printk("%s: line= %d  ,str=%s\n", __func__, __LINE__,str); 
	...
	printk("udc_base = 0x%x, udc_size = 0x%x, udc_lcd_offset = 0x%x\n", udc_base, udc_size, udc_lcd_offset);
	return 1;
}
//__setup("udc=", udc_start);

/*
static int get_udc_size(char *udc_size)
{
    struct device_node *np;
    const char *cmd_line;
    char *s = NULL;
...
{
	udc_s* udc = &g_udc;
//	char *s = kmalloc(sizeof(char)*64,GFP_KERNEL);
//	if(s==NULL){
