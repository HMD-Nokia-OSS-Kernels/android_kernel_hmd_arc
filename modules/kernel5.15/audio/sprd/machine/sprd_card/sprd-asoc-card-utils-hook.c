...
#if defined(CONFIG_SND_SOC_GPIO_SPK_RCV_HPR)
...
#if defined(CONFIG_SND_SOC_I2C_SPK_RCV_2IN1)
static int spk_on = 0;
static int rcv_on = 0;
#endif
...
#if defined(CONFIG_SND_SOC_AW87XXX_V2)
#if defined(CONFIG_SND_SOC_I2C_SPK_RCV_2IN1)
extern int aw87xxx_audio_scenne_load_spk_rcv(uint8_t dev_type,uint8_t mode,uint8_t channel);
#else
extern int aw87xxx_audio_scenne_load_v2(uint8_t mode, int32_t channel);
#endif
#else
...
static int hook_general_spk(int id, int on)
{
	...
	if (on){
		#if defined(CONFIG_SND_SOC_I2C_SPK_RCV_2IN1)
			pr_info("%s open_spk 0+1",__func__);
			aw87xxx_audio_scenne_load_spk_rcv(0,1,1);
			spk_on = 1;
		#else
			pr_info("%s open_spk 0+1,aw87xxx_audio_scenne_load_v2",__func__);
			aw87xxx_audio_scenne_load_v2(1,1);
		#endif
	}else{
		#if defined(CONFIG_SND_SOC_I2C_SPK_RCV_2IN1)
			if (rcv_on == 1){
		            pr_info("%s close_spk 0",__func__);
				aw87xxx_audio_scenne_load_spk_rcv(0,0,0);
			 }else{
				aw87xxx_audio_scenne_load_spk_rcv(0,0,1);
			}
			spk_on = 0;
		#else
			aw87xxx_audio_scenne_load_v2(0,0);
		#endif
	}
  ...
  if (on){
		pr_info("%s open_spk 0+1,aw87xxx_audio_scenne_load",__func__);
		aw87xxx_audio_scenne_load(1,1);
		aw87xxx_audio_scenne_load(1,0);
  }else{
		aw87xxx_audio_scenne_load(0,0);
		aw87xxx_audio_scenne_load(0,1);
  }
	msleep(22);
	...
	return HOOK_OK;
#else   /*ZCFG_CONFIG_SND_SOC_AW87XXX_V2*/
	int gpio, mode;
#if 0
...
#if defined(CONFIG_SND_SOC_I2C_SPK_RCV_2IN1)
static int hook_general_rcv(int id, int on)
{

	pr_info("%s id: %d, on: %d\n",
		 __func__, id, on);
	if (on){
		pr_info("%s open_rcv 1",__func__);
		aw87xxx_audio_scenne_load_spk_rcv(2,4,1);
		rcv_on = 1;
  	} else {
		if (spk_on == 1) {
			pr_info("%s close_rcv null",__func__);
		} else {
			pr_info("%s close_rcv 1",__func__);
			aw87xxx_audio_scenne_load_spk_rcv(2,0,1);
		}
		rcv_on = 0;
 	}
	msleep(22);
...
#if defined(CONFIG_SND_SOC_I2C_SPK_RCV_2IN1)
	{0},
	{"general_rcv", hook_general_rcv, EN_LEVEL},
#endif
};
...
#if defined(CONFIG_SND_SOC_AW87XXX)||defined(CONFIG_SND_SOC_AW87XXX_V2)
	pr_info("%s hook i2c amp directly\n", __func__);
	ext_hook->ext_ctrl[BOARD_FUNC_SPK] = ext_hook_arr[BOARD_FUNC_SPK].hook;
	#if defined(CONFIG_SND_SOC_I2C_SPK_RCV_2IN1)
	ext_hook->ext_ctrl[BOARD_FUNC_EAR] = ext_hook_arr[BOARD_FUNC_EAR].hook;
	#endif
	return 0;
#endif
...
#if defined(CONFIG_SND_SOC_GPIO_SPK_RCV_HPR)
		hook_spk_priv.rcv_shared_multi_spk = of_property_read_bool(np, 
		"sprd,rcv_shared_multi_spk");
