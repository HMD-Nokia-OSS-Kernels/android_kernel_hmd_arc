...
#if 1
static char *aw_profile[] = {"Music", "Off","Receiver"};/*aw87xxx_acf.bin Œƒº˛≈‰÷√≥°æ∞*/
int aw87xxx_set_profile(int dev_index, char *profile);
char *aw87xxx_show_current_profile(int dev_index);

enum aw87xxx_dev_index {AW_DEV_0 = 0,AW_DEV_1};

int aw87xxx_audio_scenne_load_v2(uint8_t mode, int32_t channel)
{
	int ret = 0;
	if(mode == AW87XXX_MUSIC_MODE){
	ret = aw87xxx_set_profile(AW_DEV_0, aw_profile[0]);
		}
	else if(mode == AW87XXX_OFF_MODE){
...
EXPORT_SYMBOL_GPL(aw87xxx_audio_scenne_load_v2);

int aw87xxx_audio_scenne_load_spk_rcv(uint8_t dev_type,uint8_t mode,uint8_t channel)
{
	int ret = 0;
...
EXPORT_SYMBOL_GPL(aw87xxx_audio_scenne_load_spk_rcv);
