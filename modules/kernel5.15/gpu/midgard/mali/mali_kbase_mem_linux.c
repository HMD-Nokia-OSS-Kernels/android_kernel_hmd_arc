...
{
#else
static vm_fault_t kbase_cpu_vm_fault(struct vm_fault *vmf)      //SPRD k54
{
	...
	if (fault_pgoff >= map->alloc->nents)
	...
	while (i < map->alloc->nents && (addr < vma->vm_end >> PAGE_SHIFT)) {
		int ret = vmf_insert_pfn_prot(vma, addr << PAGE_SHIFT,
		    PFN_DOWN(as_phys_addr_t(map->alloc->pages[i])),vma->vm_page_prot);  //SPRD k54
		if (ret < 0 && ret != -EBUSY)
			goto locked_bad_fault;
...
//	        dev_warn(kctx->kbdev->dev,
//                 "lu.xue kbase_cpu_mmap addr is %lld,pgprot is %lld.",
//                 addr, PFN_DOWN(phys));
//			err = vmf_insert_pfn(vma, addr, PFN_DOWN(phys));
//            dev_warn(kctx->kbdev->dev,
//                 "err=vmf_insert_pfn is %d",
//                  err);
			if (WARN_ON(err))
				break;
