...
#include "firmware/tp_list.h"
//#include <gslx680.h>

#ifdef SENSOR_HUB_PROX
...
#define TS_DEBUG_MSG
#ifdef TS_DEBUG_MSG
#define GSL168X_DBG(format, ...)	printk(KERN_INFO "[GSL168X] " format, ## __VA_ARGS__)
#else
...
#if defined(JZHK_CTP_J517_BOPAI_P010_FWVGA_DC_AND_HSD) || defined(JZHK_CTP_J517_BOPAI_P001_QHD_480_960)
static unsigned int *gsl_config_data_id = gsl_config_data_id_DC;
#else
...
#else
static int SCREEN_MAX_Y	= 1280;
#endif

// tj copy from rk ts driver @{
int xy_swap = 0;
int x_reverse = 0;
int y_reverse = 0;
int xy_deal = 0;
int xy_deal_minor = 0;
int sf_hwrotation = 0;
int ata_hwrotation = 0;
int max_x = 0;
int max_y = 0;
static const struct fw_data *ptr_fw = NULL;
unsigned int *gsl_config_data_id = NULL;
u32 ptr_fw_len;

// static int check_is_factorytest(void)
// {
//     #define BOOT_MODE_KEY "androidboot.mode="
// 	char strval[]="factorytest";
//     extern char *saved_command_line;
//     int val = 0;
//     char *command_line = strstr(saved_command_line, BOOT_MODE_KEY);
//     //char ch[12];
//     if (command_line == NULL) {
//         pr_info("%s : Can not get %s from kernel command line!\n",BOOT_MODE_KEY, __func__);
//         return 0;
//     } 	
//     command_line += strlen(BOOT_MODE_KEY);	
//     //strncpy(ch,command_line,11);
// 	//pr_info("%s: cmdline: %s\n",__func__,ch);
//     if(0 == strncmp(command_line,strval,11)){
//         val = 1;
// 	}
// 	else{
// 		printk("check_is_factorytest: cmdline != factorytest\n");
//     }
//     printk("check_is_factorytest: val=%d, cmdline=%s\n", val, command_line);
//     return val;
// }

static inline int control_input_orientation(u16 *x, u16 *y, int max_x, int max_y)
{
	int tpd_x_res=max_x, tpd_y_res=max_y;
	u16 z;
	if(1 == xy_swap){
		z=*y; *y=*x; *x=z;
	}
...
static int gslX680_sysfs_init(struct i2c_client *client);
//static int ts_filesys_create(struct i2c_client *client);
...
#ifdef TOUCH_MODUEL_GSL3670_SF960ASA_GY_P10068A_01
	{KEY_HOMEPAGE, 580, 650, 1930, 2020},
#else
	{KEY_HOMEPAGE, 580, 650, 1930, 2020},
#endif	
...
#if defined (JZHK_CTP_J517_BOPAI_P010_FWVGA_DC_AND_HSD)
    #define CTP_BUTTON_KEY_Y   1200
    #define MENU_CTP_BUTTON_X  300
    #define HOME_CTP_BUTTON_X  200
    #define BACK_CTP_BUTTON_X  100 
#elif defined (JZHK_CTP_J106_31E_TAIYI_SP111A)
    #define CTP_BUTTON_KEY_Y   1000
    #define MENU_CTP_BUTTON_X  300
    #define HOME_CTP_BUTTON_X  200
    #define BACK_CTP_BUTTON_X  100 	
#elif defined (JZHK_CTP_J106_31E_TAIYI_SP111B_HD720)
    #define CTP_BUTTON_KEY_Y   1800
    #define MENU_CTP_BUTTON_X  450
    #define HOME_CTP_BUTTON_X  250
    #define BACK_CTP_BUTTON_X  100 	
#elif defined (JZHK_CTP_J106_32E_TAIYI_SP111)
    #define CTP_BUTTON_KEY_Y   1000
...
#define BUTTON_WIDTH 40 // 65
#define BUTTON_HEIGHT 40 // 45
extern void gsl_FunIICRead(unsigned int (*fun) (unsigned int *,unsigned int,unsigned int));
				...
				unsigned int addr,unsigned int len)
{
	...
	GSL168X_DBG(" ammygsl_read_oneframe_data, data[%d] = 0x%08x\n", i, data[i]);
...
//	const struct fw_data *ptr_fw;
	...
	pr_info("=============gsl_load_fw start 11111 ==============\n");
	...
	for (source_line = 0; source_line < source_len; source_line++) 
	{
		if(1 == DMA_TRANS_LEN)
		{
			...
			buf[1] = (u8)((ptr_fw[source_line].val & 0x0000ff00) >> 8);
			buf[2] = (u8)((ptr_fw[source_line].val & 0x00ff0000) >> 16);
			buf[3] = (u8)((ptr_fw[source_line].val & 0xff000000) >> 24);
		...
		{
			if (1 == send_flag % (DMA_TRANS_LEN < 0x08 ? DMA_TRANS_LEN : 0x08))
				reg = ptr_fw[source_line].offset;

			buf[cur + 0] = (u8)(ptr_fw[source_line].val & 0x000000ff);
			buf[cur + 1] = (u8)((ptr_fw[source_line].val & 0x0000ff00) >> 8);
			buf[cur + 2] = (u8)((ptr_fw[source_line].val & 0x00ff0000) >> 16);
			buf[cur + 3] = (u8)((ptr_fw[source_line].val & 0xff000000) >> 24);
			cur += 4;

			if (0 == send_flag % (DMA_TRANS_LEN < 0x08 ? DMA_TRANS_LEN : 0x08)) 
			{
				i2c_smbus_write_i2c_block_data(client, reg, DMA_TRANS_LEN*4, buf); 	
				cur = 0;
			}
...
#endif
static struct proc_ops tp_ps_fops = {
	//.proc_opsowner			= THIS_MODULE,
	.proc_open			= tp_ps_open,
	.proc_release		= tp_ps_release,
    .proc_unlocked_ioctl = tp_ps_ioctl,
};
...
#ifdef SENSOR_HUB_PROX
        if (1 == dps_data){
            printk ("gslx680 proximity near\n");
            shub_report_proximity_event(0x40a00000);// 1;  near
        }else if (0 == dps_data){
            printk ("gslx680 proximity far-away\n");
            shub_report_proximity_event(0);// 0;  far-away
        }
#else
...
#elif defined(JZHK_CTP_J517_BOPAI_P010_FWVGA_DC_AND_HSD)
#define GSL_C		100
#define GSL_CHIP_1	0xff807801//0xfff07801 //DC 
#define GSL_CHIP_2	0xff807807//0xffeaa500 //HSD  
#elif defined(JZHK_CTP_J517_BOPAI_P001_QHD_480_960)
#define GSL_C		100
#define GSL_CHIP_1	0xff80f801//DC 
...
/*
	...
	/*
		...
		*/
		if(tmp == tmp1){
#if defined(JZHK_CTP_J517_BOPAI_P010_FWVGA_DC_AND_HSD) || defined(JZHK_CTP_J517_BOPAI_P001_QHD_480_960)
			gsl_config_data_id = gsl_config_data_id_DC;
#else
...
// 	y = SCREEN_MAX_Y-y;
// #endif
	control_input_orientation(&x, &y, max_x, max_y);
	...
	if(gsl_gesture_flag==1 && GE_NOWORK == gsl_gesture_status){
		goto schedule;
				...
				if(gsl_gesture_status==GE_ENABLE&&gsl_gesture_flag==1){
					...
					__pm_wakeup_event(tp_wakelock, msecs_to_jiffies(2000));
...
//		int flag = 0;	
        ...
        case 0xa1fb:
            ...
            break;
        case 0xa1fc:            
			key_data = KEY_NEXTSONG;
            tmp_c = 'U';
            break;
        case 0xa1fd:
			key_data = KEY_PREVIOUSSONG;			
            tmp_c = 'D';
            break;
        case '*':
            key_data = KEY_POWER;
		...
		//	gsl_irq_mode_change(this_client,0);
			...
			__pm_wakeup_event(tp_wakelock, msecs_to_jiffies(200));
			power_key_status = 1;
			gesture_flag_lock = 1;
...
static void gsl_monitor_worker(struct work_struct *work)
{
	...
	if(i2c_lock_flag != 0)
			...
			i2c_lock_flag = 1;
//GSL168X_DBG("----------------gsl_monitor_worker esd start-----------------\n");		
...
static void gsl_enter_doze(struct i2c_client *client)
{
	...
	printk("gsl_enter_doze:  gsl_gesture_status=%d\n",gsl_gesture_status);
...
//	u32 tmp;
	...
	printk("gsl_quit_doze:  gsl_gesture_status=%d\n",gsl_gesture_status);
	gslx680_hw_reset();
...
static unsigned int gsl_gesture_init(struct i2c_client *client)
{
	int ret;
	
	ret = sysfs_create_file(&client->dev.kobj, &dev_attr_tpgesture.attr);
	...
	ret = sysfs_create_file(&client->dev.kobj, &dev_attr_tpgesture_func.attr);
	//ret = device_create_file(gsl_debug_kobj, &dev_attr_tpgesture);
    if (ret)
    {
        printk("%s: sysfs_create_version_file failed\n", __func__);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static int gslX680_ts_suspend(struct early_suspend *handler)
#endif
static int gslX680_ts_suspend(void)
{
#if 1
	...
	int ret;
#endif
	printk("==gslX680_ts_suspend----start=\n");
	//tp_ps_enable();
#ifdef USE_TP_PSENSOR
		if (ps_en == 1)
		{
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static int gslX680_ts_resume(struct early_suspend *handler)
#endif
static int gslX680_ts_resume(void)
{
#if 1
...
#endif
	gsl_halt_flag=0;
	printk("--zxw--==gslX680_ts_resume start=\n");
...
// tj copy from rk @{
	of_property_read_u32(np, "xy_swap", &xy_swap);
	of_property_read_u32(np, "x_reverse", &x_reverse);
	of_property_read_u32(np, "y_reverse", &y_reverse);
	of_property_read_u32(np, "xy_deal", &xy_deal);
	of_property_read_u32(np, "xy_deal_minor", &xy_deal_minor);
	of_property_read_u32(np, "sf_hwrotation", &sf_hwrotation);
	of_property_read_u32(np, "max_x", &max_x);
	of_property_read_u32(np, "max_y", &max_y);	
	//HQ add factorytest rotation @{
	ret = of_property_read_u32(np, "ata_hwrotation", &ata_hwrotation);	
	if(ret){
		dev_err(dev, "fail to get ata_hwrotation\n");
	}
	else{
		// if(1 == check_is_factorytest()  ){
		// 	if( ata_hwrotation == 90 ){
		// 		xy_swap = !xy_swap;
		// 		y_reverse = !y_reverse;		
		// 	}
		// 	if( ata_hwrotation == 270){
		// 		xy_swap = !xy_swap;
		// 		x_reverse = !x_reverse;
		// 	}
		// 	printk("tp: ata_hwrotation = %d\n",ata_hwrotation);
		// 	swap(max_x, max_y);
		// }
	}
	//@}
	if(sf_hwrotation == 90 || sf_hwrotation == 270){
		swap(max_x, max_y);
	}
	if(max_x > 0) {
		#ifdef ZCFG_LCM_WIDTH
			SCREEN_MAX_X = ZCFG_LCM_WIDTH;
		#else
			SCREEN_MAX_X = max_x;
		#endif
		#ifdef ZCFG_LCM_HEIGHT
			SCREEN_MAX_Y =  ZCFG_LCM_HEIGHT;
		#else
			SCREEN_MAX_Y = max_y;
		#endif
	}else {
	...
	of_property_read_string(np, "gsl_name", &gsl_name);
	if(gsl_name == NULL){
		GSL168X_DBG("---zxw-->%s,gsl_name FAIL..\n",__func__);
		return NULL;
	}
	//if(gsl_name)
	{
		dev_info(dev, "gsl_name=%s", gsl_name);
	
		for(i=0; i< ARRAY_SIZE(all_tps); ++i){
			if(strcmp(gsl_name, all_tps[i].name)==0){
				break;
			}
		}
	}
	if(i >= ARRAY_SIZE(all_tps)) i = 0;
	dev_info(dev, "gsl_name=%s, choose=%d", gsl_name, i);
	ptr_fw = all_tps[i].fw;
	ptr_fw_len = all_tps[i].fw_len;
	gsl_config_data_id = all_tps[i].config;
// @}
...
static int gslX680_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GSL168X_DBG("--gslX680---==kzalloc=ok\n");
	...
	GSL168X_DBG("I2C addr=%x\n", client->addr);

	INIT_WORK(&gslX680_ts->pen_event_work, gslX680_ts_worker);
	GSL168X_DBG("gslX680 INIT_WORK OK\n");
...
//	input_set_capability(input_dev, EV_KEY, REPORT_KEY_VALUE);	
	//wxt
    input_set_capability(input_dev, EV_KEY, KEY_POWER);
    input_set_capability(input_dev, EV_KEY, KEY_MAIL);
    input_set_capability(input_dev, EV_KEY, KEY_CAMERA);
    input_set_capability(input_dev, EV_KEY, KEY_WWW);
...
//	gsl_GestureExternInt(gsl_model_extern,sizeof(gsl_model_extern)/sizeof(unsigned int)/18);
	...
	//ts_filesys_create(client);
	gslX680_sysfs_init(client);
	...
	tp_wakelock = wakeup_source_create("tp_input_wakelock");
	wakeup_source_add(tp_wakelock);	
#endif
...

	{
		...
		zyt_info_s2("[TP] : ", GSLX680_TS_NAME);
	}
	msleep(30);	
	...
	printk("gslx680 gslX680_ts_probe ok");
...
static int gslX680_ts_remove(struct i2c_client *client)
{
	...
	wakeup_source_destroy(tp_wakelock);
#endif
...
static int ts_filesys_create(struct i2c_client *client)
{
  ...
  if (!tp_ctrl_kobj){
      dev_err(&client->dev,"Create ts_sysfs_init failed!\n");
      return -ENOMEM;
  }
  return sysfs_create_group(tp_ctrl_kobj, &ts_attr_group);
}
#endif
static ssize_t ts_suspend_show(struct device *dev,struct device_attribute *attr, char *buf)
{
...
static int gslX680_sysfs_init(struct i2c_client *client)
{
	...
	ret = sysfs_create_file(&client->dev.kobj, &dev_attr_ts_suspend.attr);
	if (ret < 0) {
		printk("Fail to create debug files!");
		return -ENOMEM;
	}
