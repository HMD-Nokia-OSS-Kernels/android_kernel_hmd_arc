...
int himax_parse_dt(struct himax_ts_data *ts, struct himax_platform_data *pdata)
{
	...
	if (prop) {
		...
		pdata->abs_x_min = 0;
		pdata->abs_x_max = ZCFG_LCM_WIDTH -1;
		pdata->abs_y_min = 0;
		pdata->abs_y_max = ZCFG_LCM_HEIGHT -1;
...
#if defined(HX_CONFIG_DRM) || defined(HX_CONFIG_FB) || defined(HX_CONFIG_PM)
...
{
	return scnprintf(buf, PAGE_SIZE, "himax-touchscreen\n");
}
