...
int fts_wait_tp_to_valid(void)
{
    ...
    do {
		...
		FTS_INFO("zjx TP Ready,chip_idh:0x%02x,idh :0x%02x,idl :0x%02x", chip_idh,idh,idl);
...
{
	struct ts_resume_worker *resume =
			container_of(work, struct ts_resume_worker, resume_kwork);
	struct  fts_ts_data *ts_data =
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	}//modify touchscreen suspend/resume
