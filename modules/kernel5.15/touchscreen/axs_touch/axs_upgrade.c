...
#define AXS_UPGRADE_FILE                ZCFG_AXS_DOWNLOAD_FIRMWARE_FILE_TYPE_I
#else
#define AXS_UPGRADE_FILE                "firmware/firmware_flash.i"
#endif

#define FW_BIN_UPGRADE_NAME "axs_pram.bin"
...
#if AXS_BUS_SPI
	if (request_firmware(&fw, FW_BIN_UPGRADE_NAME, &g_axs_data->spi->dev)) 
    {
...
#endif
#if AXS_BUS_IIC
	if (request_firmware(&fw, FW_BIN_UPGRADE_NAME, &g_axs_data->client->dev)) 
    {
		AXS_ERROR("firmware read failed");
        return false;
	}
...
bool flash_read_init(void)
{
    ...
    if (ret < 0)
    {
        AXS_DEBUG("flash_read_init fail");
		return false;
    }	
	msleep(2);
	return true;
