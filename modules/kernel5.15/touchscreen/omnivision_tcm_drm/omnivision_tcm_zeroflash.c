...
static int zeroflash_get_fw_image(void)
{
	...
	if (!rc) {
			...
			pr_info("can't not parse bootargs property\n");
	}
#endif
	...
	while(retry_cnt--) {
#ifdef FS327L6_FIRM_COMPAT
		if (strcmp(lcd_name, "lcd_td4160_mipi_FS327L6_KC_HX") == 0)
		{
			retval = request_firmware(&zeroflash_hcd->fw_entry,
				"hdl_firmware_huaxian.img",
				tcm_hcd->pdev->dev.parent);
			pr_info("find lcd name: %s\n", lcd_name);
		}
		else if(strcmp(lcd_name, "lcd_td4160_mipi_FS327L6_KC") == 0)
		{
			retval = request_firmware(&zeroflash_hcd->fw_entry,
				"hdl_firmware_dzx.img",
				tcm_hcd->pdev->dev.parent);
			pr_info("find lcd name: %s\n", lcd_name);
		}
		else
		{
			pr_info("no find lcd name: %s\n", lcd_name);
		}
#else
		retval = request_firmware(&zeroflash_hcd->fw_entry,
			FW_IMAGE_NAME,
