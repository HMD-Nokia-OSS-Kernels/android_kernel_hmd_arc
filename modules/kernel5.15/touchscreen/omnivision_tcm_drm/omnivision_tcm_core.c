...
{
	uint8_t omnivisionName[25];
	strcpy(omnivisionName, TOUCH_INPUT_NAME);
	strcat(omnivisionName, "\n");
	return scnprintf(buf, PAGE_SIZE, omnivisionName);
}
...
static struct attribute *omnivision_dev_name_atts[] = {
		&dev_attr_name.attr,
		NULL
	};

static const struct attribute_group omnivision_dev_name_atts_group = {
    .attrs = omnivision_dev_name_atts,
};
static const struct attribute_group *omnivision_dev_attr_groups[] = {
	&omnivision_dev_name_atts_group,
	NULL
};
int omnivision_sysfs_add_device(struct device *dev)
{
	int retval = 0, idx;
	struct ovt_tcm_hcd *tcm_hcd; 
	tcm_hcd = g_tcm_hcd; 
for (idx = 0; omnivision_dev_attr_groups[idx]; idx++) {
retval = sysfs_create_group(&tcm_hcd->pdev->dev.kobj,omnivision_dev_attr_groups[idx]);
	if (retval) {
		while (--idx >= 0) {
		sysfs_remove_group(&tcm_hcd->pdev->dev.kobj,omnivision_dev_attr_groups[idx]);
		}
		break;
	}
	}
...
static int ovt_tcm_identify(struct ovt_tcm_hcd *tcm_hcd, bool id)
{
	...
	/* 9988 display */
	{
		if (!zyt_tp_info_display) { 
			char tp_fm_ver[12] = {0};
			
			extern void zyt_info_s2(char* s1,char* s2);
			sprintf(tp_fm_ver,"%d",tcm_hcd->packrat_number);
			
#ifdef ZCFG_FS322L6_TRX_DISPLAY_CHANGE
			zyt_info_s2("[TP] : ","td4160");
			zyt_info_s2("[Td4160] : FW Version:",tp_fm_ver);
#else
			zyt_info_s2("[TP] : ","omnivision");
			zyt_info_s2("[Omnivision] : FW Version:",tp_fm_ver);
#endif
			zyt_tp_info_display = 1;
		}
	}
...
//#endif
	...
	retval = omnivision_sysfs_add_device(&tcm_hcd->pdev->dev);
	if (sysfs_create_link(NULL, &tcm_hcd->pdev->dev.kobj, "touchscreen") < 0)
