...
int file_write(struct file_buffer *file, bool new_open)
{
	...
	ILI_ERR("GKI version not allow drivers to use filp_open_block\n");
	return -1;
#else
	struct file *f = NULL;
	...
	if (new_open)
		f = filp_open_block(file->fname, O_WRONLY | O_CREAT | O_TRUNC, 644);
	else
		f = filp_open_block(file->fname, O_WRONLY | O_CREAT | O_APPEND, 644);
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ILI_INFO("ioctl: glove \n");
		if (data[1] == 0x00) {			
			if (ili_ic_func_ctrl("glove", 0x00) < 0) {//glove close
				ILI_ERR("Write glove cmd failed\n");
			} 
			ILI_INFO("ioctl: glove mode closed\n");				
		} else {
			if (ili_ic_func_ctrl("glove", 0x01) < 0) {//glove open
					ILI_ERR("Write glove cmd failed\n");
			}
			ILI_INFO("ioctl: glove mode open\n");			
		}
