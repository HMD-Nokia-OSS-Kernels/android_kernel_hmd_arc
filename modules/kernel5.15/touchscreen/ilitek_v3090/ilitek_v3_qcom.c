...
{
	return scnprintf(buf, PAGE_SIZE, "ILITEK_TDDI\n");
}
...
static struct attribute *ilitek_dev_name_atts[] = {
		&dev_attr_name.attr,
		NULL
	};

static const struct attribute_group ilitek_dev_name_atts_group = {
    .attrs = ilitek_dev_name_atts,
};
...
static int ilitek_plat_probe(void)
{
...

	{
		...
		sprintf(tp_id,"%4x",ilits->chip->id);
		tp_id[4]='\0';
		
		sprintf(tp_fm_ver,"0x%02d",(ilits->chip->fw_ver >> 8) & 0xFF);
		tp_fm_ver[10]='\0';
		
		zyt_info_s2("[TP] : ilitek",tp_id);
		zyt_info_s2("[ILITEK] : FW Version:",tp_fm_ver);
	}
