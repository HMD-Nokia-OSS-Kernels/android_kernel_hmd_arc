...
bool tp_suspend_enter = false;
bool cts_show_debug_log = false;
#if defined(CFG_CTS_PROXIMITY_SHUB) || defined(CFG_CTS_PROXIMITY)
extern bool cts_phone_switch;
#endif
...
#ifdef ZCFG_CTP_DISABLE_AVDD_AVEE
//extern void set_ctpstatus(int stu);
static int g_proximity_device_status = 0;//device
static int g_proximity_calling_status = 0;//device

void set_proximity_calling_device(int stu)
{
	g_proximity_calling_status = stu;
}
int get_proximity_calling_device(void)
{
	return g_proximity_calling_status;
}

void set_proximity_device(int stu)
{
	g_proximity_device_status = stu;
}
int get_proximity_device(void)
{
	return g_proximity_device_status;
}
#endif

#if defined(CFG_CTS_PROXIMITY_SHUB)
extern struct device * psensor_dev;
extern struct input_dev *ps_input_dev;
extern int cts_prox_ctrl(bool enable);

int shub_report_proximity_flush_event(u32 value)
{
  s64 k_timestamp;
...
void input_dev_common()
{
	struct device * psensor_input_dev = get_cts_device();
	
	if(device_create_file(psensor_input_dev, &dev_attr_psensor_enable) < 0) // /sys/class/sprd_sensorhub_tp/device/psensor_enable
		printk("Failed to create device file(%s)!\n", dev_attr_psensor_enable.attr.name);

	if(device_create_file(psensor_input_dev, &dev_attr_psensor_flush) < 0) // /sys/class/sprd_sensorhub_tp/device/psensor_enable
		printk("Failed to create device file(%s)!\n", dev_attr_psensor_enable.attr.name);
...
{
#if 0 
	cts_info("cts_phone_switch = %d", cts_phone_switch);
//defined(CFG_CTS_PROXIMITY_SHUB) || defined(CFG_CTS_PROXIMITY)
	if(cts_phone_switch)
		return count;
#endif
	if ((buf[0] == '1') && !tp_suspend_enter){
	count = cts_driver_suspend(dev_get_drvdata(dev));
	}
	else if ((buf[0] == '0') && tp_suspend_enter){
	count = cts_driver_resume(dev_get_drvdata(dev));
	}
#if defined(CFG_CTS_PROXIMITY_SHUB) || defined(CFG_CTS_PROXIMITY)	
	else{
		cts_info("[ts_suspend_store] cts_phone_switch = %d,failed,pls check params...", cts_phone_switch);
	}
#endif
	return count;
}

static ssize_t ts_name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "touchscreen\n");
}

static ssize_t ts_name_store(struct device *dev, struct device_attribute *attr, const char *buf,
				size_t count)
{
	return count;
}
static DEVICE_ATTR_RW(ts_suspend);
static DEVICE_ATTR_RW(ts_name);
...
int cts_driver_resume(struct chipone_ts_data *cts_data)
{
    int ret;
#ifdef ZCFG_CTP_DISABLE_AVDD_AVEE	
	int status = get_proximity_device();
	int calling_status = get_proximity_calling_device();
#endif
    cts_info("Resume");
	tp_suspend_enter = false;
#if defined(CFG_CTS_PROXIMITY_SHUB) || defined(CFG_CTS_PROXIMITY)
	 if(cts_phone_switch && (real_suspend_flag == 0)){
        //if(cts_phone_switch){
		 real_suspend_flag = 0;
#ifdef ZCFG_USE_INCELL_CTP		
		//reset face-mode,or report far-way always.
		//if(calling_status==1)
		//{
			cts_prox_ctrl(1);
		//}
		//cts_info("[cts_driver_resume] status:0x%x,calling_status:0x%x.",status,calling_status);
#endif	
#if defined(ZCFG_USE_INCELL_CTP)
		incellctp_set_sprd_panel_phone_call_flag(1);
		 return 0;
...
#ifdef ZCFG_CTP_DISABLE_AVDD_AVEE
	status = get_proximity_device();
	calling_status = get_proximity_calling_device();

	if(status != 0)
	{
		//cts_info("Resume restart:%s",status==PROXIMITY_DEVICE_EIO_ON?"on PROXIMITY_DEVICE":"off PROXIMITY_DEVICE");
		if(status == PROXIMITY_DEVICE_EIO_ON)
		{
			cts_info("Resume --PROXIMITY_DEVICE_EIO_ON:%d",PROXIMITY_DEVICE_EIO_ON);
			cts_prox_ctrl(1);
			set_proximity_device(0);
		}
		else if(status == PROXIMITY_DEVICE_EIO_OFF)//PROXIMITY_DEVICE_EIO_OFF
		{
			cts_info("Resume --PROXIMITY_DEVICE_EIO_OFF:%d",PROXIMITY_DEVICE_EIO_OFF);
			cts_prox_ctrl(1);
			set_proximity_device(0);
		}
		cts_info("Resume --status:0x%x,calling_status:0x%x.",status,calling_status);
	}
	else
	{
		cts_info("[power][to]Resume ++status:0x%x,calling_status:0x%x.",status,calling_status);
		if(calling_status==1)
		{
			cts_prox_ctrl(1);
            #if defined(ZCFG_USE_INCELL_CTP)
		...
		{
			cts_info("[power][to]Resume to reset power-off. calling_status:0x%x.",calling_status);
			//set_ctpstatus(0);//yes to power-off
            #if defined(ZCFG_USE_INCELL_CTP)
            incellctp_set_sprd_panel_phone_call_flag(0);
            #endif
		}
...
int cts_driver_probe(struct device *device, enum cts_bus_type bus_type)
{
    ...
    ret=sysfs_create_group(&cts_data->device->kobj, &cts_group);
	if (ret < 0) {
		cts_err("Can not create sysfs group!");
	}

	ret = sysfs_create_link(NULL, &cts_data->device->kobj, "touchscreen");
	if (ret < 0) {
		cts_err("Failed to create link!: %d",ret);
...
static struct attribute *cts_attrs[] = {
    &dev_attr_ts_suspend.attr,
    &dev_attr_ts_name.attr,
    NULL
};

const struct attribute_group cts_group = {
    .attrs = cts_attrs,
};
