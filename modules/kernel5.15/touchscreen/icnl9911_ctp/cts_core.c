...
bool cts_palm_status_old = false;
bool cts_palm_status = false;
#ifdef ZCFG_CTP_DISABLE_AVDD_AVEE
//extern void set_ctpstatus(int stu);
#endif
int cts_prox_ctrl(bool enable)
{
    int ret;
	if(NULL == cts_dev_golbal){
		cts_info("cts_dev_golbal is NULL");
		return 0;
	}
    ret = cts_send_command(cts_dev_golbal, enable ? 0xa0 : 0xa1);
    if (ret) {
        cts_info("proximity change state failed %d(%s),cts_phone_switch:%d.", ret, cts_strerror(ret),cts_phone_switch);
        return ret;
    }
    cts_phone_switch = enable;
    cts_info("[PROXIMITY] enable = %d", enable);
    return 1;
}
#endif
...
int shub_report_proximity_event(u32 value)
{
  s64 k_timestamp;
...
int cts_prox_ctrl(bool enable)
{
	...
	if(1 == enable){
		ret = cts_send_command(cts_dev_golbal, 0xa0);
		if (ret) {
	        cts_info("proximity change state failed %d(%s)", ret, cts_strerror(ret));
	        return ret;
	    }
		__pm_stay_awake(&proximity_wake_lock);
		cts_info("cts TP_face_mode_switch %d\n", enable);
		cts_phone_switch = 1;
	}else if(0 == enable){
		__pm_relax(&proximity_wake_lock);
		cts_phone_switch = 0;
		ret = cts_send_command(cts_dev_golbal, 0xa1);
		if (ret) {
       		cts_info("proximity change state failed %d(%s)", ret, cts_strerror(ret));
        	return ret;
   		}
		cts_info("[PROXIMITY_SHUB]cts TP_face_mode_switch %d\n", enable);
	}
	return 0;
}
#endif
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    zyt_info_sx("[ICNL9911] : FW Version:", fwdata->version);
...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if (unlikely(cts_dev->rtdata.suspended)) {
        ...
        if (ret) {
            cts_err("[cts_irq_handler] Get touch info failed %d(%s)",
                ret, cts_strerror(ret));
#ifdef ZCFG_CTP_DISABLE_AVDD_AVEE
			if(ret == PROXIMITY_DEVICE_EIO)
			{
				cts_info("proximity PROXIMITY_DEVICE_EIO.");
				//set_proximity_device(PROXIMITY_DEVICE_EIO_IRQ);
			}
#endif			
            return ret;
        }

        //cts_info("Touch info: vkey_state %x, num_msg %u",
        //    touch_info->vkey_state, touch_info->num_msg);

#ifdef CFG_CTS_PROXIMITY
        if (cts_phone_switch) {
			//cts_info("cts_phone_switch = true");
            if (touch_info->vkey_state == 0x80){
                cts_palm_status = true;
				cts_info("proximity near\n");
			}
            else if(touch_info->vkey_state == 0x00){
                cts_palm_status = false;
				cts_info("proximity far-away\n");
        	}
#ifdef ZCFG_CTP_DISABLE_AVDD_AVEE
			cts_info("cts_palm_status = %d. no drop 3power.",cts_palm_status);
			//set_ctpstatus(1);//no power-off
#endif			
            if (cts_palm_status != cts_palm_status_old) 
			{

                cts_palm_status_old = cts_palm_status;
				cts_info("report proximity:%s.",cts_palm_status==true?"near":"far-away");
                cts_report_customer_key(cts_dev->pdata, cts_palm_status);
...
#if defined(CFG_CTS_PROXIMITY) || defined(CFG_CTS_PROXIMITY_SHUB)
	cts_dev_golbal = cts_dev;
#endif
read_fwid:
    ret = 0;
#ifdef CONFIG_CTS_I2C_HOST
