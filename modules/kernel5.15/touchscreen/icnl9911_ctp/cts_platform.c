...
static long cts_ioctl_operate(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret_cts = 0;
	//cts_info("cts_ioctl_operate %d!\n", cmd);

	switch(cmd)
	{
	case FT_IOCTL_PROX_ON:
		cts_info("cts_ioctl_operate-> FT_IOCTL_PROX_ON");
		ret_cts = cts_prox_ctrl(1);  
#ifdef ZCFG_CTP_DISABLE_AVDD_AVEE		
		if(ret_cts < 0)
		{
			cts_info("cts_ioctl_operate :%d,ret_cts:%d,on-failed..", cmd,ret_cts);
			if(ret_cts == PROXIMITY_DEVICE_EIO)
			{
				set_proximity_device(PROXIMITY_DEVICE_EIO_ON);
			}
			return 0;
		}
		//set_ctpstatus(1);//no power-off
		set_proximity_calling_device(1);
#endif		
		break;

	case FT_IOCTL_PROX_OFF:
		cts_info("cts_ioctl_operate-> FT_IOCTL_PROX_OFF");
		ret_cts = cts_prox_ctrl(0);
#ifdef ZCFG_CTP_DISABLE_AVDD_AVEE		
		if(ret_cts < 0)
		{
			cts_info("cts_ioctl_operate :%d,ret_cts:%d,off-failed..", cmd,ret_cts);
			if(ret_cts == PROXIMITY_DEVICE_EIO)
			{
				set_proximity_device(PROXIMITY_DEVICE_EIO_OFF);
			}
			return 0;
		}
		//set_ctpstatus(0);//yes to power-off
		set_proximity_calling_device(0);
#endif		
		break;
...
static struct file_operations CTS_proximity_fops = {
	.owner = THIS_MODULE,
	.open = NULL,
	.release = NULL,
	.unlocked_ioctl = cts_ioctl_operate
};

struct miscdevice CTS_proximity_misc = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = PROXIMITY_DEVICE,		//match the hal's name 
	.fops = &CTS_proximity_fops,
};
#endif 

#ifdef CFG_CTS_PROXIMITY_SHUB
...
#elif defined(CTP_MAX_WIDTH)
	pdata->res_x = CTP_MAX_WIDTH;
#else
...
#elif defined(CTP_MAX_HEIGHT)
	pdata->res_y = CTP_MAX_HEIGHT;
#else
        ...
        struct device *device, enum cts_bus_type bus_type)
{
...
#ifdef CFG_CTS_PROXIMITY
	ret = misc_register(&CTS_proximity_misc);
	if (ret < 0)
	{
...
#ifdef CFG_CTS_PROXIMITY
    //__set_bit(CTS_KEY_IN, input_dev->keybit);
    //__set_bit(CTS_KEY_OUT, input_dev->keybit);
    ...
    if(!ps_input_dev){
        ret = -ENOMEM;
        dev_err(&pdata->i2c_client->dev, "cts error::failed to allocate ps-input device\n");
        goto exit_input_register_device_failed;
    }
...
#elif defined(CFG_CTS_PROXIMITY)
	misc_deregister(&CTS_proximity_misc);
#endif
	return ret;
}
...
#ifdef CFG_CTS_PROXIMITY
void cts_report_customer_key(struct cts_platform_data *pdata, bool status)
{
    struct input_dev *input_dev = pdata->ts_input_dev;
    unsigned long key = status ? CTS_KEY_IN : CTS_KEY_OUT;

    cts_info("reprot key: %d\n", key);
	/*
    ...
    input_sync(input_dev);
    */
	input_report_abs(input_dev, ABS_DISTANCE, key);
	//input_mt_sync(input_dev);
	//input_sync(input_dev);
}
#endif
...
#ifdef CFG_CTS_PROXIMITY_SHUB
struct device* get_cts_device(void)
{
	return  psensor_dev;
}
#endif
