...
struct device *fw_psensor_dev;
struct device* get_fw_device(void)
{
	return  fw_psensor_dev;
}

int fw_shub_report_proximity_flush_event(u32 value)
{
  s64 k_timestamp;
...
int fw_shub_report_proximity_event(u32 value)
{
  s64 k_timestamp;
  struct shub_event_params event;
    ...
    const char *buf, size_t count)
{
  //int ps_enable = 0;
  //sscanf(buf, "%d\n",&ps_enable);
  fw_shub_report_proximity_flush_event(0);
  return count;
}

static DEVICE_ATTR(psensor_flush, 0644, proximity_sensor_flush_show, proximity_sensor_fulsh_store);

void fw_input_dev_common(void)
{
	struct device * fw_psensor_input_dev = get_fw_device();
	
	if(device_create_file(fw_psensor_input_dev, &dev_attr_psensor_enable) < 0) // /sys/class/sprd_sensorhub_tp/device/psensor_enable
		printk("Failed to create device file(%s)!\n", dev_attr_psensor_enable.attr.name);

	if(device_create_file(fw_psensor_input_dev, &dev_attr_psensor_flush) < 0) // /sys/class/sprd_sensorhub_tp/device/psensor_enable
		printk("Failed to create device file(%s)!\n", dev_attr_psensor_enable.attr.name);
...
//	printk("<tian> data_size = %d\n",data_size);
	...
	if(gdev->PS_Enale == true){
        ...
        if (gdev->tel_screen_off==1){
            printk ("gcore proximity near\n");
            fw_shub_report_proximity_event(0);// 1;  near
        }else if (gdev->tel_screen_off==0){
            printk ("gcore proximity far-away\n");
            fw_shub_report_proximity_event(0x40a00000);// 0;  far-away
        }
...
int gcore_probe_device(void)
{
	...
	while (retries++ < 3) {
		...
		if (id == chip_type ||id == 0x7302 ||id == 0x0302 ||id == 0x9303 ||id == 0x1602 ||id == 0x3632 ||id == 0x9502 || id == 0x1603 || id == 0x7203 || id == 0x0703 || id == 0x2303) {
			char tp_id[5];
			#if defined(CONFIG_ENABLE_CHIP_TYPE_GC7302)
				id = 0x7302;
			#elif defined(CONFIG_ENABLE_CHIP_TYPE_GC7202) || defined(CONFIG_ENABLE_CHIP_TYPE_GC7202H)
				id = 0x7202;
			#endif

			sprintf(tp_id,"%4x",id);
			tp_id[4]='\0';
			zyt_info_s2("[TP] : GC",tp_id);
...
check_ctp_chip();
	...
	fw_psensor_dev = device_create(psensor_class, NULL, 0, NULL, "device");
	if(IS_ERR(fw_psensor_dev))
		printk("fw Failed to create device!\n");
		fw_input_dev_common();
#endif
