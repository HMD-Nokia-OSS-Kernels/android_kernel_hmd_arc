...
#else
			//LOGD(tcm_hcd->pdev->dev.parent,"Finger %d: x = %d, y = %d\n",idx, x, y);
#endif
			touch_count++;
			break;
		default:
...
*/
#if SUPPORT_FACE_DETECT
extern int omn_shub_report_proximity_event(u32 value);
static int ovt_check_face_state(int current_face_state)
{
	//printk("[omniVision_face] ovt_check_face_state face\n");
	if ((current_face_state == FACE_FAR_FROM_1_2_3_CLOSE_WHEN_SCREEN_ON) ||
		...
		//report far event
		//printk("[omniVision_face]tcm check face far-face\n");
		omn_shub_report_proximity_event(0x40a00000);// 0;  far-away

	} else if ((current_face_state == FACE_CLOSE_1_SMALL_SIGNAL) ||
				(current_face_state == FACE_CLOSE_FROM_1_2_3_CLOSE_WHEN_SCREEN_OFF)) {
		//report close event
		//printk("[omniVision_face]tcm check face close-face\n");
		omn_shub_report_proximity_event(0);// 1;  near
	}else{
		//printk("[omniVision_face]tcm face error status:%d\n",current_face_state);
	}
