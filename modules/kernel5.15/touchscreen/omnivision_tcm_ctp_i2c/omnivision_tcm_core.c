...
//#define CFG_CTS_PROXIMITY_SHUB
#ifdef ZCFG_MK_TP_PROXIMITY_SHUB
	#define CFG_CTS_PROXIMITY_SHUB
#endif

#if defined(CFG_CTS_PROXIMITY_SHUB)
#include <linux/shub_api.h>
static struct wakeup_source *omn_proximity_wake_lock;
static struct device *omn_psensor_dev;
static struct class *omn_psensor_class;
static struct input_dev *omn_ps_input_dev;
//extern int cts_prox_ctrl(bool enable);
static bool omn_phone_face_switch = 0;

void omn_set_phone_face_switch(bool enable){
	omn_phone_face_switch = enable;
}
bool omn_get_phone_face_switch_flag(void){
	return omn_phone_face_switch ;
}

int omn_shub_report_proximity_flush_event(u32 value)
{
  s64 k_timestamp;
...
//struct cts_device *cts_dev_golbal = NULL; 
//bool cts_phone_switch = 0;
//extern struct input_dev *ps_input_dev;

int omn_shub_report_proximity_event(u32 value)
{
  s64 k_timestamp;
  struct shub_event_params event;
...
#endif
void launch_proximity_work(void);
void cancel_proximity_work(void);

int omn_proximity_ctrl(bool enable)
{
	//int ret;
	//printk("[omniVision_face] proximity enable=%d.\n", enable);
	if(enable){
		ovt_tcm_set_func_face_detect_en_state(1);//enable

		launch_proximity_work();
		//__pm_stay_awake(omn_proximity_wake_lock);
	}else{
		ovt_tcm_set_func_face_detect_en_state(0);//disable

		cancel_proximity_work();

		incellctp_set_sprd_panel_phone_call_flag(0);
		//__pm_relax(omn_proximity_wake_lock);
	}

	omn_set_phone_face_switch(enable);
...
{
	//cts_info("show_proximity_sensor buf==%d\n", omn_phone_switch);
	return sprintf(buf, "tp proximity enable = %d\n", omn_phone_face_switch);
}
		...
		const char *buf, size_t count)
{
	int ps_enable = 0;
	sscanf(buf, "%d\n",&ps_enable);
	printk("[store_proximity_sensor] proximity ps_enable=%d.\n", ps_enable);
    //cts_info("%s sys control tp face mode! ps_enable = %d\n", __func__, ps_enable);//debug
    omn_proximity_ctrl(ps_enable);
    return count;
}
    ...
    const char *buf, size_t count)
{
  //int ps_enable = 0;
  //sscanf(buf, "%d\n",&ps_enable);
  omn_shub_report_proximity_flush_event(0);
  return count;
}

static DEVICE_ATTR(psensor_flush, 0644, proximity_sensor_flush_show, proximity_sensor_fulsh_store);

struct workqueue_struct *proximity_polling_workqueue;
struct delayed_work proximity_work;
#define POLLING_proximity_MS 100

void launch_proximity_work(void){
	queue_delayed_work(proximity_polling_workqueue,
				&proximity_work,
				msecs_to_jiffies(POLLING_proximity_MS));
}

void cancel_proximity_work(void){
	cancel_delayed_work_sync(&proximity_work);
	flush_workqueue(proximity_polling_workqueue);
	//destroy_workqueue(proximity_polling_workqueue);
}

static void ovt_tcm_proximity_work_func(struct work_struct *work)
{
	//int retval;
	//printk("[omniVision_face] ovt_tcm_proximity_work enable=%d.\n", omn_phone_face_switch);
	if(omn_phone_face_switch){
		ovt_tcm_set_func_face_detect_en_state(1);//enable
...
void create_proximity_work(void){
	proximity_polling_workqueue = create_singlethread_workqueue("proximity_polling_work");
	INIT_DELAYED_WORK(&proximity_work, ovt_tcm_proximity_work_func);

}


void omn_init_ps_dev(struct device *ps_device)
{

//#if defined(CFG_CTS_PROXIMITY_SHUB)
	int ret = 0;

	//ovt_tcm_set_func_face_detect_en_state(1);//enable face
	omn_ps_input_dev = input_allocate_device();
    if(!omn_ps_input_dev){
        ret = -ENOMEM;
        dev_err(ps_device, "cts error::failed to allocate ps-input device\n");
        //goto exit_input_register_device_failed;
        return ;
    }
    omn_ps_input_dev->name = "alps_pxy";
    set_bit(EV_ABS, omn_ps_input_dev->evbit);
    input_set_capability(omn_ps_input_dev, EV_ABS, ABS_DISTANCE); 
    input_set_abs_params(omn_ps_input_dev, ABS_DISTANCE, 0, 1, 0, 0);
    ret = input_register_device(omn_ps_input_dev);
    if (ret) {
        dev_err(ps_device,"failed to register input device: %s\n",
                "omniVision_tcm_i2c");
        //goto exit_input_register_device_failed;
         return ;
    }
    ...
    // /sys/class/sprd_sensorhub_tp/device/psensor_enable
    
    omn_psensor_class = class_create(THIS_MODULE,"sprd_sensorhub_tp");
	if(IS_ERR(omn_psensor_class)){
		printk("[omniVision_tcm_i2c] Failed to create class!\n");
		 return ;
	}

	omn_psensor_dev = device_create(omn_psensor_class, NULL, 0, NULL, "device");
	if(IS_ERR(omn_psensor_dev)){
		printk("[omniVision_tcm_i2c] Failed to create device!\n");
		 return ;
	}

	if(device_create_file(omn_psensor_dev, &dev_attr_psensor_enable) < 0) // /sys/class/sprd_sensorhub_tp/device/psensor_enable
	{
		printk("[omniVision_tcm_i2c] Failed to create device file(%s)!\n", dev_attr_psensor_enable.attr.name);
	}

	if(device_create_file(omn_psensor_dev, &dev_attr_psensor_flush) < 0) // /sys/class/sprd_sensorhub_tp/device/psensor_enable
	{
		printk("[omniVision_tcm_i2c] Failed to create device file(%s)!\n", dev_attr_psensor_enable.attr.name);
	}
//    wakeup_source_init(&omn_proximity_wake_lock, "prox_delayed_work");
	omn_proximity_wake_lock = wakeup_source_create("prox_delayed_work");
	wakeup_source_add(omn_proximity_wake_lock);

	create_proximity_work();
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtouint(buf, 10, &input))
		return -EINVAL;
	printk("[omniVision_face] ts_suspend input:%d.\n",input);
	if (input == 1)
...
retry:
	...
	//LOGN(tcm_hcd->pdev->dev.parent,			"Status report code = 0x%02x\n",			tcm_hcd->status_report_code);

	//LOGN(tcm_hcd->pdev->dev.parent,			"Payload length = %d\n",			tcm_hcd->payload_length);
...
static int ovt_tcm_resume(struct device *dev)
{
...
#if defined(CFG_CTS_PROXIMITY_SHUB)	
	if(omn_phone_face_switch){
		//printk("[omniVision_face] ovt_tcm_resume already omn_phone_face_switch.\n");
		printk("[omniVision_face] ovt_tcm_resume omn_phone_face_switch. %d\n",omn_phone_face_switch);
		omn_proximity_ctrl(1);
		//return 0;
#if defined(ZCFG_USE_INCELL_CTP)		
	...
	if (!tcm_hcd->in_suspend  || tcm_hcd->ovt_tcm_driver_removing){
		//printk("[omniVision_face] ovt_tcm_resume already resume.\n");
		return 0;
	}	
	mutex_lock(&tcm_hcd->suspend_resume_mutex);
	if (tcm_hcd->in_hdl_mode) {
...
exit:
	tcm_hcd->in_suspend = false;//resume
	mutex_unlock(&tcm_hcd->suspend_resume_mutex);
	return retval;
#endif
}
...
static int ovt_tcm_suspend(struct device *dev)
{
	struct ovt_tcm_module_handler *mod_handler;
	struct ovt_tcm_hcd *tcm_hcd = dev_get_drvdata(dev);
	//printk("[omniVision_face] ovt_tcm_suspend omn_phone_face_switch. %d\n",omn_phone_face_switch);
#if defined(CFG_CTS_PROXIMITY_SHUB)
	if(omn_phone_face_switch){
		printk("[omniVision_face] ovt_tcm_suspend omn_phone_face_switch. \n");
		omn_proximity_ctrl(1);
#if defined(ZCFG_USE_INCELL_CTP)		
		incellctp_set_sprd_panel_phone_call_flag(1);
#endif		
...
#endif
	if (tcm_hcd->in_suspend || tcm_hcd->ovt_tcm_driver_removing){
		printk("[omniVision_face] ovt_tcm_suspend already suspend.\n");
		return 0;
	}	
	mutex_lock(&tcm_hcd->suspend_resume_mutex);
...
//	struct drm_panel *active_panel = tcm_get_panel();
...

	{
		...
		sprintf(tp_fm_ver,"0x%02d",tcm_hcd->packrat_number);
		
		zyt_info_s2("[TP] : omnivision","om");
		zyt_info_s2("[OMNIVISION] : FW Version:",tp_fm_ver);
	}	
