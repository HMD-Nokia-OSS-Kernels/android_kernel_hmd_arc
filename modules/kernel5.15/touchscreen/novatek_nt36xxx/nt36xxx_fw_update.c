...
static int32_t nvt_bin_header_parser(const u8 *fwdata, size_t fwsize)
{
	...
	for (list = 0; list < partition; list++) {
		...
		if (list < ilm_dlm_num) {
			...
			if (list == 0)
				snprintf(bin_map[list].name, 12, "ILM");
			else if (list == 1)
				snprintf(bin_map[list].name, 12, "DLM");
		}
		...
		if ((list >= ilm_dlm_num) && (list < (ilm_dlm_num + info_sec_num))) {
			...
			if ((bin_map[list].BIN_addr == 0) && (bin_map[list].size != 0)) {
				snprintf(bin_map[list].name, 12, "Header");
			} else {
				snprintf(bin_map[list].name, 12, "Info-%d", (list - ilm_dlm_num));
			}
		}
		...
		if (list >= (ilm_dlm_num + info_sec_num)) {
			...
			snprintf(bin_map[list].name, 12, "Overlay-%d", (list- ilm_dlm_num - info_sec_num));
		}
