...
bool nt36672c_flag = false;
#define NT36672E_LCD_NAME "lcd_nt36672e_truly_mipi_fhd"
#define NT36672C_LCD_NAME "lcd_nt36672c_truly_mipi_fhd"
...
info_retry:
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		ts->abs_y_max = nt36672c_flag ? NT36672C_TOUCH_DEFAULT_MAX_HEIGHT : NT36672E_TOUCH_DEFAULT_MAX_HEIGHT;
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id, uint8_t *data)
{
	...
	switch (gesture_id) {
		...
		case GESTURE_SLIDE_RIGHT:
			...
			break;
		default:
			NVT_ERR("gesture_id %d is invalid\n", gesture_id);
			break;
	}
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	if (ret1) {
		...
		nt36672c_flag ? NT36672C_TOUCH_DEFAULT_MAX_HEIGHT : NT36672E_TOUCH_DEFAULT_MAX_HEIGHT);
	} else {
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		nvt_update_firmware(nt36672c_flag ? NT672C_BOOT_UPD_FW_NAME : NT672E_BOOT_UPD_FW_NAME);
	...
	for (retry = 5; retry > 0; retry--) {
		...
		for (list = 0; list < (sizeof(trim_id_table) / sizeof(struct nvt_ts_trim_id_table)); list++) {
			...
			if (found_nvt_chip) {
				...
				NVT_LOG("trim_id_table.id = 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
				trim_id_table[list].id[0], trim_id_table[list].id[1],trim_id_table[list].id[2],
				trim_id_table[list].id[3], trim_id_table[list].id[4], trim_id_table[list].id[5]);
...
int32_t nvt_ts_resume(struct device *dev)
{
	...
	gpio_set_value(ts->reset_gpio, 1);
#endif
	if (nvt_update_firmware(nt36672c_flag ? NT672C_BOOT_UPD_FW_NAME : NT672E_BOOT_UPD_FW_NAME)) {
		NVT_ERR("download firmware failed, ignore check fw state\n");
		retval = -1;
		goto EXIT;
	} else if (nvt_check_fw_reset_state(RESET_STATE_REK)) {
		NVT_ERR("download firmware failed, nvt_check_fw_reset_state failed.\n");
		retval = -1;
		goto EXIT;
	}
...
EXIT:
	...
	NVT_LOG("nvt_ts_resume [%s].\n", retval?"failures":"success");
...
static int32_t __init nvt_driver_init(void)
{
	...
	if (!ret && !(strstr(lcd_name, NT36672E_LCD_NAME) || strstr(lcd_name, NT36672C_LCD_NAME)))
	{
		NVT_ERR("%s: match %s fail,return\n",__func__,lcd_name);
		return 0;
	}
	nt36672c_flag = !!strstr(lcd_name, NT36672C_LCD_NAME);
	NVT_LOG("start\n");
