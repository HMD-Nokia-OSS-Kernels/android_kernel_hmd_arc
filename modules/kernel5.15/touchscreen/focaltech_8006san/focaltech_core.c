...
static int nvt_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
   ...
   FTS_INFO("waterworld  event = %d", event);
   if (event == DISPC_POWER_OFF) {
        fts_ts_suspend(fts_data->dev);
		FTS_INFO("DISPC_POWER_OFF\n");
   }else if (event == DISPC_POWER_ON) {
        fts_ts_resume(fts_data->dev);
		FTS_INFO("DISPC_POWER_ON\n");
	}
   else
       FTS_INFO("waterworld wrong event");
    return 0;
}
#endif 
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
    ...
    if (1/*regulator_count_voltages(ts_data->vdd) > 0*/) {
    ...
    if (!IS_ERR_OR_NULL(ts_data->vcc_i2c)) {
        if (1/*regulator_count_voltages(ts_data->vcc_i2c) > 0*/) {
...
static int tp_sysfs_init(struct i2c_client *client)
{
	...
	if (0 != err) {
		...
		FTS_INFO("%s() - sysfs_create_group() succeeded.\n", __func__);
	}
	return err;
}

void tp_release_sysfs(struct i2c_client *client)
{
	sysfs_remove_group(&client->dev.kobj, &tp_attr_group);
}
#endif
...
#if defined(ZCFG_TP_NOT_USE_SUSPEND_RESUME_NODE)
			ts_data->drm_notifier.notifier_call = nvt_drm_notifier_callback;
			ret = disp_notifier_register(&ts_data->drm_notifier);
		
			if(ret < 0){
				FTS_ERROR("Init drm notifier failed %d", ret);
...
#endif
#if defined(ZCFG_TP_NOT_USE_SUSPEND_RESUME_NODE)
	disp_notifier_unregister(&ts_data->drm_notifier);
#endif
    if (ts_data->ts_workqueue)
        destroy_workqueue(ts_data->ts_workqueue);
...
#ifndef ZCFG_TP_NOT_USE_SUSPEND_RESUME_NODE
	tp_sysfs_init(ts_data->client);
#endif
	ret = sysfs_create_link(NULL, &ts_data->client->dev.kobj, "touchscreen");
	if (ret < 0) {
		FTS_ERROR("Failed to create link!: %d",ret);
		return -ENOMEM;
	}
	check_ctp_chip();
	{
		...
		zyt_info_s2("[TP] : ","FT8006S");
		zyt_info_sx("[FT8006SAN] : FW Version:",val);
	}
