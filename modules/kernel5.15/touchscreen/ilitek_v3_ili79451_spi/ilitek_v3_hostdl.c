...
int ili_fw_dump_iram_data(u32 start, u32 end, bool save, bool mcu)
{
	...
	if (save) {
#if GENERIC_KERNEL_IMAGE
			ILI_ERR("GKI version not allow drivers to use filp_open_block, dump IRAM data\n");
			debug_en = DEBUG_ALL;
			...
			goto out;
#else
		f = filp_open_block(DUMP_IRAM_PATH, O_WRONLY | O_CREAT | O_TRUNC, 644);
		if (ERR_ALLOC_MEM(f)) {
...
#endif
#else
		f = filp_open_block(ilits->md_fw_filp_path, O_RDONLY, 0644);
		if (ERR_ALLOC_MEM(f)) {
