...
{
	static char fw_ver[11];
	static bool Frist_Read_Fw=1;
#ifdef CONFIG_UPDATE_FIRMWARE_BY_BIN_FILE
...
if(Frist_Read_Fw)
		{
			sprintf(fw_ver,"0x%x",get_fw_ver());
			fw_ver[10]='\0';
			zyt_info_s2("[TP]:FW Version:",fw_ver);
			Frist_Read_Fw=0;
		}
...
//	const int spi_buf_len = 8;
	...
	mutex_lock(&gdev_fwu->transfer_lock);
	ret = gcore_spi_write_and_read(spi_wbuff, 8, spi_rbuff, 8);
	mutex_unlock(&gdev_fwu->transfer_lock);
	if (ret){
