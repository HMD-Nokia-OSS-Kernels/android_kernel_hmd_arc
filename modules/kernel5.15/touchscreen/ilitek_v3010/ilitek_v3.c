...
static ssize_t do_proximity_enable_store(unsigned long on_off ,size_t size)
{
    ...
    //    ts->proximity_enable = 0;
        PROXIMITY_SWITCH = 0;
		ilits->prox_face_mode = 0;
		if (ili_ic_func_ctrl("proximity", 0x00) < 0) {
			ILI_ERR("Write proximity cmd failed\n");
			}
		//incellctp_set_sprd_panel_phone_call_flag(0);//enable
    ...
    else
    {
     ...
     //   ts->proximity_enable = 1;
        PROXIMITY_SWITCH = 1;
	 	ilits->prox_face_mode = 0x02 & 0xFF;
		if (ili_ic_func_ctrl("proximity", 0x01) < 0) {
			ILI_ERR("Write proximity cmd failed\n");
			}
	 	//incellctp_set_sprd_panel_phone_call_flag(1);//enable
...
ILI_INFO("wabsizeof(struct shub_event_params = %zu\n",sizeof(struct shub_event_params));
...
//  dev_info(&g_sensor->sensor_pdev->dev,
  ILI_INFO("sizeof(struct shub_event_params = %zu\n",sizeof(struct shub_event_params));
  peri_send_sensor_event_to_iio((u8 *)&event,
      sizeof(struct shub_event_params));
...
#ifdef ZCFG_MK_TP_PROXIMITY_SHUB
	ILI_INFO("wab proxmity face status :%X\n", ilits->tr_buf[0]);
	ILI_INFO("wab0 ilits->prox_face_mode :%d tr_buf[1]:%d \n", ilits->prox_face_mode,ilits->tr_buf[1]);
	if (ilits->tr_buf[0] == P5_X_DEMO_PROXIMITY_ID) {
		if (ilits->prox_face_mode == PROXIMITY_SUSPEND_RESUME) {
			...
			if (ilits->tr_buf[1] == PROXIMITY_NEAR_STATE_1 || ilits->tr_buf[1] == PROXIMITY_NEAR_STATE_2 || ilits->tr_buf[1] == PROXIMITY_NEAR_STATE_3) {
				ILI_INFO("wabproxmity face status : Near, cmd : %X\n", ilits->tr_buf[1]);
				incellctp_near_close_backlight(0);
			} else if(ilits->tr_buf[1] == PROXIMITY_FAR_STATE_5) {
				ILI_INFO("wabproxmity face status : Far, cmd : %X\n", ilits->tr_buf[1]);
				incellctp_near_close_backlight(1);
				//shub_report_proximity_event(0x40a00000);
			} else {
				ILI_INFO("wabproxmity face status : Ignore, cmd : %X\n", ilits->tr_buf[1]);
				incellctp_near_close_backlight(1);
				//shub_report_proximity_event(0x40a00000);
			}
		}
...
int ili_tddi_init(void)
{
	...
	if(!ps_input_dev){
		ILI_ERR("jadard error::failed to allocate ps-input device\n");
		return 0;
	}
	ilits->ps_input_dev = ps_input_dev;
	...
	if (ret) {
		ILI_ERR("failed to register input device\n");
		return 0;
	}
	...
	if(device_create_file(psensor_dev, &dev_attr_psensor_flush) < 0) // /sys/class/sprd_sensorhub_tp/device/psensor_flush
