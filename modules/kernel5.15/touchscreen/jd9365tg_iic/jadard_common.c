...
#if defined(ZCFG_MK_TP_PROXIMITY_SHUB)
#include <linux/shub_api.h>

#if defined ZCFG_FIRST_BRIGHT_FLAG
int first_bright = 0;
#endif

#if defined ZCFG_CTP_NEAR_CLOSE_BACKLIGHT
extern void incellctp_near_close_backlight(bool enable);
#endif
...
static int jadard_halt_flag = 0;
static int jadard_suspend = 0;
...
static ssize_t do_proximity_enable_store(unsigned long on_off ,size_t size)
{
    ...
    if(on_off==0)
    {
        JD_I("%s: proximity is disable\n",__func__);
        ts->proximity_enable = 0;
        PROXIMITY_SWITCH = 0;
        __pm_relax(proximity_wake_lock);
    }else
    {
        JD_E("%s: proximity is enable \n",__func__);
        ts->proximity_enable = 1;
        PROXIMITY_SWITCH = 1;
        __pm_stay_awake(proximity_wake_lock);
    }
    // incellctp_set_sprd_panel_phone_call_flag(ts->proximity_enable);
...
#ifndef ZCFG_CTP_NEAR_CLOSE_BACKLIGHT
...
{
	JD_I("show_proximity_sensor buf==%d\n", PROXIMITY_SWITCH);
		...
		const char *buf, size_t count)
{
	int ps_enable = 0;
    sscanf(buf, "%d\n",&ps_enable);
    JD_E("%s sys control tp face mode! ps_enable = %d\n", __func__, ps_enable);//debug
...
#if defined ZCFG_FIRST_BRIGHT_FLAG
    if(first_bright == 1 && ts->proximity_detect){
        ts->proximity_detect = 0;
        ts->pre_proximity_detect = 0;
        first_bright = 0;
        JD_I("first_bright == %d\n",first_bright);
    }
#endif    
    JD_E("%s:ts->proximity_detect = %d ts->pre_proximity_detect = %d ts->proximity_enable = %d\n", __func__ ,ts->proximity_detect ,ts->pre_proximity_detect , ts->proximity_enable);
    if (ts->proximity_detect == 1) {
        /* Proximity detect from 0 to 1 */
        if ((ts->proximity_enable) && (ts->pre_proximity_detect == 0)) {
            JD_E("%s: Proximity On(Near)\n", __func__);
            JD_E("%s: jadard_suspend = %d===(Near)\n", __func__,jadard_suspend);
            if(jadard_suspend==1)
            {
                    //jadard_suspend = 0;
            ...
            {
                    PROXIMITY_STATE = 1;
                    JD_I("%s: Proximity On====(Near),PROXIMITY_STATE = %d\n", __func__,PROXIMITY_STATE);
                    //sensorhub_prox_do->prox_reprot_func(ts->input_dev, SENSORHUB_FACE_STATUS_NEAR);
                    #ifdef ZCFG_CTP_NEAR_CLOSE_BACKLIGHT
                        incellctp_near_close_backlight(0);
                        ts->ctp_status = true;
                        JD_I("incellctp_near_close_backlight == 0\n");
                        jadard_report_all_leave_event(ts);
                    #else
        ...
        if (ts->pre_proximity_detect) {
            JD_E("%s: Proximity Off(Far)\n", __func__);

            /* Add report proximity far event to system */
            PROXIMITY_STATE = 0;
            JD_E("%s: Proximity On====(Far),PROXIMITY_STATE = %d\n", __func__,PROXIMITY_STATE);
            //sensorhub_prox_do->prox_reprot_func(ts->input_dev, SENSORHUB_FACE_STATUS_FAR);
            #ifdef ZCFG_CTP_NEAR_CLOSE_BACKLIGHT
                incellctp_near_close_backlight(1);
                ts->ctp_status = false;
                JD_I("incellctp_near_close_backlight == 1\n");
            #else
...
static int jadard_parse_report_points(struct jadard_ts_data *ts, int ts_status)
{
    ...
    for (i = 0; i < pjadard_ic_data->JD_MAX_PT; i++) {
        if ((ts->proximity_enable) && (ts->pre_proximity_detect == 1)) {
...
static ssize_t jadard_name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "jadard_touchscreen\n");
}

static ssize_t jadard_name_store(struct device *dev, struct device_attribute *attr, const char *buf,
				size_t count)
{
	return count;
}
...
#ifdef ZCFG_CTP_NEAR_CLOSE_BACKLIGHT
static ssize_t ctp_status_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct jadard_ts_data *ts = pjadard_ts_data;
...
static DEVICE_ATTR_RO(ctp_status);
#endif

static DEVICE_ATTR_RW(ts_suspend);
static DEVICE_ATTR_RW(jadard_name);



static struct attribute *jadard_ts_debug_attrs[] = {
    &dev_attr_ts_suspend.attr,
    &dev_attr_jadard_name.attr,
#ifdef ZCFG_CTP_NEAR_CLOSE_BACKLIGHT
    &dev_attr_ctp_status.attr,
#endif
    NULL,
};
...
int jadard_input_register(struct jadard_ts_data *ts)
{
  ...
  ps_input_dev = input_allocate_device();
    if(!ps_input_dev){
        ret = -ENOMEM;
        JD_E("jadard error::failed to allocate ps-input device\n");
        return JD_INPUT_REG_FAIL;
    }
    ts->ps_input_dev = ps_input_dev;
...
int jadard_chip_common_suspend(struct jadard_ts_data *ts)
{
    ...
    JD_I("%s:  Proximity_enable = %d.\n", __func__, ts->proximity_enable);
        ...
        if( ts->proximity_enable==1)
        {
            #ifdef ZCFG_USE_INCELL_CTP    
            incellctp_set_sprd_panel_phone_call_flag(1);
            JD_E("incellctp_set_sprd_panel_phone_call_flag == 1\n");
            #endif
            ts->suspended = true;
            //g_module_fp.fp_ic_reset(false, true);
            //sensorhub_prox_ctrl(1);
    ...
    atomic_set(&ts->suspend_mode, 1);
        ...
        JD_I("%s: gpio_reset = %d status = 0\n", __func__,pjadard_ts_data->rst_gpio);
...
#ifdef ZCFG_MK_TP_PROXIMITY_SHUB
            JD_I("%s:  jadard_chip_common_resume = %d.jadard_halt_flag=%d\n", __func__, ts->proximity_enable,jadard_halt_flag);
			jadard_suspend = 0;
            if( (ts->proximity_enable==1) && (jadard_halt_flag == 0) )
            {
                #ifdef ZCFG_USE_INCELL_CTP
                incellctp_set_sprd_panel_phone_call_flag(1);
                JD_E("incellctp_set_sprd_panel_phone_call_flag == 1\n");
                ...
                JD_E("%s:  jadard_chip_common_resume ==data_reinit==.\n", __func__);
				//return 0;
            }else{
                #ifdef ZCFG_USE_INCELL_CTP
                incellctp_set_sprd_panel_phone_call_flag(0);
...
if(!ts->proximity_enable){
    ...
    } else if (g_module_fp.fp_0f_upgrade_fw(vendor_fw) < 0) {
