...
//extern int  jadard_jd9365tr_init(void);
//extern void  jadard_jd9365tr_exit(void);
...
int jadard_dev_set(struct jadard_ts_data *ts)
{
    ...
    ts->input_dev->name = "jd9365tr_touch";
	ts->tp_on_off = 1;
...
{
	JD_I("[zwenguo]222Jadard common touch panel driver init\n");
...
{
	printk("enter jadard_jd9365tr_exit2222222222\n");
	if (spi) {
		printk("%s\n","shifangSPI");
		spi_unregister_device(spi);
		spi = NULL;
	}
//	jadard_jd9365tr_exit();
