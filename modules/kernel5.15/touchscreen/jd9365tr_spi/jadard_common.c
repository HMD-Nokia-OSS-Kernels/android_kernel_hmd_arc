...
char *jd_i_CTPM_firmware_name = "Jadard_jd9365tr_spi.bin";
...
struct class *touchsreen_suspend_class = NULL;
struct device *touchsreen_suspend_dev = NULL;
...
static int jadard_ts_operation(struct jadard_ts_data *ts, int irq_event, int ts_status)
{
	...
	uint8_t *touch_data = NULL;
	touch_data = vmalloc(touch_data_len);
	if (touch_data == NULL) {
		JD_E("malloc memory for jadard_bus_write buffer fail");
		return -ENOMEM;
	}
	JD_D("%s: Entering, ts_status=%d, irq_event=%d\n", __func__, ts_status, irq_event);
	//memset(touch_data, 0x00, sizeof(touch_data));
	memset(touch_data, 0x00, touch_data_len);
...
static ssize_t name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "jd9365tr_touch\n");
}
...
#ifdef JD_SMART_WAKEUP
	ts->SMWP_enable = false;
	ts->ts_SMWP_wake_lock = wakeup_source_register(NULL, JADARD_common_NAME);//kernel5.4
#endif
