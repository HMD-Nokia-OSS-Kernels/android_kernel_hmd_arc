...
#define JADARD_common_NAME "jadardtr_tp"
#define JADARD_BUS_RETRY_TIMES 10

#if defined(CONFIG_TOUCHSCREEN_JADARD_DEBUG)
extern bool jd_g_dbg_enable;
#define JD_I(x...) printk("[JDTPR] " x)
#define JD_W(x...) printk("[JDTPR][WARNING] " x)
#define JD_E(x...) printk("[JDTPR][ERROR] " x)
#define JD_D(x...) \
do { if (jd_g_dbg_enable) printk("[JDTPR][DEBUG] " x); } while (0)
#else
#define JD_I(x...)
#define JD_W(x...)
#define JD_E(x...)
