...
static bool jadard_ts_diag_func(void)
{
    ...
    if ((KeepFrame > 0) && (KeepFrame > jd_diag_mutual_cnt)) {
#if 1//def JD_KEEP_RAWDATA_DISABLE
        struct jadard_diag_mutual_data dump;
	    ...
	    /* kernel5.4 GKI not allow use filp_open_block/kernel_write/kernel_read */
    ...
    if ((DataType > 0) && (KeepType > 0)) {
        ...
        if (KeepFrame > 0) {
            ...
            case JD_DATA_TYPE_RawData:
                jd_diag_mutual_fn = filp_open_block(JD_RAWDATA_DUMP_FILE, O_CREAT | O_TRUNC | O_WRONLY, 0);
                jd_diag_mutual_fn->f_pos = 0;
                break;
            case JD_DATA_TYPE_Baseline:
                jd_diag_mutual_fn = filp_open_block(JD_BASE_DUMP_FILE, O_CREAT | O_TRUNC | O_WRONLY, 0);
                jd_diag_mutual_fn->f_pos = 0;
                break;
            case JD_DATA_TYPE_Difference:
                jd_diag_mutual_fn = filp_open_block(JD_DIFF_DUMP_FILE, O_CREAT | O_TRUNC | O_WRONLY, 0);
                jd_diag_mutual_fn->f_pos = 0;
                break;
            case JD_DATA_TYPE_LISTEN:
                jd_diag_mutual_fn = filp_open_block(JD_LISTEN_DUMP_FILE, O_CREAT | O_TRUNC | O_WRONLY, 0);
                jd_diag_mutual_fn->f_pos = 0;
                break;
            case JD_DATA_TYPE_LABEL:
                jd_diag_mutual_fn = filp_open_block(JD_LABEL_DUMP_FILE, O_CREAT | O_TRUNC | O_WRONLY, 0);
                jd_diag_mutual_fn->f_pos = 0;
            case JD_DATA_TYPE_LAPLACE:
                jd_diag_mutual_fn = filp_open_block(JD_LAPLACE_DUMP_FILE, O_CREAT | O_TRUNC | O_WRONLY, 0);
                jd_diag_mutual_fn->f_pos = 0;
                break;
            default:
...
static void jadard_ts_fw_dump_work_func(struct work_struct *work)
{
    ...
    fn = filp_open_block(JD_FW_DUMP_FILE, O_CREAT | O_TRUNC | O_WRONLY, 0);
