...
#else
//static unsigned short SCREEN_MAX_X = 1200 ;   //变量未使用
#endif
#ifdef SCREEN_PHY_HIGHT
static unsigned short SCREEN_MAX_Y = (unsigned short)SCREEN_PHY_HIGHT ;
#else
//static unsigned short SCREEN_MAX_Y = 2000 ;    //变量未使用
#endif
...
void jadard_report_points(struct jadard_ts_data *ts)
{
    ...
    JD_I("%s:ts->proximity_detect = %d ts->pre_proximity_detect = %d ts->proximity_enable = %d\n", __func__ ,ts->proximity_detect ,ts->pre_proximity_detect , ts->proximity_enable);
    ...
    for (i = 0; i < pjadard_ic_data->JD_MAX_PT; i++) {
		...
		if (pjadard_ic_data->JD_X_RES <= pjadard_ic_data->JD_Y_RES) {
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X,SCREEN_MAX_X- pjadard_host_data->x[i]);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,SCREEN_MAX_Y- pjadard_host_data->y[i]);
#else
...
int jadard_distribute_touch_data(struct jadard_ts_data *ts, uint8_t *buf, int irq_event, int ts_status)
{
    ...
    JD_D("%s: Entering, ts_status=%d JD_REPORT_COORD = %d \n", __func__, ts_status ,JD_REPORT_COORD);
...
static int jadard_get_FW(void)
{
    ...
    for (RetryCnt = 0; RetryCnt < JD_UPGRADE_FW_RETRY_TIME; RetryCnt++) {
        ret = request_firmware(&fw, vendor_fw, pjadard_ts_data->dev);
        if (ret < 0) {
...
static ssize_t name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "jd9365tg_touch\n");
}
...
static ssize_t do_proximity_enable_store(int on_off ,size_t size)
{
...
#ifdef SENSORHUB_PROXIMITY_TOUCHSCREEN
        set_bit(EV_ABS, ts->input_dev->evbit);
        input_set_capability(ts->input_dev, EV_ABS, ABS_DISTANCE);
...
#if defined(PRJ_FEATURE_H_BOARD_INCELL_TP_PROXIMITY_CONTROL)
...
export_open_tp_proximity = &open_tp_proximity;
	...
	wakeup_source_add(wakeup_wakelock);
        ...
        printk("---%s: ==sensorhub_init=\n",__func__);

        sensorhub_info.input = ts->input_dev;
...
int jadard_chip_common_resume(struct jadard_ts_data *ts)
{
...
#ifdef SENSORHUB_PROXIMITY_TOUCHSCREEN
            JD_I("%s:  jadard_chip_common_resume = %d.\n", __func__, ts->proximity_enable);
            jadard_suspend = 0;
            if(( ts->proximity_enable==1 )&&(jadard_halt_flag == 0))
            {
                JD_I("%s:  jadard_chip_common_resume ==data_reinit==.\n", __func__);
                jadard_report_all_leave_event(ts);
    ...
    atomic_set(&ts->suspend_mode, 0); //cxp
