...
/*******************************************************
Description:
	Novatek touchscreen isp check firmware checksum function.

return:
	Executive outcomes. 0---checksum not match.
	1---checksum match. -1--- checksum read failed.
*******************************************************/
int32_t Check_CheckSum_ISP(const struct firmware *fw_entry)
{
	uint8_t buf[64] = {0};
...
int32_t Check_CheckSum(const struct firmware *fw_entry)
{
	if(ts->use_gcm)
		...
		return Check_CheckSum_ISP(fw_entry);
}
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Write_Flash(const struct firmware *fw_entry)
{
	uint8_t buf[64] = {0};
...
return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Verify_Flash(const struct firmware *fw_entry)
{
	uint8_t buf[64] = {0};
	uint32_t XDATA_Addr = ts->mmap->READ_FLASH_CHECKSUM_ADDR;
	int32_t ret = 0;
...
/*******************************************************
Description:
	Novatek touchscreen isp update firmware function.

return:
	Executive outcomes. 0---succeed. negative---failed.
*******************************************************/
int32_t Update_Firmware_ISP(const struct firmware *fw_entry)
{
	int32_t ret = 0;
	...
	// Step 4 : Program
	ret = Write_Flash(fw_entry);
	if (ret) {
		return ret;
	}

	// Step 5 : Verify
	ret = Verify_Flash(fw_entry);
	if (ret) {
		return ret;
	}
...
int32_t Update_Firmware(const struct firmware *fw_entry)
{
	if (ts->use_gcm)
		...
		return Update_Firmware_ISP(fw_entry);
}
...
*******************************************************/
int32_t nvt_check_flash_end_flag_isp(void)
{
	uint8_t buf[8] = {0};
...
int32_t nvt_check_flash_end_flag(void)
{
	if (ts->use_gcm)
		...
		return nvt_check_flash_end_flag_isp();
