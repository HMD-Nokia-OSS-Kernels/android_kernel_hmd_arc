 ...
 *
 * $Revision: 108740 $
 * $Date: 2022-11-21 10:30:18 +0800 (Mon, 21 Nov 2022) $
 *
...
static int nvt_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
   ...
   NVT_LOG("waterworld  event = %d", event);
   if (event == DISPC_POWER_OFF) {
       ...
       NVT_LOG("waterworld wrong event");
    return 0;
}
#endif 
/*******************************************************
...
void nvt_stop_crc_reboot(void)
{
	...
	nvt_set_page(I2C_BLDR_Address, ts->chip_ver_trim_addr);

	//---read to check if buf is 0xFC which means IC is in CRC reboot ---
	buf[0] = ts->chip_ver_trim_addr & 0xFF;
	...
	if ((buf[1] == 0xFC) ||
		...
		for (retry = 5; retry > 0; retry--) {
			...
			//---clear CRC_ERR_FLAG---
			nvt_set_page(I2C_BLDR_Address, ts->crc_err_flag_addr);

			buf[0] = ts->crc_err_flag_addr & 0xFF;
			buf[1] = 0xA5;
...
static int32_t nvt_ts_check_chip_ver_trim(struct nvt_ts_hw_reg_addr_info hw_regs)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		buf[5] = 0x00;
		buf[6] = 0x00;
		CTP_I2C_WRITE(ts->client, I2C_BLDR_Address, buf, 7);
		...
		for (list = 0; list < (sizeof(trim_id_table) / sizeof(struct nvt_ts_trim_id_table)); list++) {
			...
			if (found_nvt_chip) {
				...
				ts->use_gcm = trim_id_table[list].hwinfo->use_gcm;
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    return sprintf(buf, "%s\n",
        data->gtp_is_suspend ? "true" : "false");
}
...
{
    struct nvt_ts_data *data = dev_get_drvdata(dev);
	NVT_LOG("start gtp_is_suspend:%d\n",data->gtp_is_suspend);
    if ((buf[0] == '1') && data->gtp_is_suspend == 0){
        nvt_ts_suspend(dev);
		data->gtp_is_suspend = 1;
    	}
    else if ((buf[0] == '0') && data->gtp_is_suspend == 1){
        nvt_ts_resume(dev);
		data->gtp_is_suspend = 0;
    	}
	NVT_LOG("start cmd:%c,gtp_is_suspend:%d\n",buf[0],data->gtp_is_suspend);

    return count;
}

DEVICE_ATTR_RW(ts_suspend);

static struct attribute *nvt36xx_dev_suspend_atts[] = {
		&dev_attr_ts_suspend.attr,
		NULL
};
static const struct attribute_group nvt36xx_dev_suspend_atts_group = {
    .attrs = nvt36xx_dev_suspend_atts,
};
static const struct attribute_group *nvt36xx_dev_attr_groups[] = {
    &nvt36xx_dev_suspend_atts_group,
    NULL
};

int nvt36xx_sysfs_add_device(struct device *dev) {
	int ret, i;
	for(i = 0; nvt36xx_dev_attr_groups[i]; i++) {
		ret = sysfs_create_group(&dev->kobj, nvt36xx_dev_attr_groups[i]);
        if (ret) {
            while (--i >= 0) {
                sysfs_remove_group(&dev->kobj, nvt36xx_dev_attr_groups[i]);
            }
            break;
        }
	}
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifndef ZCFG_TP_NOT_USE_SUSPEND_RESUME_NODE
	nvt36xx_sysfs_add_device(&client->dev);
#endif 
	if(zyt_check_cali_mode() == true){
	//	nvt_ts_suspend(ts);
	}

	NVT_LOG("nvt Driver probe successfully!\n");

	{
		...
		sprintf(tp_fm_ver,"0x%08x",ts->fw_ver);
		tp_fm_ver[10]='\0';
		
		zyt_info_s2("[TP] : nt36525","bh");
		zyt_info_s2("[NT36525BH] : FW Version:",tp_fm_ver);
	}
#if defined(ZCFG_TP_NOT_USE_SUSPEND_RESUME_NODE)
	...
	if(ret < 0){
    	NVT_ERR("Init drm notifier failed %d", ret);
...
#else
	nvt36xx_sysfs_remove_device(&client->dev);
#endif 	
