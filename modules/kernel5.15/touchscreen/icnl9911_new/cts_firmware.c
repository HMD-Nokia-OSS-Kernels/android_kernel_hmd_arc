...
#if defined(ZCFG_ICNL9911_DRIVER_BUILTIN_FIRMWARE_COMPATIBLE)
bool cts_driver_builtin_firmware_icnl9911c_qunchuang = false;
#endif
...
#if defined(ZCFG_ICNL9911_DRIVER_BUILTIN_FIRMWARE_COMPATIBLE)
void check_current_id_from_cmdline(void)
{
	...
	printk("check_current_id_from_cmdline: cmd_line = %s\n", cmd_line);

	if (strstr(cmd_line, "lcd_id=ID92"))
	{
		cts_driver_builtin_firmware_icnl9911c_qunchuang = false;
		printk("cmdline: enter icnl9911c_tianma\n");
	}
	else if (strstr(cmd_line, "lcd_id=ID21"))
	{
		cts_driver_builtin_firmware_icnl9911c_qunchuang = true;
		printk("cmdline: enter incl9911c_qunchuang\n");
	}
...
#if defined(ZCFG_ICNL9911_DRIVER_BUILTIN_FIRMWARE_COMPATIBLE)
#include "cts_builtin_firmware_icnl9911c_tianma.h"
#include "cts_builtin_firmware_icnl9911c_qunchuang.h"
#elif defined(ZCFG_FS301_CF8_KS239_ICNL9911_BUILTIN_FIRMWARE)
#include "cts_builtin_firmware_fs301_cf8_ks239.h"
#elif defined(ZCFG_FS301_XN_ICNL9911C_BUILTIN_FIRMWARE)
#include "cts_builtin_firmware_icnl9911c_jt_v247_20220718.h"
#elif defined(ZCFG_FS301L6_XN_ICNL9911C_BUILTIN_FIRMWARE)
#include "cts_builtin_firmware_fs301l6_xn_s932.h"
#elif defined(ZCFG_FS752L7_TBD_ICNL9911_BUILTIN_FIRMWARE)
#include "cts_builtin_firmware_icnl9911c_qunchuang.h"
#elif defined(ZCFG_FS301L3_TRX_ICNL9911_BUILTIN_FIRMWARE)
#include "cts_builtin_firmware_icnl9911c_zgd_qc.h"
#else
...
#if defined(ZCFG_ICNL9911_DRIVER_BUILTIN_FIRMWARE_COMPATIBLE)
	if (cts_driver_builtin_firmware_icnl9911c_qunchuang) {
		printk("builtin_firmware: enter incl9911c_qunchuang\n");
		firmware = cts_driver_builtin_firmwares_qc;
		for (i = 0; i < ARRAY_SIZE(cts_driver_builtin_firmwares_qc); i++, firmware++) {
		...
		printk("builtin_firmware: enter icnl9911c_tianma\n");
	    firmware = cts_driver_builtin_firmwares;
...
const struct cts_firmware *cts_request_firmware(
    const struct cts_device *cts_dev, u32 hwid, u16 fwid, u16 curr_firmware_ver)
{
...
#ifdef CFG_CTS_FIRMWARE_IN_FS
#ifdef CFG_CTS_FOR_GKI
	firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev, CFG_CTS_FIRMWARE_FILENAME,
    ...
    if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH)) {
        firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev, CFG_CTS_FIRMWARE_FILEPATH,
			firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
    }
#endif
#endif /* CFG_CTS_FIRMWARE_IN_FS */
