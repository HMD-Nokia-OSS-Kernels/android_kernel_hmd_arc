...
extern void check_current_id_from_cmdline(void);
#endif
...
int cts_driver_suspend(struct chipone_ts_data *cts_data)
{
    ...
    if (ret) {
	 ...
	 fp = filp_open_block(GESTURE_FUNCTION_SWITCH, O_RDONLY , 0);
	 ret = PTR_ERR(fp);
	 if(ret == -ENOTBLK){
		cts_info("open file %s success!\n", GESTURE_FUNCTION_SWITCH);
		cts_disable_gesture_wakeup(&cts_data->cts_dev);	
	 //	filp_close(fp, NULL); 
	 }else{
		cts_info("open file %s error!\n",GESTURE_FUNCTION_SWITCH);
