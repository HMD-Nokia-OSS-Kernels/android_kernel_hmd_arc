...
#else
	        CFG_CTS_FIRMWARE_FILEPATH_HAI_FEI,
#endif
	        firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
	}else if(ID_HUAYING_ID == id){
		firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
			cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME,
#else
			CFG_CTS_FIRMWARE_FILEPATH_HUA_YING,
#endif

	        firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
	}else if(ID_BOE_ID == id){
		firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
	        cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME,
#else
	        CFG_CTS_FIRMWARE_FILEPATH_BOE,
#endif
	        firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
	}else if(ID_DZX_ID == id){
		firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
			cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME,
#else
			CFG_CTS_FIRMWARE_FILEPATH_DZX,
#endif
...
out:
    ...
    if (ret == 0) {
		...
		cts_info("Resume device.(ret:%d),firmware_sect_size:%d,xchg_sram_base:%d,failed..",
		ret,firmware_info.firmware_sect_size,cts_dev->hwdata->sfctrl->xchg_sram_base);
	}
