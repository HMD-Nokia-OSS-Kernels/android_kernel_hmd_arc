...
#if (defined ZCFG_FS311L6_TRX_CD470_TP_ICNL9916_NAME)
extern int cts_get_cts9916_id(void);
#endif
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    if(ID_HAIFEI_ID == id)
    {
        zyt_info_s2("[LCDBrand] : ","HF:");
        zyt_info_s2("[TP] : ","HF:ICNL9916");
    }
    else
    {
        zyt_info_s2("[LCDBrand] : "," ");
        zyt_info_s2("[TP] : ","ICNL9916");
    }
#else
    zyt_info_s2("[TP] : ","ICNL9916");
#endif
    
    zyt_info_sx("[ICNL9916] : FW Version:", fwdata->version);
...
#endif
        {
        	cts_info("Resume device. is_normal_mode");
            break;
        }
		else{
			cts_info("Resume device retries:%d.",retries);
		}
    }
		 ...
		 if (firmware) {
		    ...
		    retries = 0;
				do {
					...
					{
			            //cts_err("when resume Update firmware OK.");
						cts_info("Need update firmware when resume OK---");
...
#ifdef CONFIG_CTS_TP_PROXIMITY
    cts_dev->rtdata.fw_status.proximity = false;
    cts_dev->rtdata.proximity_num = 0;
    cts_dev->rtdata.proximity_status = false;
#endif
}
...
int cts_enter_normal_mode(struct cts_device *cts_dev)
{
	...
	cts_info("Enter normal mode fail--(%d)",retries);
...
int cts_probe_device(struct cts_device *cts_dev)
{
    ...
    if (!cts_plat_is_normal_mode(cts_dev->pdata)) {
        ...
        if (ret) {
            ...
            ret = cts_tcs_get_fw_ver(cts_dev, &device_fw_ver);
            if (ret) {
