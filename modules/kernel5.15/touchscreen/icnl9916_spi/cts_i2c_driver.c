...
void cts_suspend_spi(struct chipone_ts_data *cts_data)
{
	cts_suspend(cts_data);
}
void cts_resume_spi(struct chipone_ts_data *cts_data)
{
	cts_resume(cts_data);
}
...
{
#ifdef CFG_CTS_GESTURE
	struct cts_platform_data *pdata = cts_data->pdata;
#endif
    int ret;
    ...
    if (cts_is_gesture_wakeup_enabled(&cts_data->cts_dev)) {
		...
		irq_set_irq_type(pdata->irq, IRQF_TRIGGER_HIGH | IRQF_NO_SUSPEND | IRQF_ONESHOT);
...
static int cts_resume(struct chipone_ts_data *cts_data)
{
    ...
    if (cts_is_gesture_wakeup_enabled(&cts_data->cts_dev)) {
		...
		irq_set_irq_type(pdata->irq,IRQF_TRIGGER_FALLING  | IRQF_NO_SUSPEND | IRQF_ONESHOT);
...
static int cts9916_id = ID_BOE_ID;
int cts_get_cts9916_id(void){
	return cts9916_id;
}
static int sprd_cts_get_panel(void)
{
    int ret = -1;
	char panel_id_info[32+1] = { 0 };
	#define KEY_PREFIX "lcd_id="
	struct device_node *cmdline_node;
	const char *cmd_line;
	char *sub;
	char key_prefix[] = KEY_PREFIX;
	//char ic_prefix[] = "icnl";
	char *d;
    int n, len, len_max = 50;
	...
	if (ret){
		cts_info("cmd_line failed.");
		cts9916_id = ID_DEFAULT_ID;
		return ret;
	}
    if (cmd_line) {
        //cts_info("cmd_line is %s", cmd_line);
        sub = strstr(cmd_line, key_prefix);
        if (sub) {
            d = strstr(sub, " ");
            if (d)
                n = strlen(sub) - strlen(d);
            ...
            strncpy(panel_id_info, sub, len);

			if( panel_id_info[13] == '9' &&  panel_id_info[14] == '9'){
				cts9916_id = ID_BOE_ID;
			}else if( panel_id_info[13] == '7' &&  panel_id_info[14] == '8'){
				cts9916_id = ID_HAIFEI_ID;
			}else if( panel_id_info[13] == '8' &&  panel_id_info[14] == '0'){
				cts9916_id = ID_HUAYING_ID;
			}else if( panel_id_info[13] == '6' &&  panel_id_info[14] == '3'){
				cts9916_id = ID_DZX_ID;
			}else{
				cts9916_id = ID_DEFAULT_ID;
			}

			cts_info("panel_id_info=%s,cts9916_id = %c,%c,0X%08X", panel_id_info,panel_id_info[13],panel_id_info[14],cts9916_id);

        } else {
            cts_info("chipone active panel_id_info not found!");
            return ret;
        }
    } else {
        cts_info("cmd_line null!");
        return ret;
    }
...
#endif
{
	...
	ret = sprd_cts_get_panel();
	if (ret) {
        cts_info("get chipone panel error");
    }
