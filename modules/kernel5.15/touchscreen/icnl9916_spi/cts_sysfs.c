...
#define COMP_CAP_TEST_DATA_FILENAME           "comp-cap-test-data.txt"
    ...
    rtc_time64_to_tm(ts.tv_sec, &rtc_tm);
...
static DEVICE_ATTR(factory_test,S_IRUGO,factory_test_show, NULL);
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (!manualdiff_base_updated) {
        ret = cts_tcs_top_get_rawdata(cts_dev, (u8 *) manualdiff_base,
            DIFFDATA_BUFFER_SIZE(cts_dev));
        if (ret)
...
{
	//scnprintf(buf, PAGE_SIZE, "proximity mode: %d\n",cts_dev->rtdata.fw_status.proximity);
...
static struct attribute *spi9916_dev_name_atts[] = {
	&dev_attr_name.attr,
	NULL,
};

static const struct attribute_group spi9916_dev_name_atts_group = 
{
	.attrs = spi9916_dev_name_atts,
};
...
extern void cts_suspend_spi(struct chipone_ts_data *cts_data);
extern void cts_resume_spi(struct chipone_ts_data *cts_data);
...
{
	struct chipone_ts_data *pdata = dev_get_drvdata(dev);
	cts_info("Reset device cmd:%c,status:%d.",buf[0],pdata->suspend);
	if ((buf[0] == '1') && pdata->suspend == false){
	//cts_suspend_device(&pdata->cts_dev);
	cts_suspend_spi(pdata);
	pdata->suspend = true;
		}
	else if ((buf[0] == '0') && pdata->suspend  == true){
	//cts_resume_device(&pdata->cts_dev);
	cts_resume_spi(pdata);
	pdata->suspend = false;
		}
	cts_info("Reset device cmd:%d,status:%d.",buf[0],pdata->suspend);
...
static bool b_gesture = false;
static ssize_t ts_gesture_show(struct device *dev,struct device_attribute *attr, char *buf)
{
...
static ssize_t ts_gesture_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cts_info("gesture device cmd:%c,status:%d.",buf[0],b_gesture);
	if ((buf[0] == '1')){
			cts_enable_gesture_wakeup(cts_dev);
			b_gesture = true;
	}
...
static DEVICE_ATTR_RW(ts_gesture);
static struct attribute *cts_dev_gesture_atts[] = {
		&dev_attr_ts_gesture.attr,
		NULL
	};


static const struct attribute_group cts_dev_gesture_atts_group = {
    .attrs = cts_dev_gesture_atts,
};
#endif
...
int cts_sysfs_add_device(struct device *dev)
{
    ...
    ret = sysfs_create_link(NULL, &dev->kobj, "chipone-tddi-icnl9916");
    if (ret)
...
#if 1 //def TS_SUSPEND_TOUCHSCREEN

	ret = sysfs_create_group(&dev->kobj, &spi9916_dev_name_atts_group);
	if (ret) {
		cts_err("Add device name_atts failed!");
	}

	ret = sysfs_create_link(NULL, &dev->kobj, "touchscreen");
    if (ret) {
        cts_err("Create sysfs touchscreen link error:%d", ret);
    }
...
void cts_sysfs_remove_device(struct device *dev)
{
    ...
    sysfs_remove_link(NULL, "chipone-tddi-icnl9916");
