...
int cts_tcs_i2c_write_trans(const struct cts_device *cts_dev,
...
int cts_tcs_i2c_read_trans(const struct cts_device *cts_dev,
        const u8 *wbuf, size_t wlen, void *rbuf, size_t rlen)
{
    ...
    do {
        ret = i2c_transfer(cts_data->i2c_client->adapter, &msg0, 1);
        if (ret != 1) {
            if (ret >= 0) {
                ret = -EIO;
            }
    ...
    do {
        ret = i2c_transfer(cts_data->i2c_client->adapter, &msg1, 1);
        if (ret != 1) {
            if (ret >= 0) {
                ret = -EIO;
            }
...
int cts_tcs_i2c_read(const struct cts_device *cts_dev,
        enum TcsCmdIndex cmdIdx, u8 *buf, size_t len)
{
    ...
    // dump_spi("read pack: ", cts_dev->pdata->i2c_fifo_buf, txlen);
    cts_tcs_i2c_read_trans(cts_dev, cts_dev->pdata->i2c_fifo_buf,
            txlen, buf, size);
    // dump_spi("read buf: ", buf, size);

    return 0;
}

int cts_tcs_i2c_write(const struct cts_device *cts_dev,
        enum TcsCmdIndex cmdIdx, u8 *wbuf, size_t wlen)
{
    ...
    // dump_spi("write: ", cts_dev->pdata->i2c_fifo_buf, txlen);
    cts_tcs_i2c_write_trans(cts_dev, cts_dev->pdata->i2c_fifo_buf, txlen);

    return 0;
}

int cts_tcs_i2c_xtrans(const struct cts_device *cts_dev, u8 *tx,
        size_t txlen, u8 *rx, size_t rxlen)
{
    if (rxlen > 0)
        cts_tcs_i2c_read_trans(cts_dev, tx, txlen, rx, rxlen);
    else
        cts_tcs_i2c_write_trans(cts_dev, tx, txlen);
    return 0;
}
        ...
        const u8 *wbuf, size_t wlen, void *rbuf, size_t rlen)
{
    ...
    if (ret) {
        cts_err("get touch data write msg failed");
        return ret;
    }
    for (i = 0; i <= times; i++) {
        rmsg.buf = rbuf + (i * max_size);
        rmsg.len = max_size;
        if (i == times)
            rmsg.len = end_len;
        if (i2c_transfer(cts_data->i2c_client->adapter, &rmsg, 1) != 1) {
            cts_err("i2c_transfer rmsg failed");
            ret = -EIO;
            break;
        }
        // dump_spi("get touch rbuf: ", rmsg.buf, rmsg.len);
    }
    return ret;
}

int cts_tcs_i2c_read_touch(const struct cts_device *cts_dev,
        enum TcsCmdIndex cmdIdx, u8 *buf, size_t len)
{
    int txlen;
    ...
    // dump_spi("get touch wbuf: ", cts_dev->pdata->i2c_fifo_buf, txlen);
    cts_tcs_i2c_read_trans_touch(cts_dev, cts_dev->pdata->i2c_fifo_buf,
            txlen, buf, len);
        ...
        size_t txlen, u8 *rx, size_t rxlen)
{
    ...
    if (cmd_recv != cmd_send) {
        cts_err("cmd check error, send %04x != %04x recv", cmd_send, cmd_recv);
        return -EIO;
    }
    ...
    enum TcsCmdIndex cmdIdx, u8 *wdata, size_t wdatalen)
{
    ...
    return cts_dev->pdata->spi_rx_buf[0];
}
#endif
...
{
#ifdef CONFIG_CTS_I2C_HOST
    cts_tcs_i2c_xtrans(cts_dev, tx, txlen, rx, rxlen);
#else
    cts_tcs_spi_xtrans(cts_dev, tx, txlen, rx, rxlen);
#endif
    return 0;
}
    ...
    enum TcsCmdIndex cmdIdx, u8 *buf, size_t len)
{
    int ret;
#ifdef CONFIG_CTS_I2C_HOST
    ret = cts_tcs_i2c_read(cts_dev, cmdIdx, buf, len);
#else
    ret = cts_tcs_spi_read(cts_dev, cmdIdx, buf, len);
#endif
    return ret;
}
    ...
    enum TcsCmdIndex cmdIdx, u8 *buf, size_t len)
{
    int ret;
#ifdef CONFIG_CTS_I2C_HOST
    ret = cts_tcs_i2c_write(cts_dev, cmdIdx, buf, len);
#else
    ret = cts_tcs_spi_write(cts_dev, cmdIdx, buf, len);
#endif
    return ret;
}
...
int cts_tcs_get_fw_ver(const struct cts_device *cts_dev, u16 *fwver)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_INFO_FW_VER_RO,
        buf, sizeof(buf));
    if (!ret) {
        *fwver = buf[0] | (buf[1] << 8);
        return 0;
...
int cts_tcs_get_lib_ver(const struct cts_device *cts_dev, u16 *libver)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_INFO_FW_VER_RO, buf,
        sizeof(buf));
    if (!ret) {
...
int cts_tcs_get_fw_id(const struct cts_device *cts_dev, u16 *fwid)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_INFO_CHIP_FW_ID_RO,
        buf, sizeof(buf));
    if (!ret) {
        *fwid = buf[0] | (buf[1] << 8);
        return 0;
...
int cts_tcs_clr_gstr_ready_flag(const struct cts_device *cts_dev)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_GSTR_DAT_RDY_FLAG_GSTR_RW,
        &ready, sizeof(ready));
    return ret;
}
...
int cts_tcs_clr_data_ready_flag(const struct cts_device *cts_dev)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_DAT_RDY_FLAG_RW,
        &ready, sizeof(ready));
    return ret;
}
...
int cts_tcs_enable_get_rawdata(const struct cts_device *cts_dev)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_DAT_TRANS_IN_NORMAL_RW,
        buf, sizeof(buf));
    if (ret != 0)
        return ret;
...
int cts_tcs_enable_get_cneg(const struct cts_device *cts_dev)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_CNEG_RD_EN_RW,
        buf, sizeof(buf));
    if (ret != 0)
        return ret;
...
int cts_tcs_quit_guesture_mode(const struct cts_device *cts_dev)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_MNT_FORCE_EXIT_MNT_WO,
        buf, sizeof(buf));
    if (ret != 0)
        return ret;
...
int cts_tcs_get_rawdata(const struct cts_device *cts_dev, u8 *buf)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_TP_DATA_RAW_RO, buf,
...
int cts_tcs_get_diffdata(const struct cts_device *cts_dev, u8 *buf)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_TP_DATA_DIFF_RO, buf,
...
int cts_tcs_get_basedata(const struct cts_device *cts_dev, u8 *buf)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_TP_DATA_BASE_RO, buf,
        DIFFDATA_BUFFER_SIZE(cts_dev));
...
int cts_tcs_get_cneg(const struct cts_device *cts_dev, u8 *buf, size_t size)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_TP_DATA_CNEG_RO, buf, size);
    ...
    u8 *regbuf, size_t size)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_SYS_STS_READ_RO, regbuf, size);
    if (ret != 0)
        return ret;
...
int cts_tcs_set_int_test(const struct cts_device *cts_dev, u8 enable)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_INT_TEST_EN_RW, &enable,
        sizeof(enable));
    if (!ret) {
        return 0;
    }
...
int cts_tcs_set_int_pin(const struct cts_device *cts_dev, u8 high)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_SET_INT_PIN_RW, &high,
        sizeof(high));
    if (!ret) {
        return 0;
    }
...
int cts_tcs_set_workmode(const struct cts_device *cts_dev, u8 workmode)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_WORK_MODE_RW,
...
int cts_tcs_set_openshort_mode(const struct cts_device *cts_dev, u8 mode)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_OPENSHORT_MODE_SEL_RW, &mode,
        sizeof(mode));
    if (!ret)
        return 0;
...
int cts_tcs_set_tx_vol(const struct cts_device *cts_dev, u8 txvol)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_VSTIM_LVL_RW, &txvol,
        sizeof(txvol));
    if (!ret)
        return 0;
    ...
    u8 short_type)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_OPENSHORT_SHORT_SEL_RW,
    ...
    u8 *enabled)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_OPENSHORT_EN_RW, buf,
        sizeof(buf));
    if (!ret) {
        *enabled = buf[0];
        return 0;
    ...
    u8 enable)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_OPENSHORT_EN_RW, &enable,
        sizeof(enable));
    if (!ret)
        return 0;
...
int cts_tcs_set_esd_enable(const struct cts_device *cts_dev, u8 enable)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_DDI_ESD_EN_RW, &enable,
        sizeof(enable));
    if (!ret)
        return 0;
...
int cts_tcs_is_cneg_enabled(const struct cts_device *cts_dev, u8 *enabled)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_CNEG_EN_RW, enabled,
        sizeof(*enabled));
    if (!ret)
        return 0;
...
int cts_tcs_is_mnt_enabled(const struct cts_device *cts_dev, u8 *enabled)
{
    ...
    ret = cts_tcs_read(cts_dev, TP_STD_CMD_MNT_EN_RW, enabled,
        sizeof(*enabled));
    if (!ret)
        return 0;
...
int cts_tcs_set_cneg_enable(const struct cts_device *cts_dev, u8 enable)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_CNEG_EN_RW, &enable,
        sizeof(enable));
    if (!ret)
        return 0;
...
int cts_tcs_set_mnt_enable(const struct cts_device *cts_dev, u8 enable)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_MNT_EN_RW, &enable,
        sizeof(enable));
    if (!ret)
        return 0;
...
int cts_tcs_set_pwr_mode(const struct cts_device *cts_dev, u8 pwr_mode)
{
    ...
    buf[0] = pwr_mode;
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_PWR_STATE_RW,
        buf, sizeof(buf));
    return ret;
}
...
int cts_tcs_set_display_on(const struct cts_device *cts_dev, u8 display_on)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_OPENSHORT_SHORT_DISP_ON_EN_RW,
    ...
    u8 direction)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_PANEL_DIRECTION_RW,
        buf, sizeof(buf));
    if (!ret) {
        return 0;
    }
...
int cts_tcs_set_int_data_types(const struct cts_device *cts_dev, u16 type)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_DATA_CAPTURE_FUNC_MAP_RW,
        (u8 *) &type, sizeof(type));
    if (!ret)
        return 0;
...
int cts_tcs_set_int_data_method(const struct cts_device *cts_dev, u8 method)
{
    ...
    ret = cts_tcs_write(cts_dev, TP_STD_CMD_SYS_STS_DATA_CAPTURE_EN_RW,
