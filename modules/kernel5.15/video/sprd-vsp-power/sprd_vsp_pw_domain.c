...
static int vsp_bbat(struct sprd_vsp_pd *vsp_pd)
{
	int ret = 0;
  
	pr_info("VSP_DOMAIN_EB shutdown begin\n");
...
static int vsp_pd_probe(struct platform_device *pdev)
{
	...
	if (mode_check("sprdboot.mode=autotest")) {
		ret = vsp_bbat(pd);
		if (!ret) {
			pr_info("%s: bbat mode and not probe\n", __func__);
		} else {
			pr_err("%s: bbat mode vsp eb shutdown failed\n", __func__);
		}
		return 0;
	}
