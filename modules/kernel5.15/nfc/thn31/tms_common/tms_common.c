...
#define TMS_DEVICE 1
#define TMS_NFC_DEVICE 1
#define TMS_ESE_DEVICE 0
#define TMS_GUIDE_DEVICE 1

/*********** PART0: Global Variables Area ***********/
struct tms_info *tms = NULL;
unsigned int tms_debug = LEVEL_DEBUG;
EXPORT_SYMBOL(tms_debug);
/*********** PART1: Declare Area ***********/
...
EXPORT_SYMBOL(tms_buffer_dump);

struct tms_info *tms_common_data_binding(void)
{
    return tms;
}
EXPORT_SYMBOL(tms_common_data_binding);
...
#if defined(TMS_NFC_DEVICE_MODULE)
    request_module("tms_nfc_module");
#endif
#if !defined(TMS_GUIDE_DEVICE) && !defined(TMS_GUIDE_DEVICE_MODULE)
#if defined(TMS_ESE_DEVICE_MODULE)
