...
static int nfc_device_open(struct inode *inode, struct file *file)
{
    ...
    TMS_DEBUG("Kernel version : %06x, TMS version : %s\n", LINUX_VERSION_CODE,
             DRIVER_VERSION);
    TMS_DEBUG("NFC driver version : %s\n", NFC_VERSION);
...
EXPORT_SYMBOL(nfc_device_probe);
...
EXPORT_SYMBOL(nfc_device_remove);
...
EXPORT_SYMBOL(nfc_device_suspend);
...
EXPORT_SYMBOL(nfc_device_resume);
#if !IS_ENABLED(CONFIG_TMS_GUIDE_DEVICE)
