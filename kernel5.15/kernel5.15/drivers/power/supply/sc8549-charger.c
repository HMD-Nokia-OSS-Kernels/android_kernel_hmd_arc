...
static int sc8549_check_charge_enabled(struct sc8549 *sc, bool *enabled)
{
	...
	*enabled = !!(val & SC8549_CHG_EN_MASK);
	dev_err(sc->dev, "wjy sc8549_check_charge_enabled, ret = %d , enabled=%d\n", ret,*enabled);
	return ret;
}
				       ...
				       union power_supply_propval *val)
{
	...
	dev_err(sc->dev, ">>>>>psp = %d\n", psp);

	switch (psp) {
	case POWER_SUPPLY_PROP_CALIBRATE:
		ret=sc8549_check_charge_enabled(sc, &sc->charge_enabled);
		dev_err(sc->dev, "wjy POWER_SUPPLY_PROP_CALIBRATE sc8549_check_charge_enabled, ret = %d , enabled=%d\n", ret,sc->charge_enabled);

			val->intval = sc->charge_enabled;
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		cmd = val->intval;
		dev_err(sc->dev, "%s, wjy1 POWER_SUPPLY_PROP_PRESENT val->intval = %d\n", val->intval);
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		dev_info(sc->dev, "%s, result = %d channel = vbat\n", __func__, result);
		if (!ret)
			sc->vbat_volt = result;

		val->intval = sc->vbat_volt * 1000;
		dev_err(sc->dev, "ADC_VBAT = %d\n", val->intval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (val->intval == CM_IBAT_CURRENT_NOW_CMD) {
			ret = sc8549_get_adc_data(sc, ADC_IBAT, &result);
			dev_info(sc->dev, "%s, result = %d channel = ibat\n", __func__, result);
		if (!ret)
				sc->ibat_curr = result;

			val->intval = sc->ibat_curr * 1000;
			dev_err(sc->dev, "ADC_IBAT = %d\n", val->intval);
			break;
		}

	ret=sc8549_check_charge_enabled(sc, &sc->charge_enabled);
	dev_err(sc->dev, "wjy POWER_SUPPLY_PROP_CURRENT_NOW sc8549_check_charge_enabled, ret = %d , enabled=%d\n", ret,sc->charge_enabled);
			if (!sc->charge_enabled) {
				val->intval = 0;
			} else {
				ret = sc8549_get_adc_data(sc, ADC_IBUS, &result);
				dev_info(sc->dev, "%s, result = %d channel = ibus\n", __func__, result);
				if (!ret)
					sc->ibus_curr = result;
				val->intval = sc->ibus_curr * 1000;
				dev_err(sc->dev, "ADC_IBUS = %d\n", val->intval);
			}
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		...
		dev_info(sc->dev, "%s, result = %d channel = vbus\n", __func__, result);
		if (!ret)
			sc->vbus_volt = result;

		val->intval = sc->vbus_volt * 1000;
		dev_err(sc->dev, "ADC_VBUS = %d\n", val->intval);
		break;
...
ret=sc8549_check_charge_enabled(sc, &sc->charge_enabled);
dev_err(sc->dev, "wjy POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT sc8549_check_charge_enabled, ret = %d , enabled=%d\n", ret,sc->charge_enabled);
	...
	ret=sc8549_check_charge_enabled(sc, &sc->charge_enabled);
	dev_err(sc->dev, "wjy POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT sc8549_check_charge_enabled, ret = %d , enabled=%d\n", ret,sc->charge_enabled);
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		if (val->intval == CM_USB_PRESENT_CMD)
			...
			dev_err(sc->dev, "%s, wjy POWER_SUPPLY_PROP_PRESENT val->intval = %d\n", val->intval);
		break;
...
static int sc8549_init_irq(struct sc8549 *sc)
{
	...
	dev_err(sc->dev, ">>>>>>>>>>>>%d\n", sc->irq_gpio);
	ret = gpio_request_one(sc->irq_gpio, GPIOF_DIR_IN,"sc8549");
		if (ret) {
			dev_err(sc->dev, "int request failed\n");
			return ret;
		}
...
if (sc->irq_response) {
	...
	cm_notify_event(sc->fc2_psy, CM_EVENT_INT, NULL);
		...
		if (sc8549_read_byte(sc, SC8549_REG_0F, &flag))
			dev_err(sc->dev, "%s, failed to clear interrupt,flag=0x%02X\n", __func__, flag);
	}
	return IRQ_HANDLED;
}
