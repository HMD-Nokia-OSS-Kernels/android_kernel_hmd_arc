...
#define BQ2560X_REG_PN_MASK		GENMASK(3, 0)
#define BQ2560X_REG_PN_SHIFT	0
...
static struct bq2560x_charger_reg_tab reg_tab[BQ2560X_REG_NUM + 1] = {
	...
	{12, BQ2560X_REG_C, "JEITA_VSET_L/JEITA_ISET_L_EN/JEITA_ISET_H/JEITA_VT2/JEITA_VT3"},
	{13, BQ2560X_REG_D, "EN_PUMPX/PUMPX_UP/PUMPX_DN/DP_VSET/DM_VSET/JEITA_EN"},
	{14, BQ2560X_REG_E, "INPUT_DET_DONE"},
	{15, BQ2560X_REG_F, "VREG_FT/STAT_SET/VINDPM_OS"},
	{16, 0, "null"},
};
...
static void power_path_control(struct bq2560x_charger_info *info)
{
	...
	dev_err(info->dev, "power_path_control power_path_control\n",cmd_line);
...
static int bq2560x_get_vendor_id(struct bq2560x_charger_info *info, u8 *data)
{
   u8 reg_val = 0;
   int ret = 0;
   ...
   reg_val = reg_val >> BQ2560X_REG_VENDOR_ID_SHIFT;
   *data = reg_val;
   return ret;
}
static int
...
bq2560x_charger_set_termina_vol(struct bq2560x_charger_info *info, u32 vol)
{
	...
	u8 vendor_id = 0;
	int ret;
	dev_err(info->dev, "%s:line%d: set termina vol = %d\n", __func__, __LINE__, vol);
	ret = bq2560x_get_vendor_id(info, &vendor_id);
	if (vendor_id == SGM41513_VENDOR_ID)
	{
		if (vol < 3856)
		reg_val = 0x0;
		else if (vol >= 4624)
...
bq2560x_charger_set_termina_cur(struct bq2560x_charger_info *info, u32 cur)
{
	...
	u8 vendor_id = 0;
	int ret;
	dev_err(info->dev, "%s:line%d: set termina cur = %d\n", __func__, __LINE__, cur);
	ret = bq2560x_get_vendor_id(info, &vendor_id);
	if (vendor_id == SGM41513_VENDOR_ID)
	{
		cur = cur*1000;
...
static void charge_ic_zyt_info(struct bq2560x_charger_info *info)
{
	u8 vendor_id = 0;
	int ret;
	...
	if (info->role == BQ2560X_ROLE_SLAVE) {
		zyt_info_s2("[CHARGE_SUBIC]:","SGM4151X_SLAVE");
	}else{
		if (vendor_id == SGM41513_VENDOR_ID)
		{
			zyt_info_s2("[CHARGE_IC]:","SGM41513");
		}
		else
		{
			zyt_info_s2("[CHARGE_IC]:","SGM4151X");
		}
	}
}
...
static int bq2560x_charger_start_charge(struct bq2560x_charger_info *info)
{
	...
	dev_err(info->dev, "bq2560x_charger_start_charge info->role= %d\n",info->role);
	...
	#if defined(ZCFG_CHG_BQ2560X_DUAL_CTRL_GPIO)	
...
static void bq2560x_charger_stop_charge(struct bq2560x_charger_info *info, bool present)
{
	...
	dev_err(info->dev, "%s:line%d: stop charge\n", __func__, __LINE__);
...
static int bq2560x_charger_set_current(struct bq2560x_charger_info *info, u32 cur)
{
	...
	u8 vendor_id = 0;
	int ret;
	dev_err(info->dev, "%s:line%d: set ibat cur = %d\n", __func__, __LINE__, cur);
	ret = bq2560x_get_vendor_id(info, &vendor_id);
	if (vendor_id == SGM41513_VENDOR_ID)
	{
		if (cur <= 40000)
			...
			reg_val = 0x08 + (cur -40000) / 10000;
		else if (cur < 130000)
			reg_val = 0x0F;
		else if (cur <= 270000)
			reg_val = 0x0F + (cur -110000) / 20000;
		else if (cur < 300000)
			reg_val = 0x17;
		else if (cur <= 540000)
			reg_val = 0x17 + (cur -270000) / 30000;
		else if (cur < 600000)
			reg_val = 0x20;
		else if (cur <= 1500000)
			reg_val = 0x20 + (cur -540000) / 60000;
		else if (cur < 1620000)
			reg_val = 0x30;
		else if (cur <= 2940000)
			reg_val = 0x30 + (cur -1500000) / 120000;
		else 
			reg_val = 0x3d;
	}
	else
					     ...
					     u32 *limit_vol)
{
	...
	if ((reg_val >> BQ2560X_REG_TERMINAL_VOLTAGE_SHIFT) == 0xF)
		*limit_vol = 3852;
	else
		...
		const struct i2c_device_id *id)
{
	...
	ret = bq2560x_get_vendor_id(info, &vendor_id);
	if (vendor_id != SGM41511_VENDOR_ID)
	{
		client->addr = 0x1A;
