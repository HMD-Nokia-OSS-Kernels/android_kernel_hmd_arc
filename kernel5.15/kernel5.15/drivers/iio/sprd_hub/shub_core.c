...
int peri_send_sensor_event_to_iio(u8 *data, u16 len)
{
    ...
    memset(event, 0x00, MAX_CM4_MSG_SIZE);
    memcpy(event, data, len);
    if (g_sensor->log_control.udata[5] == 1) {
        for (i = 0; i < len; i++)
    ...
    } else if (!g_sensor->indio_dev->active_scan_mask) {
...
EXPORT_SYMBOL_GPL(peri_send_sensor_event_to_iio);
...
//awinic sar debug start
#undef AWINIC_SAR_AWRW_DEBUG
#ifdef AWINIC_SAR_AWRW_DEBUG

static const uint32_t attr_buf[] = {
	8, 10,
	9, 100,
	10, 1000,
};

static uint8_t g_awinic_read_buf[1024] = { 0 };
static uint32_t g_awinic_read_len = 0;
static struct mutex g_aw_sar_lock;

static int awrw_shub_sipc_read(struct shub_data *sensor,
			  u8 reg_addr, u8 *data, u8 len)
{
	int err = 0;
	int wait_ret;
	uint8_t report_data_flag = 1;
	...
	shub_send_command(sensor, 161,
			  SHUB_SET_CALIBRATION_DATA_SUBTYPE,
			  &report_data_flag, sizeof(report_data_flag));
	wait_ret =
	    wait_event_interruptible_timeout(sensor->rxwq,
					     (sensor->rx_status),
					     msecs_to_jiffies
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	err = awrw_shub_sipc_read(sensor,
			     SHUB_SET_CALIBRATION_DATA_SUBTYPE,
			     g_awinic_read_buf, g_awinic_read_len);
	if (err < 0) {
		dev_err(&sensor->sensor_pdev->dev, "Read Cali Data Fail\n");
	...
	for (i = 0; i < g_awinic_read_len; i++) {
		len += snprintf(buf + len, PAGE_SIZE - len,
						"0x%02x,", g_awinic_read_buf[i]);
	}
...
#define AWRW_ONCE_PACK_TR_REG_NUM_MAX		(6)
static ssize_t awrw_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct shub_data *sensor = dev_get_drvdata(dev);
	int sensor_type = 161; //SAR_SENSOR_TYPE
	int err = -1;
	...
	uint32_t temp = 0;
	//¡À¡ê¨¢?1byte|?¨¢D¡ä¡À¨º?? 1byte|?¨¢D¡ä??¡ä??¡Â¨ºy?? 1byte|??¡ä??¡Â¦Ì??¡¤2byte|¨ºy?Ynbyte
	uint8_t send_buf[30] = { 0 };
	uint32_t len = 0;
	int i = 0;
	int pack_index = 0;
	int whole_send_pack_num = 0;
	uint16_t reg_addr = 0;
	...
	addr_bytes = (uint8_t)datatype[0];
	data_bytes = (uint8_t)datatype[1];
	reg_num = (uint8_t)datatype[2];

	//Judging Reading and Writing
	if (count > 7 + 5 * addr_bytes) {
		send_buf[1] = 0; //write
	} else {
		send_buf[1] = 1; //read
		g_awinic_read_len = reg_num * data_bytes;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		if (sscanf(temp_buf, "%02x", &temp) == 1)
			send_buf[3 + i % 24] = (uint8_t)temp;
	}
	reg_addr = (uint16_t)send_buf[3] << 8 | (uint16_t)send_buf[4];
	pr_info("reg_addr:0x%04x\n", reg_addr);
	len = 5;
	//parse write data
	if (send_buf[1] == 0) {
	/*	if (data_bytes * reg_num + 4 > sizeof(send_buf)) {
			dev_err(&sensor->sensor_pdev->dev, "Abnormal parameter length\n");
			mutex_unlock(&g_aw_sar_lock);
			return -EINVAL;
		}*/
		//¡Á¡é¨°a:?¨²set calio¡¥¨ºy?D¦Ì£¤¡ã¨¹¡¤¡é?¨ª¡Á?¡ä¨®3¡è?¨¨?a30¡Á??¨²¡ê??????a¦Ì£¤¡ã¨¹¡Á??¨¤¡¤¡é?¨ª6????¡ä??¡Â 6 * 4
		whole_send_pack_num = reg_num / AWRW_ONCE_PACK_TR_REG_NUM_MAX;
		for (pack_index = 0; pack_index < whole_send_pack_num; pack_index++) {
			len = 5;
			for (i = pack_index * data_bytes * AWRW_ONCE_PACK_TR_REG_NUM_MAX;
				i < (pack_index + 1) * data_bytes * AWRW_ONCE_PACK_TR_REG_NUM_MAX; i++) {
				if (reg_num < attr_buf[1]) {
					temp_buf[0] = buf[attr_buf[0] + (addr_bytes + i) * 5];
					temp_buf[1] = buf[attr_buf[0] + (addr_bytes + i) * 5 + 1];
				} else if (reg_num >= attr_buf[1] && reg_num < attr_buf[3]) {
					temp_buf[0] = buf[attr_buf[2] + (addr_bytes + i) * 5];
					temp_buf[1] = buf[attr_buf[2] + (addr_bytes + i) * 5 + 1];
				} else if (reg_num >= attr_buf[3] && reg_num < attr_buf[5]) {
					temp_buf[0] = buf[attr_buf[4] + (addr_bytes + i) * 5];
					temp_buf[1] = buf[attr_buf[4] + (addr_bytes + i) * 5 + 1];
				}
				sscanf(temp_buf, "%02x", &temp);
				send_buf[5 + i % 24] = temp;
				len++;
			}
			send_buf[2] = AWRW_ONCE_PACK_TR_REG_NUM_MAX;
			send_buf[3] = reg_addr >> 8;
			send_buf[4] = reg_addr;
			shub_send_command(sensor, sensor_type,
					SHUB_SET_CALIBRATION_DATA_SUBTYPE,
					send_buf, len);
			reg_addr += 4 * AWRW_ONCE_PACK_TR_REG_NUM_MAX;
		}

		if (reg_num % AWRW_ONCE_PACK_TR_REG_NUM_MAX != 0) {
			len = 5;
			for (i = whole_send_pack_num * data_bytes * AWRW_ONCE_PACK_TR_REG_NUM_MAX;
				i < data_bytes * reg_num; i++) {
				if (reg_num < attr_buf[1]) {
					temp_buf[0] = buf[attr_buf[0] + (addr_bytes + i) * 5];
					temp_buf[1] = buf[attr_buf[0] + (addr_bytes + i) * 5 + 1];
				} else if (reg_num >= attr_buf[1] && reg_num < attr_buf[3]) {
			...
			send_buf[2] = reg_num - whole_send_pack_num * AWRW_ONCE_PACK_TR_REG_NUM_MAX;
			send_buf[3] = reg_addr >> 8;
			send_buf[4] = reg_addr;
			shub_send_command(sensor, sensor_type,
					SHUB_SET_CALIBRATION_DATA_SUBTYPE,
		...
		err = shub_send_command(sensor, sensor_type,
				...
				send_buf, len);
		if (err < 0) {
			dev_err(&sensor->sensor_pdev->dev, "sar Write CalibratorData Fail\n");
			mutex_unlock(&g_aw_sar_lock);
			return err;
		}
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	
	{
		...
		char *TB_TYPENAME[] = {
				...
				"[Prox]:", "[Light]:", "[Prs]:", "[Sar]:"};
		char TB_TYPEORDER[] = {
				ORDER_ACC, ORDER_MAG, ORDER_GYRO,
				ORDER_PROX, ORDER_LIGHT, ORDER_PRS,ORDER_SAR};
		extern void zyt_info_s2(char* ,char*);
		for (i = 0; i < ARRAY_SIZE(sensor->sensor_info_list); i++)
		{
		dev_err(&sensor->sensor_pdev->dev,
				"lhx sensorlist_show sensor->sensor_info_list[%d].name = %s\n", i, sensor->sensor_info_list[i].name);
			if (sensor->sensor_info_list[i].name[0] != 0)
			{
				for(j = 0; j < ARRAY_SIZE(TB_TYPEORDER); j ++)
				{
					if(sensor->sensor_info_list[i].handle == TB_TYPEORDER[j])
					{
						zyt_info_s2(TB_TYPENAME[j],sensor->sensor_info_list[i].name);
					}
				}
			}
		}
