 ...
 */
//Modified by wangkang to solve the problem that manually selecting MIDI mode will automatically jump to a mode not used for data transmission (reference SPCSS01384430)
...
static void musb_sprd_chg_detect_work(struct work_struct *work)
{
	...
	case USB_CHG_STATE_UNDETECT:
		...
		#ifndef ZCFG_SHUTDOWN_ENTER_FASTCHARGER
			break;
		#endif
		}
	...
	case USB_CHG_STATE_RETRY_DETECT:
		...
		#ifdef ZCFG_SHUTDOWN_ENTER_FASTCHARGER
			if( (glue->chg_type == UNKNOWN_TYPE)&&(retry_cnt>0) ){  
            	dev_info(glue->dev, "charge detect retry cnt == %d\n",retry_cnt);
               	retry_cnt--;                                   
                glue->chg_state = USB_CHG_STATE_RETRY_DETECT;  
               	rework = true;
                delay = CHARGER_DETECT_DELAY;
